...
from deepeval.metrics.execution_efficiency.template import ExecutionEfficiencyTemplate
...
class ExecutionEfficiencyMetric(BaseMetric):
    ...
    async def _a_get_score(self, task: str, test_case: LLMTestCase):
        ...
        if self.using_native_model:
            res, cost = await self.model.a_generate(prompt, schema=EfficiencyVerdict)
            self.evaluation_cost += cost
            return res
        else:
            try:
    ...
    async def _a_extract_task_from_trace(
        self,
        test_case: LLMTestCase
    ) -> str:
        prompt = ExecutionEfficiencyTemplate.extract_task_from_trace(
