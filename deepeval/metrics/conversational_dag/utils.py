...
def is_valid_dag_from_roots(root_nodes: list[ConversationalBaseNode]) -> bool:
    visited = set()
...
def is_valid_dag(
    node: ConversationalBaseNode, visited=None, stack=None
) -> bool:
    if visited is None:
        visited = set()
    if stack is None:
...
def extract_required_params(
    nodes: list[ConversationalBaseNode],
    required_params: Optional[Set[TurnParams]] = None,
) -> Set[TurnParams]:
...
def copy_graph(original_dag: DeepAcyclicGraph) -> DeepAcyclicGraph:
    ...
    visited: Dict[ConversationalBaseNode, ConversationalBaseNode] = {}

    def copy_node(node: ConversationalBaseNode) -> ConversationalBaseNode:
        if node in visited:
            return visited[node]
