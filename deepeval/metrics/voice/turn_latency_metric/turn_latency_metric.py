...
class TurnLatencyMetric(BaseConversationalMetric):
        ...
        with metric_progress_indicator(
            ...
            for turn in test_case.turns:
                if turn.role != "assistant":
                    continue
                assistant_index += 1
                if turn.latency_ms is None or turn.audio is None:
                    continue
                duration_s = compute_audio_duration_seconds(turn.audio)
                if duration_s is None:
                    continue
                value = float(turn.latency_ms) + duration_s * 1000.0
                records.append(
                    (assistant_index, value, value <= self.max_latency_ms)
                )
            ...
            self.score, self.score_breakdown, reason = summarize_turn_scores(
                ...
                label="turn latency",
                unit=" ms",
                limit_description=f"<= {self.max_latency_ms:g} ms",
            )
