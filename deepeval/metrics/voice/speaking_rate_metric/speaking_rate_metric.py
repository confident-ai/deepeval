...
class SpeakingRateMetric(BaseConversationalMetric):
        ...
        with metric_progress_indicator(
            ...
            for turn in test_case.turns:
                if turn.role != "assistant":
                    continue
                assistant_index += 1
                if turn.audio is None or not turn.content:
                    continue
                ...
                records.append(
                    (assistant_index, wpm, self.min_wpm <= wpm <= self.max_wpm)
                )
            ...
            self.score, self.score_breakdown, reason = summarize_turn_scores(
                ...
                label="speaking rate",
                unit=" wpm",
                limit_description=f"{self.min_wpm:g}-{self.max_wpm:g} wpm",
            )
