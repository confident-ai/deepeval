...
class ClippingMetric(BaseConversationalMetric):
        ...
        with metric_progress_indicator(
            ...
            for turn in test_case.turns:
                ...
                if turn.audio is None:
                    continue
                value = compute_clipping_percentage(turn.audio)
                if value is None:
                    continue
                records.append(
                    (assistant_index, value, value <= self.max_clipping_pct)
                )
            ...
            self.score, self.score_breakdown, reason = summarize_turn_scores(
                ...
                label="clipping",
                unit="%",
                limit_description=f"<= {self.max_clipping_pct:g}%",
            )
