...
def set_typesafe_model_env(
    model: Optional[str] = typer.Option(
        ...
        "-m",
        "--model",
        help="Model identifier to use for TypeSafe (default: 'jev')",
    ),
    ...
    model = coerce_blank_to_none(model) or "jev"

    settings = get_settings()
    with settings.edit(save=save) as edit_ctx:
        edit_ctx.switch_model_provider(ModelKeyValues.USE_TYPESAFE_MODEL)
        ...
        if cost_per_output_token is not None:
            settings.TYPESAFE_COST_PER_OUTPUT_TOKEN = cost_per_output_token

    handled, path, updates = edit_ctx.result

    effective_model = settings.TYPESAFE_MODEL_NAME
    if not effective_model:
        raise typer.BadParameter(
            "TypeSafe model name is not set. Pass --model (or set TYPESAFE_MODEL_NAME).",
            param_hint="--model",
        )
    ...
    _handle_save_result(
        ...
        quiet=quiet,
        success_msg=(
            f":raising_hands: Congratulations! You're now using TypeSafe `{escape(effective_model)}` for all evals that require an LLM."
        ),
    )
...
def unset_typesafe_model_env(
    save: Optional[str] = typer.Option(
        ...
        "-s",
        "--save",
        help="Remove only the TypeSafe model related environment variables from a dotenv file. "
        "Usage: --save=dotenv[:path] (default: .env.local)",
    ),
    ...
    settings = get_settings()
    with settings.edit(save=save) as edit_ctx:
        settings.USE_TYPESAFE_MODEL = None
        settings.TYPESAFE_MODEL_NAME = None
        settings.TYPESAFE_COST_PER_INPUT_TOKEN = None
        settings.TYPESAFE_COST_PER_OUTPUT_TOKEN = None
        if clear_secrets:
    ...
    if _handle_save_result(
        ...
        save=save,
        quiet=quiet,
        updated_msg="Removed TypeSafe model environment variables from {path}.",
        tip_msg=None,
    ):
        if is_openai_configured():
            ...
            print(
                "The TypeSafe model configuration has been removed. No model is currently configured, but you can set one with the CLI or add credentials to .env[.local]."
            )
