"""`deepeval set-thegrid, unset-thegrid` commands."""
...
@app.command(name="set-thegrid")
def set_thegrid_model_env(
    ...
    prompt_api_key: bool = typer.Option(
        ...
        "--prompt-api-key",
        help=(
            "Prompt for THEGRID_API_KEY (input hidden). Not suitable for CI. "
    ...
    with settings.edit(save=save) as edit_ctx:
        edit_ctx.switch_model_provider(ModelKeyValues.USE_THEGRID_MODEL)
        if model is not None:
            settings.THEGRID_MODEL_NAME = model
        if api_key is not None:
            settings.THEGRID_API_KEY = api_key
        if base_url is not None:
            settings.THEGRID_BASE_URL = base_url
...
@app.command(name="unset-thegrid")
def unset_thegrid_model_env(
    save: Optional[str] = typer.Option(
        None,
        "-s",
        "--save",
    ...
    clear_secrets: bool = typer.Option(
        ...
        "-x",
        "--clear-secrets",
        help="Also remove THEGRID_API_KEY from the dotenv store.",
    ),
    quiet: bool = typer.Option(
        False,
        "-q",
    ...
    settings = get_settings()
    with settings.edit(save=save) as edit_ctx:
        settings.USE_THEGRID_MODEL = None
        settings.THEGRID_MODEL_NAME = None
        settings.THEGRID_BASE_URL = None
        if clear_secrets:
            settings.THEGRID_API_KEY = None
