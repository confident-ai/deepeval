...
@app.command(name="set-orcarouter")
def set_orcarouter_model_env(
    model: Optional[str] = typer.Option(
        None,
        "-m",
        "--model",
        help="Model identifier to use for this provider (e.g., `openai/gpt-5.4`).",
    ),
    prompt_api_key: bool = typer.Option(
        False,
        "-k",
        "--prompt-api-key",
        help=(
            "Prompt for ORCAROUTER_API_KEY (input hidden). Not suitable for CI. "
            "If --save (or DEEPEVAL_DEFAULT_SAVE) is used, the key is written to dotenv in plaintext."
        ),
    ),
    base_url: Optional[str] = typer.Option(
        ...
        "-u",
        "--base-url",
        help="Override the API endpoint/base URL used by this provider (default: https://api.orcarouter.ai/v1).",
    ),
    temperature: Optional[float] = typer.Option(
        None,
        "-t",
    ...
    cost_per_input_token: Optional[float] = typer.Option(
        ...
        help=(
            "USD per input token used for cost tracking. "
            "If unset and OrcaRouter does not return pricing metadata, "
            "costs will not be calculated."
        ),
    ),
    cost_per_output_token: Optional[float] = typer.Option(
    ...
    if prompt_api_key:
        api_key = coerce_blank_to_none(
            typer.prompt("OrcaRouter API key", hide_input=True)
        )
    ...
    settings = get_settings()
    with settings.edit(save=save) as edit_ctx:
        edit_ctx.switch_model_provider(ModelKeyValues.USE_ORCAROUTER_MODEL)
        if model is not None:
            settings.ORCAROUTER_MODEL_NAME = model
        if api_key is not None:
            settings.ORCAROUTER_API_KEY = api_key
        if base_url is not None:
            settings.ORCAROUTER_BASE_URL = base_url
        if temperature is not None:
            settings.TEMPERATURE = temperature
        if cost_per_input_token is not None:
            settings.ORCAROUTER_COST_PER_INPUT_TOKEN = cost_per_input_token
        if cost_per_output_token is not None:
            settings.ORCAROUTER_COST_PER_OUTPUT_TOKEN = cost_per_output_token

    handled, path, updates = edit_ctx.result

    effective_model = settings.ORCAROUTER_MODEL_NAME
    if not effective_model:
        raise typer.BadParameter(
            "OrcaRouter model name is not set. Pass --model (or set ORCAROUTER_MODEL_NAME).",
            param_hint="--model",
        )
    ...
    _handle_save_result(
        ...
        quiet=quiet,
        success_msg=(
            f":raising_hands: Congratulations! You're now using OrcaRouter `{escape(effective_model)}` for all evals that require an LLM."
        ),
    )


@app.command(name="unset-orcarouter")
def unset_orcarouter_model_env(
    save: Optional[str] = typer.Option(
        None,
        "-s",
        "--save",
        help="Remove only the OrcaRouter model related environment variables from a dotenv file. "
        "Usage: --save=dotenv[:path] (default: .env.local)",
    ),
    clear_secrets: bool = typer.Option(
        False,
        "-x",
        "--clear-secrets",
        help="Also remove ORCAROUTER_API_KEY from the dotenv store.",
    ),
    quiet: bool = typer.Option(
        False,
        "-q",
    ...
    settings = get_settings()
    with settings.edit(save=save) as edit_ctx:
        settings.USE_ORCAROUTER_MODEL = None
        settings.ORCAROUTER_MODEL_NAME = None
        settings.ORCAROUTER_BASE_URL = None
        settings.ORCAROUTER_COST_PER_INPUT_TOKEN = None
        settings.ORCAROUTER_COST_PER_OUTPUT_TOKEN = None
        # Intentionally do NOT touch TEMPERATURE here; it's a global dial.
        if clear_secrets:
            settings.ORCAROUTER_API_KEY = None
    ...
    if _handle_save_result(
        ...
        save=save,
        quiet=quiet,
        updated_msg="Removed OrcaRouter model environment variables from {path}.",
        tip_msg=None,
    ):
        if is_openai_configured():
            print(
            ...
            print(
                "The OrcaRouter model configuration has been removed. No model is currently configured, but you can set one with the CLI or add credentials to .env[.local]."
            )
