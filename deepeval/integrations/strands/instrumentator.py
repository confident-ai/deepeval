...
Traceloop attribute readers (shared with AgentCore) live in the sibling
``genai_attributes`` module.
"""
...
class StrandsInstrumentationSettings(BaseInstrumentationSettings):
    """Trace-level defaults for Strands instrumentation.

    See ``BaseInstrumentationSettings`` for the accepted kwargs.
    """

    DEFAULT_INTEGRATION = Integration.STRANDS.value
    ...
    def _serialize_framework_attrs(self, span) -> None:
        ...
        if model:
            ...
            if span_type == SpanType.LLM and not attrs.get(
                ...
                provider = get_attr(span, "gen_ai.response.provider")
                if not provider:
