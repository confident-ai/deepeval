"""Strands × deepeval OTel SpanInterceptor.
...
class StrandsSpanInterceptor(SpanProcessor):
    ...
    def _set_attr_post_end(span, key: str, value: Any) -> None:
        ...
        try:
            ...
            if attrs is not None:
                try:
                    ...
                    # OTel >= 1.44 freezes ReadableSpan attributes before
                    # processors run. The backing mapping remains mutable for
                    # processor-to-processor enrichment.
                    backing = getattr(attrs, "_dict", None)
        ...
        try:
            ...
            logger.debug(
                "Direct attributes write failed for %s; "
                "falling back to set_attribute (may be dropped): %s",
                key,
                exc,
            )
    ...
    def _serialize_trace_context_to_otel_attrs(self, span) -> None:
        ...
        if not (span.attributes or {}).get("confident.trace.thread_id"):
            for key in self.thread_id_attribute_keys:
                ...
                if thread_id:
                    self._set_attr_post_end(
                        span, "confident.trace.thread_id", thread_id
                    )
                    break
    ...
    def _serialize_framework_attrs(self, span) -> None:
        ...
        if not attrs.get("confident.span.integration"):
            self._set_attr_post_end(
                span, "confident.span.integration", self.integration.value
            )
        ...
        if span_type == "agent" and (
            "confident.span.name" not in attrs
            or attrs.get("confident.span.name") == span.name
        ):
            agent_name = _get_agent_name(span)
            if agent_name:
