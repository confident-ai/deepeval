...
def _get_agent_name(span) -> Optional[str]:
    ...
    return attrs.get("agent.name") or attrs.get("gen_ai.agent.name") or span.name or None
...
def _get_tool_name(span) -> Optional[str]:
    ...
    return attrs.get("tool.name") or attrs.get("gen_ai.tool.name") or span.name or None
...
class OpenInferenceSpanInterceptor(LiveSpanContext, SpanProcessor):
