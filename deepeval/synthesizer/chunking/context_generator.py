...
class ContextGenerator:
    def __init__(
        ...
        self.source_files_to_collections_map: Optional[Dict[str, Collection]] = None
    ...
    async def a_generate_contexts(
        ...
        if self.doc_to_chunker_map is not None:
            min_doc_token_count = min(chunker.text_token_count for chunker in self.doc_to_chunker_map.values())
            num_contexts_limit = 1 + math.floor((min_doc_token_count - self.chunk_size) / (self.chunk_size - self.chunk_overlap))
            if num_contexts_limit < num_context_per_document:
                suggested_chunk_size = (min_doc_token_count + (self.chunk_overlap * (num_context_per_document - 1))) // num_context_per_document
                raise ValueError(
        ...
        async def a_chunk_and_store(key, chunker: DocumentChunker):
            self.source_files_to_collections_map[key] = await chunker.a_chunk_doc()
            
        if self.source_files_to_collections_map == None:
            self.source_files_to_collections_map = {}
        if self.doc_to_chunker_map != None:
            tasks = [a_chunk_and_store(key, chunker) for key, chunker in self.doc_to_chunker_map.items()]
            await tqdm_asyncio.gather(*tasks, desc="✨ 📚 ✨ Chunking Documents")

        # Generate contexts
        for path, collection in tqdm_bar(self.source_files_to_collections_map.items(), desc="✨ 🧩 ✨ Generating Contexts"):
            num_chunks = collection.count()
            min_num_context = min(num_context_per_document, num_chunks)
            contexts.extend(self._get_n_random_contexts_per_doc(
                path=path, n_contexts_per_doc=min_num_context, context_size=max_context_size, similarity_threshold=0.7
            ))
            for _ in contexts:
    ...
    def generate_contexts(
        ...
        if self.doc_to_chunker_map is not None:
            min_doc_token_count = min(chunker.text_token_count for chunker in self.doc_to_chunker_map.values())
            max_num_contexts_possible = 1 + math.floor((min_doc_token_count - self.chunk_size) / (self.chunk_size - self.chunk_overlap))
        ...
        if self.source_files_to_collections_map == None:
            self.source_files_to_collections_map = {}
        if self.doc_to_chunker_map != None:
            for key, chunker in tqdm_bar(self.doc_to_chunker_map.items(), "✨ 📚 ✨ Chunking Documents"):
        ...
        for path, collection in tqdm_bar(self.source_files_to_collections_map.items(), desc="✨ 🧩 ✨ Generating Contexts"):
            ...
            min_num_context = min(num_context_per_document, num_chunks)
            contexts.extend(self._get_n_random_contexts_per_doc(
                path=path, n_contexts_per_doc=min_num_context, context_size=max_context_size, similarity_threshold=0.5
            ))
            for _ in contexts:
                source_files.append(path)
            self.total_chunks += num_chunks
    ...
    async def _a_load_docs(self):
        async def a_process_document(path):
            try:
                ...
                collection = client.get_collection(name=f"processed_chunks_{self.chunk_size}_{self.chunk_overlap}")
    ...
    def _get_n_random_contexts_per_doc(
        self, path: str, n_contexts_per_doc: int, context_size: int, similarity_threshold: int
    ):
        assert n_contexts_per_doc > 0, "n_contexts_per_doc must be a positive integer."
        ...
        random_chunks = self._get_n_random_chunks_per_doc(path=path, n_chunks=n_contexts_per_doc)
        ...
        for i in range(len(random_chunks)):
            ...
            for j, similar_chunk_text in enumerate(similar_chunk_texts):
                similar_chunk_similarity = 1 - similar_chunks["distances"][num_query_docs][j]
                if similar_chunk_text not in similar_chunk_texts and similar_chunk_similarity > similarity_threshold:
                    context.append(similar_chunk_text) 
            contexts.append(context)
        return contexts


    def _get_n_random_chunks_per_doc(self, path: str, n_chunks: int) -> Tuple[List[str], List[str]]: 
        collection = self.source_files_to_collections_map[path]
        total_chunks = collection.count()
        assert n_chunks <= total_chunks, f"Requested {n_chunks} chunks, but the collection only contains {total_chunks} chunks."

        # randomly sample n chunks from the collection
        n_random_ids = [str(i) for i in random.sample(range(total_chunks), n_chunks)]
        chunks = collection.get(ids=n_random_ids)
        return chunks["documents"]

    def check_if_docs_are_loaded(self):
        if self.doc_to_chunker_map == None and self.source_files_to_collections_map == None:
            raise ValueError(
