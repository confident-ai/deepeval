...
class DemonstrationSet:
    ...
    def to_text(self, max_demonstrations: Optional[int] = None) -> str:
        ...
        demos_to_use = (
            ...
            if max_demonstrations else self.demonstrations
        )
...
class DemonstrationBootstrapper:
    ...
    def _extract_input(self, golden: Union[Golden, ConversationalGolden]) -> str:
        """Strictly extract the input text, throwing errors on invalid state."""
        if isinstance(golden, Golden):
            if not golden.input:
                raise DeepEvalError("Golden must have a valid 'input' for MIPROv2 bootstrapping.")
            return golden.input
            
        else:
            user_turns = [t.content for t in (golden.turns or []) if t.role == "user"]
            if not user_turns:
                raise DeepEvalError("ConversationalGolden must have at least one 'user' turn for MIPROv2 bootstrapping.")
            return "\n".join(user_turns)

    def _extract_expected_output(self, golden: Union[Golden, ConversationalGolden]) -> Optional[str]:
        """Strictly extract the expected output/outcome if it exists."""
        if isinstance(golden, Golden):
            if not golden.expected_output:
                raise DeepEvalError("Golden must have a valid 'expected_output' for MIPROv2 bootstrapping.")
            return str(golden.expected_output)
        else:
            if not golden.expected_outcome:
                raise DeepEvalError("ConversationalGolden must have a valid 'expected_outcome' for MIPROv2 bootstrapping.")
    ...
    def bootstrap(
        ...
        max_attempts = min(len(goldens), self.max_bootstrapped_demonstrations * 3)
        ...
        for idx in shuffled_indices[:max_attempts]:
            ...
            if expected and len(labeled_demonstrations) < self.max_labeled_demonstrations * self.num_demonstration_sets:
                labeled_demonstrations.append(Demonstration(input_text=input_text, output_text=expected, golden_index=idx))

            if len(all_demonstrations) < self.max_bootstrapped_demonstrations * self.num_demonstration_sets:
                try:
                    # 1. Generate actual output
                    actual_output = self.scorer.generate(prompt_dict, golden)
                    
                    # 2. Build the test case safely
                    test_case = self.scorer._golden_to_test_case(golden, actual_output)
                    ...
                    if is_successful:
                        all_demonstrations.append(Demonstration(input_text=input_text, output_text=actual_output, golden_index=idx))
                except Exception:
                    continue

            if (len(all_demonstrations) >= self.max_bootstrapped_demonstrations * self.num_demonstration_sets and 
                len(labeled_demonstrations) >= self.max_labeled_demonstrations * self.num_demonstration_sets):
                break

        demo_sets = self._create_demonstration_sets(all_demonstrations, labeled_demonstrations)
    ...
    async def a_bootstrap(
        ...
        max_bootstrapped = self.max_bootstrapped_demonstrations * self.num_demonstration_sets
        tasks_info = tasks_info[:max_bootstrapped]

        async def evaluate_one(idx: int, input_text: str, expected: Optional[str]) -> Optional[Demonstration]:
            golden = goldens[idx]
            try:
                # 1. Generate actual output
                actual_output = await self.scorer.a_generate(prompt_dict, golden)
                ...
                # 4. Save if all metrics passed
                if is_successful:
                    return Demonstration(input_text=input_text, output_text=actual_output, golden_index=idx)
            except Exception:
                pass
            return None

        results = await asyncio.gather(*[evaluate_one(idx, inp, exp) for idx, inp, exp in tasks_info])
    ...
    def _create_demonstration_sets(
        ...
        demo_sets: List[DemonstrationSet] = [DemonstrationSet(demonstrations=[], id="0-shot")]

        for _ in range(self.num_demonstration_sets):
            demos: List[Demonstration] = []

            if bootstrapped_demonstrations:
                n_boot = min(self.max_bootstrapped_demonstrations, len(bootstrapped_demonstrations))
                demos.extend(self.random_state.sample(bootstrapped_demonstrations, n_boot))

            if labeled_demonstrations:
                n_labeled = min(self.max_labeled_demonstrations, len(labeled_demonstrations))
                labeled_sample = self.random_state.sample(labeled_demonstrations, n_labeled)
    ...
    if prompt.type == PromptType.LIST:
        ...
        for msg in prompt.messages_template:
            if not demo_added and msg.role == "system":
                new_messages.append(PromptMessage(role=msg.role, content=f"{msg.content}\n\n{demo_text}"))
                demo_added = True
            else:
        ...
        if not demo_added and new_messages:
            ...
            new_messages[0] = PromptMessage(role=first.role, content=f"{demo_text}\n\n{first.content}")
