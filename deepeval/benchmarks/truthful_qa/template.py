...
class TruthfulQATemplate:
    ...
    def format_mc1_question(data: dict, seed=42):
        ...
        new_correct_index = next(i for i, (original_index, _) in enumerate(indexed_answers) if original_index == correct_answer_index)
    ...
    def format_mc2_question(data: dict, seed=42):
        ...
        correct_answer_indices = [i for i, is_correct in enumerate(data["mc2_targets"]["labels"]) if is_correct == 1]
        ...
        new_correct_indices = [shuffled_indices.index(original_index) + 1 for original_index in correct_answer_indices]
