class MathQATemplate:
    ...
    n_shot_examples = [
        ...
        {'Problem': 'sophia finished 2 / 3 of a book . she calculated that she finished 90 more pages than she has yet to read . how long is her book ?', 'Rationale': 'let xx be the total number of pages in the book , then she finished 23 ⋅ x 23 ⋅ x pages . then she has x − 23 ⋅ x = 13 ⋅ xx − 23 ⋅ x = 13 ⋅ x pages left . 23 ⋅ x − 13 ⋅ x = 9023 ⋅ x − 13 ⋅ x = 90 13 ⋅ x = 9013 ⋅ x = 90 x = 270 x = 270 so the book is 270 pages long . answer : b', 'options': 'a ) 229 , b ) 270 , c ) 877 , d ) 266 , e ) 281', 'correct': 'b', 'annotated_formula': 'divide(90, subtract(const_1, divide(2, 3)))', 'linear_formula': 'divide(n0,n1)|subtract(const_1,#0)|divide(n2,#1)', 'category': 'general'},
        {'Problem': '120 is what percent of 50 ?', 'Rationale': '"50 * x = 120 - - > x = 2.4 - - > 2.4 expressed as percent is 240 % . answer : b ."', 'options': 'a ) 5 % , b ) 240 % , c ) 50 % , d ) 2 % , e ) 500 %', 'correct': 'b', 'annotated_formula': 'multiply(divide(120, 50), const_100)', 'linear_formula': 'divide(n0,n1)|multiply(#0,const_100)|', 'category': 'gain'},
        {'Problem': 'there are 10 girls and 20 boys in a classroom . what is the ratio of girls to boys ?', 'Rationale': 'if girls is 10 and boys is 20 , then 10 / 20 . so ratio of girls to boys is = 10 / 20 = 1 / 2 answer : a', 'options': 'a ) 1 / 2 , b ) 1 / 3 , c ) 1 / 5 , d ) 10 / 30 , e ) 2 / 5', 'correct': 'a', 'annotated_formula': 'divide(10, 20)', 'linear_formula': 'divide(n0,n1)', 'category': 'other'}
    ]
    ...
    def generate_output(input: str, n_shots: int):
        ...
        for i in range(n_shots):
            prompt += MathQATemplate.format_question(MathQATemplate.n_shot_examples[i])
        prompt += input
        return prompt
