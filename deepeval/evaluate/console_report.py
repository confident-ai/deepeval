...
class EvaluationConsoleReport:
    ...
    def export_to_cicd_markdown(
        ...
        output_dir: str,
        evaluation_name: str = "evaluation",
        overview_section_md: Optional[str] = None,
    ):
        """Build a CI/CD-friendly markdown report and write it to disk.

        Renders collapsible passing/failing test-case sections followed by an
        overview section. By default the overview is an Aggregate Metrics table;
        pass ``overview_section_md`` to swap in a custom section (e.g. the
        regression comparison table) instead.
        """
        os.makedirs(output_dir, exist_ok=True)
        ...
        failing_cases = [
            case for case in self.test_results if not case.success
        ]
        ...
        if overview_section_md is not None:
            # Custom overview (e.g. the regression comparison table) replaces
            # the default Aggregate Metrics table.
            md.append(overview_section_md)
            md.append("\n---\n")
        else:
            # Default overview: Aggregate Metrics table.
            metric_aggregates = {}
            for case in self.test_results:
                for m in case.metrics_data:
                    if m.name not in metric_aggregates:
