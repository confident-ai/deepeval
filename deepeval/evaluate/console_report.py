...
LIGHT_THEME = TerminalTheme(
    ...
    normal=[(0,0,0), (205,49,49), (13,188,121), (229,229,16), (36,114,200), (188,63,188), (17,168,205), (229,229,229)],
    bright=[(102,102,102), (241,76,76), (35,209,139), (245,245,67), (59,142,234), (214,112,214), (41,184,219), (229,229,229)]
)
...
class EvaluationConsoleReport:
    ...
    def _build_display_elements(self, truncate: bool = True) -> Group:
        ...
        renderables = [Panel(f"[{DEEPEVAL_PURPLE} bold]🚀 DeepEval Evaluation Results[/{DEEPEVAL_PURPLE} bold]", expand=True)]
        ...
        for case in self.test_results:
            ...
            if truncate and case.success:
                ...
                renderables.append(Panel(summary_text, border_style=status_color, expand=True))
                continue
            
            content_tree = Tree(f"[{status_color} bold]{status_icon} {case.name}[/{status_color} bold]")

            if case.conversational:
                convo_tree = content_tree.add("[bold cyan]Conversation Turns[/bold cyan]")
                for turn in case.turns:
                    convo_tree.add(f"[bold]{turn.role.capitalize()}:[/bold] {turn.content}")
            else:
                ...
                if case.expected_output and case.expected_output != "N/A":
                    data_table.add_row("Expected Output:", str(case.expected_output))
                content_tree.add(data_table)

            metrics_table = Table(title="Metrics", title_justify="left", show_edge=False, header_style=f"bold {DEEPEVAL_PURPLE}", expand=True)
            ...
            for m in case.metrics_data:
                m_icon = "[bold green]PASS[/bold green]" if m.success else "[bold red]FAIL[/bold red]"
                if m.error:
                    m_icon = "[bold red]ERROR[/bold red]"
                
                score_str = f"{m.score:.2f}" if m.score is not None else "N/A"
                thresh_str = f"{m.threshold:.2f}" if m.threshold is not None else "N/A"
                reason_str = str(m.reason or m.error or "N/A")
                
                if truncate and m.success and len(reason_str) > 50:
                    reason_str = reason_str[:47] + "..."

                metrics_table.add_row(m_icon, m.name, score_str, thresh_str, reason_str)

            content_tree.add(metrics_table)
            renderables.append(Panel(content_tree, border_style=status_color, padding=(1, 2), expand=True))
        ...
        for case in self.test_results:
            for m in case.metrics_data:
                if m.name not in metric_aggregates:
                    metric_aggregates[m.name] = {"total": 0, "passes": 0, "score_sum": 0, "score_count": 0}
        ...
        if metric_aggregates:
            ...
            agg_table = Table(title="[bold]Aggregate Metrics[/bold]\n", title_justify="left", show_edge=False, header_style=f"bold {DEEPEVAL_PURPLE}", expand=True)
            ...
            for metric_name, agg in metric_aggregates.items():
                avg_score = f"{agg['score_sum'] / agg['score_count']:.2f}" if agg["score_count"] > 0 else "N/A"
                pass_rate = f"{(agg['passes'] / agg['total']) * 100:.2f}%" if agg["total"] > 0 else "N/A"
                agg_table.add_row(metric_name, avg_score, pass_rate, str(agg["total"]))
            
            renderables.append(Panel(agg_table, border_style=DEEPEVAL_PURPLE, expand=True))

        return Group(*renderables)

    def render_to_terminal(self, truncate_passing_cases: bool = True):
        self.console.print()
        self.console.print(self._build_display_elements(truncate=truncate_passing_cases))
        self.console.print()

    def export_to_html(self, output_dir: str, evaluation_name: str = "evaluation", theme_mode: str = "dark"):
        os.makedirs(output_dir, exist_ok=True)
        
        safe_name = str(evaluation_name).replace(" ", "_").lower() if evaluation_name else "evaluation"
    ...
    def export_to_markdown(self, output_dir: str, evaluation_name: str = "evaluation"):
        os.makedirs(output_dir, exist_ok=True)
        ...
        for case in self.test_results:
            ...
            md.append("<details><summary><b>View Test Case Data</b></summary>\n")
            ...
            for m in case.metrics_data:
                m_icon = "✅" if m.success else ("❌" if not m.error else "⚠️ ERROR")
                score_str = f"{m.score:.2f}" if m.score is not None else "N/A"
                thresh_str = f"{m.threshold:.2f}" if m.threshold is not None else "N/A"
                reason_str = str(m.reason or m.error or "N/A").replace('\n', ' <br> ')
                md.append(f"| {m_icon} | **{m.name}** | {score_str} | {thresh_str} | {reason_str} |")
        ...
        if metric_aggregates:
            ...
            for metric_name, agg in metric_aggregates.items():
                avg_score = f"{agg['score_sum'] / agg['score_count']:.2f}" if agg["score_count"] > 0 else "N/A"
                pass_rate = f"{(agg['passes'] / agg['total']) * 100:.2f}%" if agg["total"] > 0 else "N/A"
                md.append(f"| **{metric_name}** | {avg_score} | {pass_rate} | {agg['total']} |")
    ...
    def export_to_cicd_markdown(self, output_dir: str, evaluation_name: str = "evaluation"):
        ...
        def render_cases(cases, is_passing):
            ...
            status_title = "✅ Passing Test Cases" if is_passing else "❌ Failing Test Cases"
            md.append(f"<details><summary><b>{status_title} ({len(cases)})</b></summary>\n")
