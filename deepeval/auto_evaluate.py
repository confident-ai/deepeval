...
def auto_evaluate(
    ...
    # Generate goldens 
    raw_goldens = generate_goldens(doc_nodes=doc_nodes, include_expected_output=include_expected_output, max_goldens_per_context=max_goldens_per_context, cache_dataset=cache_dataset)

    # Populate goldens by generating responses and extracting retrieval context
    populated_goldens = populate_goldens(target_model=target_model, goldens=raw_goldens, tracer=tracer)
...
def generate_goldens(doc_nodes: Dict, include_expected_output: bool, max_goldens_per_context: int, cache_dataset: bool = True):
    cache_folder = "cache"
    ...
    if os.path.exists(cache_file_path):
        ...
        if doc_nodes:
            goldens: List[Golden] = generate_goldens_from_nodes(doc_nodes, include_expected_output, max_goldens_per_context)
        else:
            # in the future generate goldens from prompts
            pass
        if cache_dataset:
            with open(cache_file_path, "w") as cache_file:
                json.dump([golden.model_dump() for golden in goldens], cache_file, indent=4)
    return goldens

def generate_goldens_from_nodes(doc_nodes: Dict, include_expected_output: bool, max_goldens_per_context: int):
...
def populate_goldens(target_model: Callable, goldens: List[Golden], tracer: Frameworks):
    async def populate_golden(golden: Golden):
        output = await target_model(golden.input)
        golden.actual_output = output
        golden.retrieval_context = trace_manager.get_track_params().get("retrieval_context")
        return golden

    async def populate_goldens(goldens: List[Golden]):
        tasks = [populate_golden(golden) for golden in goldens]
        goldens = await atqdm.gather(*tasks, desc="Generating LLM Responses", total=len(tasks))
        return goldens
