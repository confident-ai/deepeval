...
describe("extractOpenRouterMetadata", () => {
  ...
  test("always carries a provider label, since TS spans have no field for it", () => {
    expect(extractOpenRouterMetadata({}).provider).toBe("OpenRouter");
    expect(extractOpenRouterMetadata({}, "MyGateway").provider).toBe(
      "MyGateway",
    );
  });
  ...
  test("omits fields the response did not carry", () => {
    ...
    expect(metadata).toEqual({ provider: "OpenRouter" });
  });
  ...
  test("never throws on a hostile response", () => {
    ...
    expect(extractOpenRouterMetadata(hostile).provider).toBe("OpenRouter");
  });
});
