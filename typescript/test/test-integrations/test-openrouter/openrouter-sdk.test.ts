...
//
// All HTTP is stubbed, so this runs offline and needs no key. It covers what a
// trace fixture cannot pin: streaming passthrough, unpatching, the provider
// override, and the exact metadata values. The fixture-based counterpart is
// `openrouter.test.ts`.
//
...
describe("native @openrouter/sdk wrapper", () => {
  ...
  test("the provider label can be overridden by the user", async () => {
    ...
    const metadata = onlyLlmSpan(trace).metadata.openrouter;
    expect(metadata.provider).toBe("MyRouter");
  ...
  test("nests under an enclosing observe() span", async () => {
    ...
    expect(llm.metadata.openrouter.provider).toBe("OpenRouter");
  });
  ...
  test("spans reach traceManager under evalsIterator", async () => {
    ...
    expect(llmSpans[0].metadata.openrouter.provider).toBe("OpenRouter");
  }, 30000);
