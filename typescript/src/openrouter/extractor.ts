...
/**
 * Unwrap the Speakeasy request envelope.
 *
 * `chat.send({ chatRequest })` and `responses.send({ responsesRequest })` both
 * nest the real payload one level down; fall back to the argument itself so a
 * shape change upstream degrades to "no input captured" rather than throwing.
 */
