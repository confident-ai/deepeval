...
export class PromptAlignmentMetric extends BaseMetric {
  ...
  private async generateVerdicts(
    ...
    return generateQagVerdicts(this, {
      systemOne: this.systemOneVerdictSpec(input, actualOutput),
      llm: async () => {
  ...
  private systemOneVerdictSpec(
    input: string,
    actualOutput: string,
  ): SystemOneVerdictSpec<string, PromptAlignmentVerdict> {
    return {
      instructions: this.getPrompt("_experimental_system_one_verdict"),
      items: this.promptInstructions,
      itemKey: "instruction",
      state: { input, actual_output: actualOutput },
    };
  }
  ...
  private calculateScore(): number {
    ...
    const total = Math.max(
      ...
      this.promptInstructions.length,
    );
    if (total === 0) return 1;
