...
export type ImageReferenceTemplateOverride =
  MetricTemplateOverride<"ImageReferenceMetric">;

export interface ImageReferenceMetricOptions {
  ...
  evaluationTemplate?: ImageReferenceTemplateOverride;
}

/**
 * Image Reference — does each image in `actualOutput` be referenced by the
 * surrounding text? Scores each image (0–10) against its text context via a
 * vision model, then averages. **Higher is better**.
 */
export class ImageReferenceMetric extends BaseMetric {
  private readonly maxContextSize?: number;

  constructor(options: ImageReferenceMetricOptions = {}) {
  ...
  async measure(testCase: LLMTestCase): Promise<number> {
    ...
    try {
      ...
      for (const idx of imageIndices) {
        ...
        const instructions = this.getPrompt("evaluate_image_reference", {
          context_above: above ?? "",
          context_below: below ?? "",
        });
