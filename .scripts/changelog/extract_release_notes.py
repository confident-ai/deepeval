...
def main():
    parser = argparse.ArgumentParser(description="Extract release notes for a specific tag from the generated changelog")
    parser.add_argument("--tag", required=True, help="Tag to extract notes for (e.g. v0.21.45)")
    parser.add_argument("--changelog-dir", default="docs/content/changelog", help="Directory containing changelog MDX files")
    parser.add_argument("--output", default="release_notes.md", help="Output file for the extracted notes")
    ...
    if contributors:
        ...
        out_lines.append("")
        out_lines.append("A huge thank you to our contributors for this release: " + ", ".join(formatted_contributors))
        out_lines.append("")

    with open(args.output, "w", encoding="utf-8") as f:
        f.write("\n".join(out_lines).strip() + "\n")
        
    print(f"Successfully extracted release notes for {args.tag} to {args.output}")
