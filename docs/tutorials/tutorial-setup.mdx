...
All `deepeval` test files must start with the prefix of `test_`, so create a file named `test_app.py` and paste the following code in the file.
...
You can run your first evaluation by running the following command in your terminal.
...
So you'll set your `OPENAI_API_KEY` as an environment variable.
...
While DeepEval works great standalone, you can connect it to [Confident AI](https://www.confident-ai.com) — our cloud platform for LLM evaluation dashboards, logging, team collaboration and many more. **Best of all, it's free to get started!** _(No credit card required.)_
