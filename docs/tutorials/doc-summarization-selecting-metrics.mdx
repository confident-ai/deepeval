...
concision_metric = GEval(
    name="Concision",
    criteria="Assess if the generated output remains concise while preserving all essential information.",
...
:::tip
While a brief evaluation criteria (e.g., "the generated output must be concise") is acceptable, it's generally better to be more **specific** when defining such criteria for `GEval`, which is a LLM-Judge metric. This ensures that metric scores remain consistent across evaluation runs, given the non-deterministic nature of LLMs.
:::
...
completeness_metric = GEval(
    name="Completeness",
    criteria="Assess whether the generated output retains all key information from the input.",
