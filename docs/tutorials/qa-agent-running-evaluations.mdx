...
1. An `EvaluationDataset` populated with the generated answers (`generated_output`) and the `retrieval_context` from the knowledge base for each question.
2. A selection of metrics to evaluate our `EvaluationDataset` on.

In the previous section, we defined `AnswerRelevancy` and `Faithfulness` as our metrics of choice. While we already have an `EvaluationDataset`, it has not yet been populated with these parameters. Our first step is to generate the generated outputs and retrieval contexts for each `Golden` for evaluation.
...
:::note
By default, `auto_convert_goldens_to_test_cases` is set to `True`, but we'll set it to `False` for this tutorial since the generated output is a required parameter in an `LLMTestCase`, and we haven't generated them yet.
:::
...
Next, we’ll iterate through each `Golden`'s inputs and generate the `generated_output` and `retrieval_context` for each synthetic user query.

```python
from deepeval.test_case import LLMTestCase

for golden in dataset.goldens:
    # Compute generated output and retrieval context
    generated_output = qa_agent.generate(golden.input)  # Replace with logic to compute generated output
