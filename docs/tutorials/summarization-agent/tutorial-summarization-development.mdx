...
In this section, we're going to create our **meeting summarization agent** using the OpenAI API. Our summarization agent should be able to take an entire meeting transcript as `input` and return a **concise summary** and a **list of action items** from the transcript.
