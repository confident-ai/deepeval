...
:::tip
`deepeval` provides the `@observe` decorator that helps you monitor your LLM application's entire workflow, and it requires only a single line of code. It is highly recommended to add this tracing to your application right from the development stage to make your dev life easier.
:::

:::note
If you already have an LLM-based summarization agent that you want to evaluate, feel free to skip to the [**evaluation section of this tuorial**](tutorial-summarization-evaluation).
:::
...
This also gives us more flexibility when evaluating our summarizer in the later stages. For example:
