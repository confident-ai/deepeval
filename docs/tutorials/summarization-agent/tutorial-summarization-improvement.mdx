...
In this situation, you can either use more test cases to run evaluations or use the `deepeval`'s lates `ArenaGEval` to test which model is better among the for test cases with summaries and action items combined in them. You can learn more about it [here](http://localhost:3000/docs/metrics-arena-g-eval).
