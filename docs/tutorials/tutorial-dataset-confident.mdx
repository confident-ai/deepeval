...
2. Compute the generated outputs and retrieval contexts, and **convert your goldens into test cases**.
...
To pull a dataset from Confident AI, simply call the `pull` method from an `EvaluationDataset` and specify the dataset alias you wish to retrieve. By default, `auto_convert_goldens_to_test_cases` is set to `True`, but we'll set it to `False` for this tutorial since the generated output is a required parameter in an `LLMTestCase`, and we haven't generated them yet.
...
for golden in dataset.goldens:
    ...
    generated_output = "..."  # Replace with logic to compute generated output
