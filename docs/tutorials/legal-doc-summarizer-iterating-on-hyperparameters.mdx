...
>
  <video width="100%" autoPlay loop muted playsInlines>
    <source
      src="https://confident-docs.s3.us-east-1.amazonaws.com/tutorial-legal-document-summarizer-hyperparameters.mp4"
      type="video/mp4"
    />
  </video>
</div>
...
Our prompt change was effective, improving the previously failing summary so that it now passes the test case. However, the new prompt template has introduced an entirely new issue with the third test case, which is now failing the concision metric.
...
:::note
In the next section, we'll learn how to compare evaluation test runs and catch regressions.
:::
