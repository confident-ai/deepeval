...
:::info
You can also create a dataset by uploading a CSV file or by creating goldens from scratch. `Goldens` are test cases `where` generated_outputs haven't been populated yet. They make up the golden dataset, and you can [learn more about them here](/docs/evaluation-datasets#with-goldens).
:::
...
You'll notice that only the inputs corresponding to your document texts are populated. This allows you to generate different summaries (`generated_output`) for each iteration of your summarizer.
