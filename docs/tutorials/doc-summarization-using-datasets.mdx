...
:::note
By default, `auto_convert_goldens_to_test_cases` is `True`, but it will raise an error if your dataset, `Legal Documents Dataset`, hasn't been populated with summaries in the `generated_output` field, which is a mandatory field in a test case. [Learn more about test cases here](/docs/evaluation-test-cases).
:::
...
for golden in dataset.goldens:
    generated_output = llm.summarize(golden.input)  # Replace with logic to compute generated output
