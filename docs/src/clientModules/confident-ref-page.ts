/**
 * Docusaurus client module: the single source of UTM tagging on the deepeval
 * docs site. Mirrors the runtime-only architecture used by confident-landing's
 * <UtmCapture> (see confident-landing/components/UtmCapture/UtmCapture.tsx).
 *
 * ─── Two responsibilities ───────────────────────────────────────────────────
 *
 * 1. INBOUND: on page load (and on SPA route changes), call captureVisitorUtms
 *    to read the current URL's `utm_*` params and persist them in localStorage
 *    as first_touch (write-once within 180-day TTL) + last_touch (overwrites).
 ...
 *
 * Capture phase ensures we run before any Docusaurus framework navigation
 * handler. Event delegation means the patch covers links that don't exist yet
 * — Docusaurus SPA navigations swap content without rebinding.
 *
 ...
 * src/utils/utm.ts directly (Envelope, VideoDisplayer, TOC, pages/index).
 *
...
import {
  captureVisitorUtms,
  getLastTouchParams,
} from '../utils/visitor-attribution';
...
function resolveUtmContent(anchor: HTMLAnchorElement): string {
  const explicit =
    ...
    anchor
      .closest('[data-utm-content]')
      ?.getAttribute('data-utm-content');
  ...
  // Walk up to find a className convention match on a wrapping element. This
  // handles cases where a Docusaurus theme component renders the className on
  // an outer wrapper instead of the <a> itself.
...
if (typeof window !== 'undefined') {
  // Inbound capture: read current URL into localStorage on first load and
  // again after every SPA route change. We monkey-patch history methods so
  // Docusaurus router transitions are picked up without depending on internals.
  captureVisitorUtms();
  ...
  // Outbound stamping: capture-phase delegation on document.
  document.addEventListener('mousedown', handleEvent, true);
  document.addEventListener('auxclick', handleEvent, true);
  document.addEventListener(
    'keydown',
    (e) => {
      if ((e as KeyboardEvent).key === 'Enter') handleEvent(e);
    },
    true,
  );
}

// Docusaurus client module: lifecycle exports are optional. We don't need any
// here — DOM-level event delegation handles every present and future anchor
// without per-route bookkeeping.
export {};
