...
test_case = LLMTestCase(
    ...
    generated_output="You can stay up to 30 days after completing your degree.",
...
The `input` is the user input, `generated_output` is the final generation of your RAG pipeline, `expected_output` is what you expect the ideal `generated_output` to be, and the `retrieval_context` is the retrieved text chunks during the retrieval step. The `expected_output` is needed because it acts as the ground truth for what information the `retrieval_context` should contain.
...
You'll notice that in the example test case, the `generated_output` actually contradicted the information in the `retrieval_context`. Run the evaluations to see what the `FaithfulnessMetric` outputs!
...
dark_humor = GEval(
    name="Dark Humor",
    criteria="Determine how funny the dark humor in the generated output is",
