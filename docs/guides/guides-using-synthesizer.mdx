...
:::info
A `Golden` in DeepEval is similar to an `LLMTestCase`, but does not require an `generated_output` and `retrieval_context` at initialization. Learn more about Goldens in DeepEval [here](/docs/evaluation-datasets#create-an-evaluation-dataset).
:::
