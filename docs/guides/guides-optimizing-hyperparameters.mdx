...
:::info
In `deepeval`, hyperparameters refer to independent variables that affect the final `generated_output` of your LLM application, which includes the LLM used, the prompt template, temperature, etc.
:::
...
- execute evaluations in a nested for-loop, while generating `generated_outputs` **at evaluation time** based on the current hyperparameter combination

:::note
In reality, you don't have to strictly generate `generated_outputs` at evaluation time and can evaluate with datasets of precomputed `generated_outputs`, but you ought to ensure that the `generated_outputs` in each [`LLMTestCase`](/docs/evaluation-test-cases) can be properly identified by a hyperparameter combination for this to work.
:::

Let's walkthrough a quick example hypothetical example showing how to find the best model and prompt template hyperparameter combination using the `AnswerRelevancyMetric` as a measurement. First, define a function to generate `generated_output`s for `LLMTestCase`s based on a certain hyperparameter combination:
...
def construct_test_cases(model: str, prompt_template: str) : List[LLMTestCase]:
    ...
    for input in list_of_inputs:
        test_case = LLMTestCase(
            input=input,
            # Hypothetical function to generate generated outputs
            # at evaluation time based on your hyperparameters!
            generated_output=generate_generated_output(llm, prompt)
        )
