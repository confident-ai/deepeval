...
Evaluating agents in development is all about benchmarking with datasets and metrics. Your metrics will tackle either the reasoning or action layer (see the [AI Agent Evaluation Metrics guide](/guides/guides-ai-agent-evaluation-metrics) for a detailed breakdown of each metric), while datasets make sure you're comparing different iterations of your agents on the [same set of goldens.](/docs/evaluation-datasets)
