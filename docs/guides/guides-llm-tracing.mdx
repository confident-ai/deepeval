---
id: guides-llm-tracing
title: Tracing LLM Applications
sidebar_label: Tracing LLM Applications
---
...
flowchart TD
    ...
    T --> R["Root Span<br/>answer_user_query()<br/>type: base"]
...
:::note
If you do not auto-patch your client, you will need to manually log token counts and model names using `update_llm_span()` to capture cost and usage metrics.
:::
...
@observe(metrics=[correctness_metric])  # Root span — evaluates the full pipeline output
def answer_user_query(user_query: str) -> str:
    context = retrieve_context(user_query)
    return generate_response(user_query, context)
```
...
:::tip
Use component-level metrics (on `retriever` and `llm` spans) to diagnose *where* your pipeline is failing. Use end-to-end metrics (on the root span) to measure whether the pipeline is succeeding for the user. Both are most useful together.
:::
...
3. **Explore agent tracing** — Learn how to track complex tool execution in the [Agentic Tracing guide](/guides/guides-agentic-tracing)
