...
### `mistral-{small,large,medium}-latest` through Mistral

Begin by installing the `instructor` and `mistralai` packages via pip:
...
class CustomMistral(DeepEvalBaseLLM):
    def __init__(self):
        ...
        self.llm = 'mistral-small-latest' # or 'mistral-large-latest' or 'mistral-medium-latest'
    ...
    def get_model_name(self):
        return f"Custom Mistral Model ({this.llm})"
