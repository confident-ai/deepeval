...
Every tab below uses the same matcher: the golden is the subject, and `run` produces the trace judged against it. Importing `deepeval/vitest` registers `toPass()`, and `npx deepeval test run` injects it for you.
