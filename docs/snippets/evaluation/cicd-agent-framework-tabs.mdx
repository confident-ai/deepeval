...
`toPass()` only needs the golden — the test case is built from the captured trace. Importing `deepeval/vitest` registers the matcher, and `npx deepeval test run` injects it for you.
