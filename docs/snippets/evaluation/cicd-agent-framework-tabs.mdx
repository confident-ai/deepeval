...
```typescript title="test_llm_app.ts" showLineNumbers
...
```typescript title="test_langchain_app.ts" showLineNumbers
...
```typescript title="test_openai_app.ts" showLineNumbers
...
// 2. Instrument your agent (patch your OpenAI client)
...
```typescript title="test_openai_agents_app.ts" showLineNumbers
...
The golden is the subject; `run` produces the trace judged against it. Importing `deepeval/vitest` registers the matcher, and `npx deepeval test run` injects it for you.

<NotImplemented id="typescript" feature="deepeval.integrations">

The adapters that do ship — LangChain, OpenAI, OpenAI Agents, the Vercel AI SDK and Mastra — don't have pages here yet. Until they land, instrument your agent with `observe` as above: it produces the same trace and span structure an adapter would, so the metrics you attach score the same components.
