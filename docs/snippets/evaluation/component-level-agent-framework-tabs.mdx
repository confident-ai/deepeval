...
The adapters that do ship — LangChain, OpenAI, OpenAI Agents, the Vercel AI SDK and Mastra — don't have pages here yet. Until they land, instrument those apps with `observe` as above: it produces the same trace and span structure an adapter would, so the metrics you attach score the same components.
