...
Build your agent with create_agent, pass deepeval's CallbackHandler to its invoke / ainvoke method, and set your metrics on the dataset before the loop. The CallbackHandler traces the agent automatically; the dataset-level metrics will score the resulting spans once the run finishes.
...
dataset.metrics = [AnswerRelevancyMetric()]
...
By declaring `dataset.metrics` before the loop, the metric is visible when `evals_iterator()` starts. The `CallbackHandler` drains it onto the first LLM span the LangChain agent opens. If you need to score individual tool calls or retrievers, see the [LangChain integration](/integrations/frameworks/langchain) for advanced patterns like `next_tool_span` or `next_retriever_span`.

</Tab>
<Tab value="LangGraph">

Wire your `StateGraph`, pass `deepeval`'s `CallbackHandler` to its `invoke` / `ainvoke` method, and declare your metrics on the dataset before the loop. The `CallbackHandler` drains the dataset-level metric onto the first LLM span the graph emits.
...
By declaring `dataset.metrics` before the loop, the metric is visible when `evals_iterator()` starts. The `CallbackHandler` drains it onto the first LLM span the graph emits. To score every LLM call individually, you can set metrics directly on the `CallbackHandler` (`CallbackHandler(metrics=[...])`). See the [LangGraph integration](/integrations/frameworks/langgraph) for the full surface.
