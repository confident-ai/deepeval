...
Framework-specific callbacks (LangChain, LlamaIndex, and friends) aren't documented here yet. Until they land, call your framework directly inside the callback as above — the simulator only cares that each call resolves to the next assistant `Turn`.
