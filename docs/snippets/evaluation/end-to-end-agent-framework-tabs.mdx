...
```typescript title="langchain_app.ts" showLineNumbers
...
```typescript title="openai_app.ts" showLineNumbers
...
`evalsIterator()` is async-only, so there's no sync variant to choose between — goldens are evaluated concurrently and awaited by the loop. See [tracing](/docs/evaluation-llm-tracing) for the full `observe` and `updateCurrentTrace` surface.
