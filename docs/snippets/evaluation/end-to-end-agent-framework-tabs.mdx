...
`evalsIterator()` is async-only, so there's no sync variant to choose between — goldens are evaluated concurrently and awaited by the loop.

<Tabs items={["Manual Instrumentation", "LangChain", "Mastra", "LangGraph", "OpenAI", "OpenAI Agents", "Vercel AI SDK"]}>
<Tab value="Manual Instrumentation">

Wrap the top-level function with `observe` and call `updateCurrentTrace(...)` to set the trace-level test case fields:
...
See [tracing](/docs/evaluation-llm-tracing) for the full `observe` and `updateCurrentTrace` surface.

</Tab>
<Tab value="LangChain">

Build your agent with `createAgent`, then pass `deepeval`'s `DeepEvalCallbackHandler` to its `invoke` method inside the loop:
...
`evalsIterator()` waits for the exporter to settle before scoring, so no manual flush is needed inside an eval. See the [Mastra integration](/integrations/frameworks/mastra) for the full surface.

</Tab>
<Tab value="LangGraph">

Wire your `StateGraph`, then pass the same `DeepEvalCallbackHandler` to its `invoke` method inside the loop:
...
Call `instrumentOpenAI(client)` once on the client you already construct — every completion or response call it makes becomes an LLM span under the trace:
...
See the [OpenAI integration](/integrations/frameworks/openai) for the full surface.

</Tab>
<Tab value="OpenAI Agents">

Register `DeepEvalTracingProcessor` once with the agents SDK, then run your goldens through the agent:
...
Call `configureAiSdkTracing(...)` once at startup, then pass the returned tracer into `experimental_telemetry` on every call you want traced:
...
A call without `isEnabled` and a `tracer` emits no spans at all. See the [Vercel AI SDK integration](/integrations/frameworks/ai-sdk) for the full surface.
