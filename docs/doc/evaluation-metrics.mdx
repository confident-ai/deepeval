...
test_case = LLMTestCase(input=input, output=actual_output)
...
These are the additional RAGAS metrics `deepeval` supports:
...
As an overview here are what each metric measures:

- `RagasFaithfulnessMetric` - ensures output align with context. Penalizes claims not deduced from the context. Calculated using the `actual_output` and `context`.

- `RagasContextualRelevancyMetric` - assesses the relevance of retrieved contexts to input. Penalizes redundant information. Based on the `input` and `context`.

- `RagasContextRecallMetric` - gauges the recall of the retrieved context using the annotated answer as a reference. Based on the `expected_output` and `context`.

- `RagasAnswerRelevancyMetric` - measures how directly a response addresses a question. Penalizes redundancy or incompleteness. Derived from the `input` and `actual_output`.

## Top-K Similarity

This metric is different from the rest because it measures `SearchTestCase` instead of `LLMTestCase` as introduced in the [test cases](/docs/evaluation-test-cases) section. Unlike an `LLMTestCase`, the `SearchTestCase` focuses solely on the retrieval results, and is especially relevant for assessing retrieval pipelines in RAG applications.
...
actual_retrieval_results = ["The Eiffel Tower is located in Paris.", "Jupiter is the largest planet in our solar system."]
ideal_retrieval_results = ["Paris is home to the Eiffel Tower.", "In our solar system, Jupiter holds the title for the biggest planet."]

ranking_similarity_metric = RankingSimilarity(minimum_score=0.5)
test_case = SearchTestCase(actual_retrieval_results, ideal_retrieval_results)

run_test(test_case, [ranking_similarity_metric])
```

Here, `actual_retrieval_results` will be retrieval results from your RAG application and would typically come from your vector database of choice.
