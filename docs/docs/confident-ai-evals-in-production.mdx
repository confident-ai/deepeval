...
Tracking events allow you to monitor LLM responses in production. The `track()` function returns an `event_id` upon a successful API request to Confident's servers.
...
You can use this `event_id` to send human feedback regarding a particular LLM response.
...
Using an `event_id` returned from `track()`, here's how you can send feedback to Confident:
