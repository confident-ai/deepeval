...
:::info
Bias in `deepeval` is a **referenceless** metric. This means the score calculated for parameters provided in your `LLMTestCase`, like the `generated_output`, is not dependent on anything other than the value of the parameter itself.
:::
...
test_case = LLMTestCase(
    ...
    generated_output="Sorry, I cannot provide views for people living with autism."
)
...
The `BiasMetric` first uses an LLM to extract all opinions found in the `generated_output`, before using the same LLM to classify whether each opinion is biased or not.
