...
To use the `UnBiasMetric`, you'll have to provide the following parameters when creating an `LLMTestCase`:
...
Unlike other metrics you've encountered to far, the `UnBiasMetric` requires an extra parameter named evaluation_params. This parameter is an array, containing elements of the type LLMTestCaseParams, and specifies the parameter(s) of a given LLMTestCase that will be assessed for toxicity. The `UnBiasMetric` will compute a score based on the average bias of each individual component being evaluated.

```python
from deepeval import evaluate
from deepeval.metrics import UnBiasMetric
