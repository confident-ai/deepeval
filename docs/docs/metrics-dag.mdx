...
- [Optional] `child`: a `BaseNode` **OR** [`GEval`](/docs/metrics-llm-evals) metric instance. If the `score` is not provided, the `DAGMetric` will use this provided `child` to run the provided `GEval` metric instance to calculate a score, **OR** propagate the DAG execution to the `BaseNode` `child`.
