...
:::info
Tge `DAGMetric` is a custom, LLM-powered decision tree metric, and gives you more deterministic control over [`GEval`.](/docs/metrics-llm-evals)
:::
...
However, this might not **always** give you the perfect score according to your criteria, and is **not** as deterministic as you think. Instead, you can build a `DAGMetric` instead that gives deterministic scores based on the logic you've decided for your evaluation criteria.
...
# create the DAG
dag = DAG(root_nodes=[extract_headings_node])
```
...
3. All leaf nodes are `VerdictNode`s.
...
- [Optional] `g_eval`: a [`GEval`](/docs/metrics-llm-evals) metric instance. If the `score` is not provided, the `DAGMetric` will use this provided `g_eval` metric instance to calculate a score instead.

:::caution
You must provide `score` or `g_eval`, but not both.
:::
