...
:::info
The **Multimodal Faithfulness** is the multimodal adaptation of DeepEval's [faithfulness metric](metrics-faithfulness). It accepts images in addition to text for the `input`, `actual_output`, and `retrieval_context`.
:::

## Required Arguments

To use the `MultimodalFaithfulnessMetric`, you'll have to provide the following arguments when creating a [`MLLMTestCase`](evaluation-test-cases#mllm-test-case):
