...
No, they are separate products. DeepEval is an open-source LLM evaluation framework that works entirely locally. Confident AI is a cloud platform for LLM evaluation, regression testing, tracing, and observability. The founders of Confident AI created DeepEval, and as a result `deepeval` integrates natively with Confident AI — but you are **not** required to use Confident AI to use `deepeval`.
...
[Confident AI](https://confident-ai.com) is the cloud platform built by the same team that created DeepEval. While DeepEval is the open-source evaluation framework that runs locally, Confident AI is where teams collaborate on testing AI — from development to production. It provides:

- **Shareable test reports** with regression testing across runs
- **Dataset management** with annotation tools for non-technical team members
- **LLM tracing & observability** with automatic online evals on every trace, span, and thread
- **Production monitoring** with real-time quality metrics

`deepeval` integrates natively with Confident AI — results from `evaluate()` and `deepeval test run` are automatically uploaded when you're logged in. It's free to get started:
...
[DeepTeam](https://www.trydeepteam.com/docs/getting-started) is an open-source framework for **red teaming LLM systems**, built by the same team behind DeepEval and Confident AI. While DeepEval focuses on evaluation (correctness, relevancy, faithfulness, etc.), DeepTeam is dedicated to **security and safety testing**.
...
### Do I need all three (DeepEval, DeepTeam, Confident AI)?

No. Each product works independently:

- **DeepEval only** — great for local, open-source LLM evaluation during development.
- **DeepEval + Confident AI** — adds cloud-based test reports, production tracing, online evals, and team collaboration.
- **DeepEval + DeepTeam** — covers both quality evaluation and security/safety red teaming.
- **All three** — the full stack for teams that want evaluation, red teaming, and production monitoring in one ecosystem.

Start with whichever piece solves your most immediate need and layer in the rest as your requirements grow.
