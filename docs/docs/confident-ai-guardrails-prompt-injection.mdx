...
`guard_result.breached` is a boolean that is `True` if the guard has been breached. The `score_breakdown` for `PromptInjectionGuard` is a dictionary containing:
