...
- **Emphasizes on Top Results**: WCP places a stronger emphasis on the relevance of top-ranked results. The ranking order is important are because LLMs tend to focuse more on earlier nodes in the `retrieval_context`, which when ranked incorrectly may cause downstream hallucination in the generation step of your RAG pipeline.
...
A higher contextual precision score represents a greater ability of the retrieval system to rank relevant nodes higher in the `retrieval_context`.
