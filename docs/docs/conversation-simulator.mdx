...
- [Optional] `simulator_model`: a string specifying which of Anthropic's Claude models to use for generation, **OR** [any custom LLM model](/docs/metrics-introduction#using-a-custom-llm) of type `DeepEvalBaseLLM`. Defaulted to `claude-3-7-sonnet-latest`.
