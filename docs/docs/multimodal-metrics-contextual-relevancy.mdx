...
:::info
The **Multimodal Contextual Relevancy** is the multimodal adaptation of DeepEval's [contextual relevancy metric](metrics-contextual-relevancy). It accepts images in addition to text for the `input`, `actual_output`, and `retrieval_context`.
:::

## Required Arguments

To use the `MultimodalContextualRelevancyMetric`, you'll have to provide the following arguments when creating a [`MLLMTestCase`](evaluation-test-cases#mllm-test-case):
