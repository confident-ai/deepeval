...
We can now use the nodes we've just created to create a `DeepAcyclicGraph` and use it to create a `ConversationalDAGMetric`. Here's how to do that:
...
:::tip
Building your nodes in a bottom up fashion makes it easier to create a valid DAG and understand the flow.
:::

This is exactly the same DAG we've seen in the example images till now. We can now pass this `dag` to `ConversationalDAGMetric` and run an evaluation.

```python title="main.py"
from deepeval.metrics import ConversationalDAGMetric
...

playful_chatbot = ConversationalDAGMetric(
    name="Playful Chatbot",
    dag=dag,
)
```

You can now pass a `ConversationalTestCase` and this metric in the `evaluate` function to run your evals.
