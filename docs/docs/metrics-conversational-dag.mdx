...
The `ConversationalDAGMetric` can be used to evaluate entire conversations on a decision based criteria. This allows us to create robust evaluations on conversations unlike any other metrics. The conversational dag also allows us to use the regular conversational metrics to run evaluations on the final level.

```python
from deepeval.metrics import ConversationalDAGMetric

metric = ConversationalDAGMetric(
    name="Custom Conversational DAG Metric",
    dag=dag
)
```
...
![DAG Image for Multi-Turn](https://deepeval-docs.s3.us-east-1.amazonaws.com/docs:conversational-dag.png)

<details>
<summary><strong>Why use DAG?</strong></summary>
...
A `DeepAcyclicGraph` for a `ConversationalDAGMetric` can be created using the conversational nodes from `deepeval`.

## Conversational DAG nodes

There are exactly **FOUR** different node types you can choose from to create a `DeepAcyclicGraph`.
...
![DAG with turns window](https://deepeval-docs.s3.us-east-1.amazonaws.com/docs:conversational-dag-turn-window.png)
:::
...
There are **THREE** mandatory and **THREE** optional parameter when creating a `ConversationalTaskNode`:
...
There are **TWO** mandatory and **THREE** optional parameter when creating a `ConversationalBinaryJudgementNode`:
...
There are **TWO** mandatory and **THREE** optional parameter when creating a `ConversationalNonBinaryJudgementNode`:
...
ConversationalVerdictNode(verdict="Good", score=9),
```

There is **ONE** mandatory and **TWO** optional parameter when creating a `ConversationalVerdictNode`:
...
Now that we've learnt everything about the Conversational DAG. Let's take an example use case to create an actual DAG and use the `ConversationalDAGMetric` to evaluate our DAG.

We will take an example conversation between a user and an assistant. In this conversation, we will systematically evaluate whether the assistant is satisfying the user's requests while being playful with the user. Here's an example conversational test case:
...
test_case = ConversationalTestCase(
    turns=[
        ...
        Turn(role="assistant", content="Why do you care bro?"),
        ...
        Turn(role="assistant", content="Are you stupid?"),
    ],
...
<Timeline>
<TimelineItem title="Summarizing the conversation">

Sometimes, conversations can get pretty long which means we might have to summarize it entirely while keeping important details specific for our use case.  The `ConversationalTaskNode` allows us to perform tasks like this on our test cases.
...
You can also pass a `turn_window` to focus on just some parts of the conversation as needed.

:::note
There are no children for this node yet, however, we will modify these individual nodes later to create a final DAG.
:::

</TimelineItem>
<TimelineItem title="Checking user's satisfaction">
...
binary_node = ConversationalBinaryJudgementNode(
    ...
    children=[
        ConversationalVerdictNode(verdict=False, score=0),
        ConversationalVerdictNode(verdict=True, child=10),
    ],
)
```

Here the `score` for satisfaction is 10. This will later change to a `child` that allows us to traverse a new path if user was satisfied.

</TimelineItem>
<TimelineItem title="Classifying assistant's behavior">

Decisions like behaviour analysis can be a multi-class classification. We will use the `ConversationalNonBinaryJudgementNode` to classify assiatant's behaviour from a given list of options from our verdicts.
...
<TimelineItem title="Connecting the nodes">
...
Here, we can see that we've made the `non_binary_node` as the child for `binary_node` when `verdict` is `True`. We have also made the `binary_node` as the child of `task_node` after the summary has been extracted.

With this we've successfully created our DAG (deep acyclic graph) with the `task_node` as our root node.
...
We have created exactly the same DAG as shown in the above example images. We can now pass this `dag` to `ConversationalDAGMetric` and run an evaluation.
