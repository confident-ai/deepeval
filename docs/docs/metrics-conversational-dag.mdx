...
We can now use these nodes to create a `DeepAcyclicGraph` and pass it in a `ConversationalDAGMetric`. We will first create all our nodes separately in a top-down approach.

:::note
We will create the same DAG as shown in the example images till now.
:::

### Creating DAG

<Timeline>
<TimelineItem title="Using task node to extract summary">

We will use the `ConversationalTaskNode` to extract the summary of the entire conversation.
...
<TimelineItem title="Classifying summary using the binary judgement node">

We will use the `ConversationalBinaryJudgementNode` to make a binary classification that determines whether the user was satisfied or not.
...
We will use the `ConversationalNonBinaryJudgementNode` to classify assiatant's behaviour towards user to see if it matches the ideal behaviour pattern.
...
<TimelineItem title="Connecting the DAG">
...
Use the `evaluate` function with conversational test cases and this metric to run evals.
...
Get your eval results by running the following command:
