...
:::tip
You can also provide a [`name` when creating an `LLMTestCase`](evaluation-test-cases#name). This is particularly useful if you're importing test cases from an external datasource and would like a way to identify and search for them on Confident AI.
:::
