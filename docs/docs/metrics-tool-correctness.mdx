...
The tool correctness metric can also be used to evaluate your LLM's ability to call the right MCP primitives. Learn more about [MCP evaluations here](https://www.deepeval.com/docs/evaluation-mcp).
...
Click here to learn more about [how to create a single-turn MCP test case](www.deepeval.com/docs/evaluation-mcp#single-turn).
...
test_case = LLMTestCase(
    ...
    # Your MCP related arguments here (if any)
)
...
<Equation formula="\text{Tool Correctness} = \text{AlignmentScore(Primitives Used, Primitives Available)}" />

The **Alignment Score** measures whether the primitives used were the best options from all the available primitives, it is judged by an LLM.