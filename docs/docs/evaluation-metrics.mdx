...
Hallucination metric measure the factual correctness between the `actual_output (output of LLM/typically summary)` of your LLM application to the provided `context`.  You'll have to supply `context` when creating an `LLMTestCase` to evaluate hallucination metric. 
...
:::note
This metric uses vectara's hallucination evaluation model, which was specifically designed for summarization task. However, if you are just looking for factual consistency between input and output 
where the application is not designed for QA task, this metric should work. 
:::
