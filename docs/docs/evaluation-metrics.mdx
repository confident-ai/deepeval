...
Hallucination metric measures whether your LLM application is saying things that are untrue by comparing the `actual_output' to the provided `context`. You'll have to supply `context`when creating an`LLMTestCase` to evaluate hallucination.
