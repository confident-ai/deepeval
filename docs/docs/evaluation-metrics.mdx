...
`RankingSimilarity` focuses solely on the context retrieved from your knowledge base at runtime, and is especially relevant for assessing retrievers in RAG applications.
...
# Replace this with the actual retrieved context from RAG pipeline
retrieval_context = ["Only shoes can be refunded."]

ranking_similarity_metric = RankingSimilarity(minimum_score=0.5)
test_case = LLMTestCase(input=input, actual_output=actual_output, context=context, retrieval_context=retrieval_context)

run_test(test_case, [ranking_similarity_metric])
```

Here, `retrieval_context` will be retrieval results from your RAG application and would typically come from your vector database of choice.
