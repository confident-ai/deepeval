...
`deepeval` only tracks errors and exceptions raised within the package for the purpose of improving the package and **does not collect any user or company data in any way**. To disable error reporting, set the `NO_ERROR_REPORTING` environment variable to 'Y'.

```console
export NO_ERROR_REPORTING="Y"
```
