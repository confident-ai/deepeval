...
`deepeval` uses `Sentry` to track only very basic telemetry data (number of evaluations run and which metric is used). Personally identifiable information is explicitly excluded.
By default any telemetry data is not sent, but you can enable it to help the team iterate on product better, by using an environment variable:

```console
export DEEPEVAL_ENABLE_TELEMETRY="YES"
```
