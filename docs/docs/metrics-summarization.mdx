...
These scores are calculated by generating `n` closed-ended questions that can only be answered with either a 'yes or a 'no', and seeing whether the source document and summary yields the same answer.
