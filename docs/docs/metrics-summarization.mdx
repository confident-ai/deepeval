...
These scores are calculated by generating `n` of closed-ended questions that can only be answered with a 'yes or a 'no', and seeing whether the source document and summary yields the same answer.
