...
These scores are calculated by generating `n` closed-ended questions that can only be answered with either a 'yes or a 'no', and taking the ratio of which the source document and summary yields the same answer.
