...
The summarization metric uses LLM-as-a-judge to determine whether your LLM (application) is generating factually correct summaries while including the necessary details from the original text. In a summarization task within `deepeval`, the original text refers to the `input` while the summary is the `generated_output`.
...
Let's take this `input` and `generated_output` as an example:
...
# This is the summary, replace this with the generated output from your LLM application
generated_output="""
...
test_case = LLMTestCase(input=input, generated_output=generated_output)
