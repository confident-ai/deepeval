...
These environment variables configure Anthropic usage in DeepEval.

Precedence: explicit constructor args (e.g. `AnthropicModel(model=..., api_key=...)`) override DeepEval settings/env vars. If neither explicit args nor env vars are set for the API key, DeepEval will raise an error at runtime.

| Variable              | Values         | Effect |
| --------------------- | -------------- | ------ |
| `ANTHROPIC_API_KEY`   | string / unset | Required at runtime unless provided as `AnthropicModel(api_key=...)`. Anthropic API key. |
| `ANTHROPIC_MODEL_NAME` | string / unset | Optional. Default model used when `model` is not passed to `AnthropicModel(...)` (falls back to `claude-3-7-sonnet-latest`). |

`temperature` resolution for `AnthropicModel(...)`: explicit `temperature` arg -> global `TEMPERATURE` setting -> `0.0` default.

### Azure OpenAI {#model-settings-azure-openai}

These environment variables configure Azure OpenAI usage in DeepEval.

Precedence: explicit constructor args (e.g. `AzureOpenAIModel(model=..., api_key=...)`) override DeepEval settings/env vars. If neither explicit args nor env vars are set for the API key, DeepEval will raise an error at runtime.

| Variable                | Values          | Effect |
| ----------------------- | --------------- | ------ |
| `AZURE_OPENAI_API_KEY`   | string / unset  | Required at runtime unless provided as `AzureOpenAIModel(api_key=...)`. Azure OpenAI API key. |
| `AZURE_MODEL_NAME`       | string / unset  | Optional. Default model used when `model` is not passed to `AzureOpenAIModel(...)` (falls back to `gpt-4.1` if unset). |
| `AZURE_OPENAI_ENDPOINT`  | string / unset  | Required at runtime unless provided as `AzureOpenAIModel(base_url=...)`. Azure OpenAI endpoint URL. |
| `AZURE_DEPLOYMENT_NAME`  | string / unset  | Required at runtime unless provided as `AzureOpenAIModel(deployment_name=...)`. Azure deployment name. |
| `OPENAI_API_VERSION`     | string / unset  | Required at runtime unless provided as `AzureOpenAIModel(openai_api_version=...)`. OpenAI API version used in your deployment. |

`temperature` resolution for `AzureOpenAIModel(...)`: explicit `temperature` arg -> global `TEMPERATURE` setting -> `0.0` default.

### DeepSeek Model Settings {#model-settings-deep-seek}

These environment variables configure DeepSeek model usage in DeepEval.

Precedence: explicit constructor arguments (e.g., `DeepSeekModel(model=..., api_key=...)`) override DeepEval settings or environment variables. If neither explicit arguments nor environment variables are set for the API key, DeepEval will raise an error at runtime.

| Variable              | Values         | Effect                                                                                                                   |
| --------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `DEEPSEEK_API_KEY`    | string / unset | Required at runtime unless provided as `DeepSeekModel(api_key=...)`. Your DeepSeek API key.                              |
| `DEEPSEEK_MODEL_NAME` | string / unset | Optional. Default model used when `model` is not passed to `DeepSeekModel(...)`. Defaults to `deepseek-chat` if unset.   |
| `TEMPERATURE`         | float / unset  | Optional. Default temperature used when `temperature` is not passed to `DeepSeekModel(...)`. Defaults to `0.0` if unset. |
| `DEEPSEEK_ENDPOINT`   | string / unset | Optional. Custom endpoint URL for the DeepSeek API.                                                                      |

`temperature` resolution for `DeepSeekModel(...)`: explicit `temperature` argument -> global `TEMPERATURE` setting -> `0.0` default.

Here is the `Model Settings` section for the Gemini model:

### Gemini Model Settings {#model-settings-gemini}

These environment variables configure Gemini model usage in DeepEval.

Precedence: explicit constructor arguments (e.g., `GeminiModel(model=..., api_key=...)`) override DeepEval settings or environment variables. If neither explicit arguments nor environment variables are set for the API key, DeepEval will raise an error at runtime.

| Variable                     | Values               | Effect                                                                                                                |
| ---------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `GOOGLE_API_KEY`             | string / unset       | Required at runtime unless provided as `GeminiModel(api_key=...)`. Google API key for authentication.                 |
| `GEMINI_MODEL_NAME`          | string / unset       | Optional. Default model used when `model` is not passed to `GeminiModel(...)`. Defaults to `gemini-1.5-pro` if unset. |
| `GOOGLE_CLOUD_PROJECT`       | string / unset       | Optional. Google Cloud project ID. Required if using Vertex AI (unless passed as `GeminiModel(project=...)`).         |
| `GOOGLE_CLOUD_LOCATION`      | string / unset       | Optional. Google Cloud location. Required if using Vertex AI (unless passed as `GeminiModel(location=...)`).          |
| `GOOGLE_GENAI_USE_VERTEXAI`  | `yes` / `no` / unset | Optional. When set to `yes`, Vertex AI will be used for generation. Defaults to unset (uses default service).         |
| `GOOGLE_SERVICE_ACCOUNT_KEY` | string / unset       | Optional. Service account key in JSON format for authentication when using Vertex AI.                                 |

`temperature` resolution for `GeminiModel(...)`: explicit `temperature` argument -> global `TEMPERATURE` setting -> `0.0` default.

### Grok Model Settings {#model-settings-grok}

These environment variables configure Grok (xAI) usage in DeepEval.

Precedence: explicit constructor arguments (e.g., `GrokModel(model=..., api_key=...)`) override DeepEval settings / environment variables.

| Variable           | Values        | Effect |
| ------------------ | ------------- | ------ |
| `USE_GROK_MODEL`   | `1` / unset   | Optional. When set, selects Grok as the default LLM provider (where applicable). |
| `GROK_API_KEY`     | string / unset| Required at runtime unless provided as `GrokModel(api_key=...)`. Grok API key for authentication. |
| `GROK_MODEL_NAME`  | string / unset| Required at runtime unless provided as `GrokModel(model=...)`. Must be one of DeepEval’s supported Grok model names (validated at runtime). |

`temperature` resolution for `GrokModel(...)`: explicit `temperature` argument -> global `TEMPERATURE` setting -> `0.0` default.

### LiteLLM Model Settings {#model-settings-litellm}

These environment variables configure LiteLLM usage in DeepEval.

Precedence: explicit constructor arguments (e.g., `LiteLLMModel(model=..., api_key=..., base_url=...)`) override DeepEval settings / environment variables.

| Variable                 | Values             | Effect |
| ------------------------ | ------------------ | ------ |
| `USE_LITELLM`            | `1` / unset        | Optional. When set, selects LiteLLM as the default LLM provider (where applicable). |
| `LITELLM_MODEL_NAME`     | string / unset     | Required at runtime unless provided as `LiteLLMModel(model=...)`. Must include a provider prefix (e.g., `openai/gpt-3.5-turbo`). |
| `LITELLM_API_KEY`        | string / unset     | Optional. Primary API key used by `LiteLLMModel` when `api_key` is not passed. |
| `LITELLM_PROXY_API_KEY`  | string / unset     | Optional. Fallback API key used if `LITELLM_API_KEY` is unset. |
| `LITELLM_API_BASE`       | string / unset     | Optional. Default base URL used when `base_url` is not passed (useful for custom endpoints). |
| `LITELLM_PROXY_API_BASE` | string / unset     | Optional. Fallback base URL used if `LITELLM_API_BASE` is unset. |

API key fallback behavior: if `api_key` is not passed to `LiteLLMModel(...)`, DeepEval attempts (in order) `LITELLM_API_KEY`, `LITELLM_PROXY_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, then `GOOGLE_API_KEY`. If none are set, the key is left unset and the underlying LiteLLM/provider behavior applies.

`temperature` resolution for `LiteLLMModel(...)`: explicit `temperature` argument -> global `TEMPERATURE` setting -> `0.0` default.

### Ollama Model Settings {#model-settings-ollama}

These environment variables configure Ollama (local) model usage in DeepEval.

Precedence: explicit constructor arguments (e.g., `OllamaModel(model=..., base_url=...)`) override DeepEval settings / environment variables.

| Variable               | Values         | Effect |
| ---------------------- | -------------- | ------ |
| `OLLAMA_MODEL_NAME`    | string / unset | Required at runtime unless provided as `OllamaModel(model=...)`. Ollama model name (e.g. `deepseek-r1:1.5b`). |
| `LOCAL_MODEL_BASE_URL` | string / unset | Optional. Base URL of the Ollama server. Defaults to `http://localhost:11434` if unset. |

`temperature` resolution for `OllamaModel(...)`: explicit `temperature` argument -> global `TEMPERATURE` setting -> `0.0` default.
