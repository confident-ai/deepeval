...
DeepEval autoloads `.env.local`, then `.env`, from current working directory at import time.
...
Set a project wide default so you don’t have to pass `--save` every time:
