...
Before the end of some `with` blocks for some `trace_type`s, as you'll learn later, **requires** you to call the `set_parameters()` method to set additional attribute associated with your trace. For example, you'll need to set the `model` name of the LLM used for an `LLM` `TraceType` via the `set_parameters()` method.
...
with Tracer(trace_type=TraceType.LLM) as llm_trace:
    ...
    llm_trace.set_parameters(
        output=llm_response,
        metadata=LlmMetadata(model='gpt-4o')
    )
```
