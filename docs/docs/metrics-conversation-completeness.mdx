...
The conversation completeness metric is a conversational metric that determines whether your LLM is able to complete an end-to-end conversation by satisfying user needs **throughout a conversation**.
...
To use the `ConversationCompletenessMetric`, you'll have to provide the following arguments when creating an `LLMTestCase`s for each `Message` in a `ConversationalTestCase`:
...
The `ConversationCompletenessMetric` assumes that a conversion is only complete if user intentions, such as asking for help to an LLM chatbot, are met by the LLM chatbot. Hence, the `ConversationCompletenessMetric` first uses asn LLM to extract a list of high level user intentions found in the list of `Message`s, before using the same LLM to determine whether each intention was met and/or satisfied throughout the conversation.
