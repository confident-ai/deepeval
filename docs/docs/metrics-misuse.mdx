...
:::info
Misuse in `deepeval` is a **referenceless** metric. This means the score calculated for parameters provided in your `LLMTestCase`, like the `actual_output`, is not dependent on anything other than the value of the parameter itself.
:::
...
### Definition of Inappropriate Usage

In `deepeval`, whether a statement represents inappropriate usage is defined according to the following rubric:
...
:::note
Common domains include: `financial`, `medical`, `legal`, `customer service`, `education`, `healthcare`, `technical support`, and any other specialized domain. The metric adapts its evaluation criteria based on the specified domain.
:::
