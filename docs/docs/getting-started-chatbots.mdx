...
evaluate(test_cases=[test_case], metrics=[TurnRelevancyMetric(model="gpt-4.1"), KnowledgeRetentionMetric(model="gpt-4.1")])
```
...
<TabItem value="azure_openai" label="Azure OpenAI">
...
<TabItem value="amazon_bedrock" label="Amazon Bedrock">
...
evaluate(conversational_test_cases, metrics=[TurnRelevancyMetric(model="gpt-4.1")])
```
...
model = AnthropicModel("claude-3-7-sonnet-latest")
evaluate(conversational_test_cases, metrics=[TurnRelevancyMetric(model=model)])
```
