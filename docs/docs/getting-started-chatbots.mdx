...
We recommend using Confident AI's dataset editor to curate goldens in bulk.

<VideoDisplay src="https://deepeval-docs.s3.us-east-1.amazonaws.com/getting-started%3Achatbot-evals%multiturn-dataset.mp4" />
...
:::info
Your model callback should accept an `input`, and optionally `turns` and `thread_id`, and return a `Turn` object.
:::
...
The test cases should now be populated with simulated conversation `Turn`s.
...
You'll need a [Confident AI](https://app.confident-ai.com) account to trace conversations in production and run offline evaluations.
...
Navigate to [Confident AI](https://app.confident-ai.com) and create a [metric collection](https://documentation.confident-ai.com/docs/llm-evaluation/metrics/create-on-the-cloud#create-a-metric-collection) with the metrics you wish to use.
