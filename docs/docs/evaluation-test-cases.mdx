...
test_case = LLMTestCase(
    ...
    tools_called=[ToolCall(name="Calculator Tool"), ToolCall(name="WebSearch Tool")]
)
```
...
As you've seen, the `tools_called` parameter accepts a list of `ToolCall` objects:

```python
calculator_tool = ToolCall(
    name="calculator tool",
    description="A tool that calculates mathematical equations or expressions.",
    reasoning="User asked for a solution to a math equation; invoking the calculator tool for computation.",
    output=5,
    input_parameters={
...
test_case = LLMTestCase(
    ...
    # Replace this with the tools that were actually used
    tools_called=[ToolCall(name="Calculator Tool"), ToolCall(name="WebSearch Tool")],
    expected_tools=[ToolCall(name="Calculator Tool")]
)
```
