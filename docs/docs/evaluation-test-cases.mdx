...
A test case is a blueprint provided by `deepeval` to unit test LLM outputs based on seven parameters:
...
:::note
`tools_used` and `expected_tools` are LLM test case parameters that are utilized only in **agent evaluation metrics**. These parameters allow you to assess the effectiveness of tool usage in your LLM application and ensure that it meets the expected tool usage standards.
:::
