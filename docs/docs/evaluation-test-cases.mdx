...
Hyperparameters such as **prompt templates are independent variables that you try to optimize for** based on the metric scores you get from evaluation, which is why you should never include it as part of a test case. Another way to think about it is for a given `input`, which prompt template would give you the best `actual_output`.
