...
:::tip
If you're looking for a quick way to run arena comparisons, [Confident AI](https://app.confident-ai.com) (DeepEval Cloud) allows you to easily test different prompts, models, tools, and output configurations **side by side** and evaluate these comparisons using any `deepeval` metric beyond `ArenaGEval` directly on the platfrom.
...
An experiment is a collection of arena comparisons that you can run on a dataset and a set of metrics.
...
Further more, you can also run arena evals using the `compare()` function.
