...
:::info
Although `deepeval` is integrated with Pytest, to avoid any unexpected errors we highly recommend you to **AVOID** executing `deepeval`'s `LLMTestCase`s directly via the `pytest`.
:::
