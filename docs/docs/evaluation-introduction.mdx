...
:::info
`deepeval`'s `Synthesizer` is highly customizable, and you can learn more about it [here.](synthesizer-introduction)
:::
...
There are two mandatory and eleven optional arguments when calling the `evaluate()` function:
