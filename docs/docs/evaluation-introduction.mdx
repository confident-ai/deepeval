...
The `-v` flag (with no arguments) allows you to turn on `verbose_mode` for all metrics ran using `deepeval test run`. Not supplying the `-v` flag will default each metric's `verbose_mode` to its instantiated value.
...
:::note
When a metric's `verbose_mode` is `True`, it prints the [intermediate steps used to calculate said metric](metrics-introduction#debugging-a-metric) to the console during evaluation.
:::
