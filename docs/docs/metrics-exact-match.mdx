...
The Exact Match metric measures whether your LLM application's `actual_output` exactly matches the `expected_output`. The outputs must be **identical** for the metric to pass.
...
The `ExactMatchMetric` performs a strict equality check and does not compute token-level similarity. For token-level evaluation, see [`PrecisionMetric`](/docs/metrics-precision), [`RecallMetric`](/docs/metrics-recall), or [`F1Metric`](/docs/metrics-f1).
