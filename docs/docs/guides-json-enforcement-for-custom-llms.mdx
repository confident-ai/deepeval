---
# id: guides-json-enforcement-for-custom-llms
title: JSON Enforcement for Custom LLMs
sidebar_label: JSON Enforcement for Custom LLMs
---

If you've ever used custom LLMs for evaluation in `deepeval`, you may have encountered the following error:

```bash
ValueError: Evaluation LLM outputted an invalid JSON. Please use a better evaluation model.
```

This error arises when your custom LLM used for evaluation is unable to generate valid JSONs during metric calculation. Although `deepeval` allows you to use literally any LLM for evaluation, the inability to output valid JSONs stops the evaluation process altogether. This happens because for smaller and less powerful LLMs, prompt engineering alone is not sufficient to enforce JSON outputs, which is the method used in `deepeval`'s metrics to output JSONs. As a result, it's vital to find a workaround for users not using OpenAI's GPT models for evaluation.

This guide will demonstrate **various methods to enforce JSON LLM outputs** by leveraging `deepeval`'s built-in Pydantic schemas for validation. This method ensures that the generated JSONs adhere to the given Pydantic schemas, which helps prevent issues like missing brackets or incomplete strings. We'll be showing examples from a wide range of LLM implementations, including:
...
Here's an example of an invalid JSON an open-source model like `mistralai/Mistral-7B-Instruct-v0.3` might output:
