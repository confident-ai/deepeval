...
You can also [click here to find an example of offloading LLM inference to a separate thread](metrics-introduction#mistral-7b-example) as a workaround, although it might not work for all use cases.
:::
...
:::note
Although you're free to implement your own rouge scorer, you'll notice that although not documented, `deepeval` additionally offers a `scorer` module for more traditional NLP scoring method and can be found [here.](https://github.com/confident-ai/deepeval/blob/main/deepeval/scorer/scorer.py)
:::
