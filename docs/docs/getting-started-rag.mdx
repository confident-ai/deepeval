...
- Install `deepeval`
- A Confident AI API key (recommended). Sign up for one [here.](https://www.confident-ai.com)
...
def retriever(query):
    # Your retriever implemetation goes here
    update_current_span(
        test_case=LLMTestCase(input=query, actual_output="...", retrieval_context=["..."])
    )
    pass
```
...
def generator(query, text_chunks):
    # Your retriever implemetation goes here
    update_current_span(
        test_case=LLMTestCase(input=query, actual_output="...", retrieval_context=text_chunks)
    )
    pass
```
...
```python showLineNumbers {6,10,21}
from deepeval.dataset import EvaluationDataset, Golden
...
