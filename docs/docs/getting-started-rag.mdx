...
Select [RAG metrics](/docs/metrics-answer-relevancy) to evaluate your RAG components, or define your own using [G-Eval](/docs/metrics-llm-evals).
...
Supply the metrics to the `@observe` decorator of each function, then define a test case in `update_span` with the input, output, and retrieval context.

```python title=main.py showLineNumbers={true} wrapLines={true} metastring="{1,8,12,13,14,19,23,24,25}"
from deepeval.tracing import observe, update_current_span
from deepeval.test_case import LLMTestCase
...
...
<VideoDisplay src="https://deepeval-docs.s3.us-east-1.amazonaws.com/getting-started%3Arag-evals%3Acomponent.mp4" />
...
DeepEval also lets you evaluate RAG in chatbot systems at a conversation level. This is especially useful for chatbot systems that rely on RAG to generate responses, such as customer support chatbots and QA agents.
...
```python title=main.py showLineNumbers={true} wrapLines={true} metastring="{1,9,15}"
...
conversational_faithfulness = ConversationalGEval(
    name="Faithfulness",
...
DeepEval offers the following multi-turn metrics out-of-the-box, but they do not evaluate RAG-specific parameters such as retrieval context:
...
If you're logged in to Confident AI, you'll be automatically directed to view your RAG chatbot evaluation report.

:::tip
We recommend logging in to Confident AI to analyze your RAG performance in detail.
...
<VideoDisplay src="https://deepeval-docs.s3.us-east-1.amazonaws.com/getting-started%3Arag-evals%3Aconversation.mp4" />
