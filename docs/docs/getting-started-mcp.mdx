...
```python title="main.py" {2,8-10,16,20,24}
...
def interact_with_mcp_app(input):
    ...
    resources_called.append(MCPResourceCall( uri=resource_uri, result=resource_result))
    ...
    # For all prompt calls, create a MCPPromptCall object and add it to list
    prompt_result = await session.get_prompt(prompt_name)
    prompts_called.append(MCPPromptCall( name=prompt_name, result=prompt_result))
    ...
```
...
🎉🥳 **Congratulations!** You just ran your first MCP evaluation. Here's what happened:
