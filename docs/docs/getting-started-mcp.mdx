...
mcp_use_metric = MCPUseMetric(model="gpt-4.1")
```
...
mcp_use_metric = MCPUseMetric(model=model)
```
...
mcp_use_metric = MultiTurnMCPUseMetric(model="gpt-4.1")
mcp_task_completion = MCPTaskCompletionMetric(model="gpt-4.1")
```
...
model = AnthropicModel("claude-3-7-sonnet-latest")
mcp_use_metric = MultiTurnMCPUseMetric(model=model)
mcp_task_completion = MCPTaskCompletionMetric(model=model)
```
