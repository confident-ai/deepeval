...
Supply the metrics to the `@observe` decorator of each function, then define a test case in `update_span` if needed. The test case should include every [parameter](/docs/metrics-introduction#test-case-parameters) required by the metrics you select.

<details>
<summary>Click to see a detailed explanation of the code example below</summary>
...
```python title=main.py showLineNumbers={true} wrapLines={true} metastring="{1,9,13,16,25}"
...
def llm(query):
    ...
    update_span(
        test_case=LLMTestCase(
            input=query,
            actual_output=str(response.output),
