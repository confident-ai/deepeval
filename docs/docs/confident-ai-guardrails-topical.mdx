...
`guard_result.breached` is a boolean that is `True` if the guard has been breached. The `score_breakdown` for `TopicalGuard` a list of dictionaries, each containing:
