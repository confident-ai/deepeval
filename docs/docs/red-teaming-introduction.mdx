...
`deepeval` offers a powerful `RedTeamer` that can **scan LLM applications for safety risks and vulnerabilities** in just a few lines of code.
...
It works by first generating adversarial attacks aimed at provoking harmful responses from your LLM and evaluates how effectively your application handles these attacks, and is a fast and scalable way to test for LLM safety.
