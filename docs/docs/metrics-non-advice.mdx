...
:::tip
This metric is particularly important for customer service bots, general assistants, and any LLM application where providing unlicensed professional advice could create liability issues.
:::
...
There are **ONE** required and **SIX** optional parameters when creating a `NonAdviceMetric`:
...
:::note
Supported advice types include: 'financial', 'medical', 'legal', 'tax', 'insurance', and other professional domains. You can specify multiple types like `['financial', 'medical']` to detect various forms of inappropriate advice in a single evaluation.
::: 