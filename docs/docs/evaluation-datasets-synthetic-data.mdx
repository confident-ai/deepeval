...
- [Optional] `include_expected_output`: a boolean which when set to `True`, will additionally generate an `expected_output` for each synthetic `Golden`. Defaulted to `False`.
- [Optional] `max_goldens`: the maximum number of golden to be generated if `contexts` isn't supplied, or the max number of golden per context if `contexts` is supplied. Defaulted to 2.
- [Optional] `num_evolutions`: the number of evolution steps to apply to each generated input. This parameter controls the **complexity and diversity** of the generated dataset by iteratively refining and evolving the initial inputs. Defaulted to 1.
- [Optional] `attacks`: a list of `RTAdversarialAttack`s, specifying methods used during data evolution. Defaulted to all `RTAdversarialAttack`s.
- [Optional] `vulnerabilities`: a list of `RTVulnerability`s, representing the red teaming behaviour you are trying to elicit. Defaulted to all `RTVulnerability`s.
