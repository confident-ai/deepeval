...
- [Optional] `context_quality_threshold`: a float representing the minimum quality for context selection. If the context quality is lower than this threshold, the context will be rejected. Defaulted to `0.5`.
- [Optional] `context_similarity_threshold`: a float representing the minimum similarity score required for context matching. Contexts with similarity scores lower than this threshold will not be used. Defaulted to `0.5`.
...
- [Optional] `scenario`: a string, specifying the context or setting in which the inputs are fed into the target LLM to be evaluated.
- [Optional] `task`: a string, representing the purpose of evaluating the target LLM, outlining the specific goal or focus of the evaluation.
- [Optional] `input_format`: a string, representing the structure and format of the synthetic input, allowing for precise control over how the inputs are organized.
- [Optional] `expected_output_format`: a string, representing the structure and format of the synthetic expected output.
...
synthesizer.generate_goldens_from_scratch(
    scenario="Harmful and toxic prompts, with emphasis on dark humor",
    task="Red-team LLMs",
    output_format="2-sentence string",
...
- `scenario`: a string, specifying the context or setting in which the inputs are fed into the target LLM to be evaluated.
- `task`: a string, representing the purpose of evaluating the target LLM, outlining the specific goal or focus of the evaluation.
- `input_format`: a string, representing the structure and format of the synthetic input, allowing for precise control over how the inputs are organized.
...
- [Optional] `evolutions`: a dict with `Evolution` keys and sampling probability values, specifying the distribution of data evolutions to be used. Defaulted to all `Evolution`s (except `MULTICONTEXT`) with equal percentage.
