...
- [Optional] `attacks`: a dict of `RTAdversarialAttack`s keys and sampling probability values, specifying the distribution of adversarial attacks you want generated for your synthetic `Golden`s. Defaulted to all `RTAdversarialAttack`s.
- [Optional] `vulnerabilities`: a dict of `RTVulnerability`s keys and sampling probability values, representing the distribution of undesirable vulnerability you are trying to elicit. Defaulted to all `RTVulnerability`s.
