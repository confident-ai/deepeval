...
3. Generating synthetic red-teaming `Golden`s from a **list of provided context.**
4. Generating synthetic `Golden`s from a **list of provided prompts.**
5. Generating synthetic `Golden`s from **scratch**
...
### 3. Generating Red-Teaming `Goldens` From Provided Contexts

`deepeval` also allows you to generate synthetic red-teaming `Goldens` from a manually provided a list of contexts.
...
:::info
`RedTeamEvolution` is an `ENUM` that specifies the different **types of red-teaming** prompts you wish to include in your final dataset, while the `Response` `ENUM` is the **type of response you are trying to get your LLM to produce**.
:::

### 4. Generating From Provided Prompts
