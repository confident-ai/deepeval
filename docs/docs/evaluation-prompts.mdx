...
You can also evaluate prompts side-by-side using `ArenaGEval` to pick the best-performing prompt for your given criteria. Instead of supplying your `hyperparameters` dictionary directly to `compare`, you can pass in a dictionary mapping the names of your contestants to their corresponding hyperparameter dictionaries.
...
test_case = ArenaTestCase(
    contestants={
        "Version 1": LLMTestCase(input='Who wrote the novel "1984"?', actual_output="George Orwell"),
        "Version 2": LLMTestCase(input='Who wrote the novel "1984"?', actual_output='"1984" was written by George Orwell.'),
    },
)
...
compare(test_cases=[test_case], metric=arena_geval, hyperparameters={"Version 1": {"prompt": prompt_1}, "Version 2": {"prompt": prompt_2}})
```
