...
actual_output = your_llm_app(input, prompt.messages)
...
```python title="main.py" showLineNumbers={true} {11,17}
from openai import OpenAI
from deepeval.tracing import observe, update_llm_span
from deepeval.prompt import Prompt

prompt_1 = Prompt(alias="First",  messages=[{"role":"system","content":"You are helpful."}])
prompt_2 = Prompt(alias="Second", messages=[{"role":"system","content":"You are helpful."}])

@observe(type="llm", metrics=[AnswerRelevancyMetric()])
def gen1(input: str):
    res = OpenAI().chat.completions.create(model="gpt-4o", messages=prompt_1.messages+[{"role":"user","content":input}])
    update_llm_span(prompt=prompt_1)
    return res.choices[0].message.content

@observe(type="llm", metrics=[AnswerRelevancyMetric()])
def gen2(input: str):
    res = OpenAI().chat.completions.create(model="gpt-4o", messages=prompt_2.messages+[{"role":"user","content":input}])
...
:::caution
When evaluating prompts, you must call `load` or `pull` before passing the prompt to the `hyperparameters` dictionary when oing end-to-end vals, and beore you call `update_llm_span` when running component-level evals.
:::
