...
<head>
  <link rel="canonical" href="https://deepeval.com/docs/metrics-plan-quality" />
</head>
...
# Initialize metric
metric = PlanQualityMetric(threshold=0.7, model="claude-3-7-sonnet-latest")
...
- [Optional] `model`: a string specifying which of Anthropic's Claude models to use, **OR** [any custom LLM model](/docs/metrics-introduction#using-a-custom-llm) of type `DeepEvalBaseLLM`. Defaulted to 'claude-3-7-sonnet-latest'
