...
<Equation formula="\text{Plan Adherence Score} = \text{PlanQualityScore}(\text{Task}, \text{Plan})" />

- **Task** and **Plan** are extracted from the trace using an LLM.
- The **PlanQualityScore** measures the quality of the agent's plan for the given task.