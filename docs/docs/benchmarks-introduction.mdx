...
Start by creating a custom model by inheriting the `DeepEvalBaseLLM` class (visit the [custom models section](metrics-introduction#using-a-custom-llm) for a full guide on how to create a custom model):
