...
The argument correctness metric can also be used to evaluate your LLM's ability to generate arguments for MCP primitives. Learn more about [MCP evaluations here](https://deepeval.com/docs/evaluation-mcp).
...
<Equation formula="\text{Argument Correctness} = \text{AlignmentScore(PrimitiveCall, Args)}" />

The **PrimitiveCall** here refers to a tool or resource or prompt called by the LLM app. The **AlignmentScore** is determined by the evaluation model. 
