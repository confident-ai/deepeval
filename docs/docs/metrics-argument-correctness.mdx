...
:::info
The `ArgumentCorrectnessMetric` uses an LLM to determine argument correctness, and is also referenceless. If you're looking to determistically evaluate argument correctness, please refer to the [tool correctness metric](/docs/metrics-tool-correctness) instead.
:::
...
The `ArgumentCorrectnessMetric` assesses the correctness of the arguments (input parameters) for each tool call, based on the task outline in the input.
