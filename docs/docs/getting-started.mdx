...
# 1. Decorate components with metrics you wish to evaluate or debug
@observe(metrics=[AnswerRelevancyMetric()])
...
for golden in dataset.evals_iterator():
    ...
    graph.invoke(
        ...
        # 5. Configure deepeval's callback handler (and optionally metrics for trace-level evals)
...
# 1. Decorate components with metrics you wish to evaluate or debug
@tool(metrics=[AnswerRelevancyMetric()])
...
# 2. Create dataset
dataset = EvaluationDataset(goldens=[Golden(input="What's the weather in SF?")])
...
for golden in dataset.evals_iterator():
    ...
    agent.invoke(
        {"messages": [{"role": "user", "content": golden.input}]},
        # 5. Configure deepeval's callback handler (and optionally metrics for trace-level evals),
        config={"callbacks": [CallbackHandler(metrics=[AnswerRelevancyMetric()])]}
    )
```

</TabItem>

<TabItem value="pydantic" label="Pydantic"></TabItem>

<TabItem value="openai-agents" label="OpenAI Agents"></TabItem>

<TabItem value="llamaindex" label="LlamaIndex"></TabItem>

<TabItem value="openai" label="OpenAI"></TabItem>

<TabItem value="anthropic" label="Anthropic"></TabItem>
