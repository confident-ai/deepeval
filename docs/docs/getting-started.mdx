...
While end-to-end evals treat your LLM app as a black-box, you also evaluate the individual components that form your LLM app (think retrievers, agents, tool calls, LLMs, etc.) instead through **component-level evaluation**. This is the recommended way to evaluate **AI agents.**
...
The task completion metric is the most powerful metric `deepeval` has to offer for evaluating agents. No need to learn about test cases, and `deepeval` will determine whether the desired outcome was achieved based on the traced agentic flow.

You can also run `deepeval view` (or `deepeval login`) to see the test run with LLM traces on Confident AI:
...
You'll need to also use [`ConversationalTestCase`s](/docs/evaluation-multiturn-test-cases#conversational-test-case) instead of regular `LLMTestCase` for conversational metrics:
