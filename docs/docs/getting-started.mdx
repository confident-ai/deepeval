...
<details>
  ...
  <ul>
    ...
    <li>
      <strong>Arena:</strong> evaluates multiple single-turn interactions with the same user input to find the best interaction. Ideal
      for comparing different iterations of your LLM app, such as when picking the best models and prompts for your application.
    </li>
  </ul>
 <p>
  Single‑turn evaluations require an <a href="/docs/evaluation-test-cases#llm-test-case">LLM test case</a>, which represents a single interaction in <code>deepeval</code>. Multi-turn evaluations require <a href="/docs/evaluation-multiturn-test-cases">multi-turn test cases</a>, which represent a sequence of interactions. Arena evaluations require <a href="/docs/evaluation-arena-test-cases">arena test cases</a>, which represents a collection of single-turn LLM Test Cases with the same input. Finally, agentic evaluations require you to set up tracing.
</p>
...
You'll need to configure your <code>OPENAI_API_KEY</code> in your environment
variable to run <code>GEval</code>.

Most deepeval metrics including <code>GEval</code> are LLM-judge metrics, and default to using OpenAI's <code>gpt-4.1</code>, but you can specify any OpenAI model you wish to use:

<pre>correctness_metric = GEval(..., model="o1")</pre>
...
You'll need to configure your <code>OPENAI_API_KEY</code> in your environment
variable to run <code>ConversationalGEval</code>.

Most deepeval metrics including <code>ConversationalGEval</code> are LLM-judge metrics, and default to using OpenAI's <code>gpt-4.1</code>, but you can specify any OpenAI model you wish to use:

<pre>professionalism_metric = ConversationalGEval(..., model="o1")</pre>
...
<TabItem value="agentic" label="Agentic" default>
  <Admonition type="info">
    To run Agentic Evaluations, see{" "}
    <a href="/docs/getting-started#evaluate-nested-components">
...
Run `test_example.py` from the root directory of your project to run arena evaluations your LLM app:

```bash
python test_example.py
```

Congratulations! You should've found your winning contestant ✅ Let's breakdown what happened.

- The variable `input` mimics a user input, and `actual_output` is a placeholder for what your application's supposed to output based on this input.
- Each of your contestants' `LLMTestCase` represents the `input` and `actual_output` of a different LLM model.
- In this example, the arena metric `criteria` chooses the winner based on your application's friendliness.

<details> <summary><strong>Set up your LLM Provider</strong> </summary>

You'll need to configure your <code>OPENAI_API_KEY</code> in your environment
variable to run <code>ArenaGEval</code>.

Most deepeval metrics including <code>ArenaGEval</code> are LLM-judge metrics, and default to using OpenAI's <code>gpt-4.1</code>, but you can specify any OpenAI model you wish to use:

<pre>arena_geval = ArenaGEval(..., model="o1")</pre>
...
You'll need to configure your <code>OPENAI_API_KEY</code> in your environment
variable to run <code>MultimodalGEval</code>.

Most deepeval metrics including <code>MultimodalGEval</code> are LLM-judge metrics, and default to using OpenAI's <code>gpt-4.1</code>, but you can specify any OpenAI model you wish to use:

<pre>text_image_coherence = MultimodalGEval(..., model="o1")</pre>
...
Component‑level evaluations focus on individual components rather than the overall end‑to‑end result, making it easy to identify and fix specific modules in your LLM application. Moreover, agentic metrics—such as <a href="/docs/metrics#task-completion">Task Completion</a>—can only be applied through component-level evals.
