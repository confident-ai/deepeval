...
`deepeval` runs evaluations locally on your enviornment. However, you can also **run evaluations directly on** [Confident AI](confident-ai-introduction), the leading evaluation platform for DeepEval:
...
:::tip Did you know?
You can also create a dataset on Confident AI from a test run, which allows you to reuse the set of `input`s and any `expected_output`, `context`, etc. for subsequent evaluations.
...
:::info
You can also run all of `deepeval`'s metrics on Confident AI's infrastructure. Start by following [this](confident-ai-testing-n-evaluation-experiments) guide.
:::
