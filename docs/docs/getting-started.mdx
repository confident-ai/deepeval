...
- The variable `retrieval_context` contains the retrieved context from your knowledge base, and `AnswerRelevancyMetric(threshold=0.7)` is an default metric provided by DeepEval for you to evaluate how factually correct your application's output is based on the provided context.
...
Alternatively, you can evaluate entire datasets without going through the CLI, which is especially useful in a notebook enviornment:
