...
The task completion metric evaluates how effectively an **LLM agent accomplishes a task** as outlined in the `input`, based on `tools_called` and the `actual_output` of the agent. `deepeval`'s task completion metric is a self-explaining LLM-Eval, meaning it outputs a reason for its metric score.
...
>
  <img
    src="https://confident-bucket.s3.amazonaws.com/task-completion.png"
    alt="LangChain"
    style={{
      margin: "40px",
      height: "auto",
