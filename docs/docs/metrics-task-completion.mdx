...
def trip_planner_agent(input):
    ...
    def itinerary_generator(destination, days):
        return [
            f"Day 1: Eiffel Tower, Le Jules Verne.",
            f"Day 2: Louvre Museum, Angelina Paris.",
        ]

    itinerary = itinerary_generator(destination="Paris", days=3)
    restaurants = restaurant_finder(city="Paris")
    output = (
...
evaluate(observed_callback=trip_planner_agent, goldens=[Golden(input="Hi!")])
```
