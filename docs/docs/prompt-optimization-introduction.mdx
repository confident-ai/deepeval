...
- [Optional] `list_input_role`: role used when appending golden input to LIST-style prompts. Default is "user".
...
- [Optional] `prompt_type`: string indicating the prompt type (`"TEXT"` or `"LIST"`).
...
- [Optional] `target_type`: `PromptListMutationTargetType` indicating which message in a LIST-style prompt is eligible for mutation. Options are `"first"`, `"random"`, or `"fixed_index"`. Default is `"first"`.
...
- [Optional] `target_index`: zero-based index used when `target_type` is `"fixed_index"`. Default is `None`.
