...
If you want more context in your callback, you can declare extra **keyword arguments**. When present in the function signature, the optimizer will populate them for you.

There are **FOUR** optional keyword arguments your `model_callback` can accept:
...
If you don’t declare a parameter in your callback’s signature, DeepEval simply does not pass it.
...
- [Required] `model_callback`: a callable that receives `prompt_text` (and any optional callback keyword arguments you declare) and returns your model or app’s response.
