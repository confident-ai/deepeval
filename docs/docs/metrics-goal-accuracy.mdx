...
The `GoalAccuracyMetric` score is calculated according to the following equation:

<Equation formula="\text{Goal Accuracy Score} = \text{GoalAccuracy}(\text{Task}, \text{Plan}, \text{StepsTaken})" />
