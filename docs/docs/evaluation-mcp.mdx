...
- `mcp_data` — a list of `MCPServer` objects giving information on MCP servers that your MCP host can use
...
The `ConversationalTestCase` accepts an optional parameter called `mcp_data` to add your `MCPServer` objects in a list, here's an example implementation on how to add it:
