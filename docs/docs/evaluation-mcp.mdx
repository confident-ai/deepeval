...
- `mcp_data` — a list of `MCPMetaData` objects giving information that your LLM application has access to
...
You can now use the [`ToolCorrectnessMetric`](https://deepeval.com/docs/metrics-tool-correctness) and [`ArgumentCorrectnessMetric`](https://deepeval.com/docs/metrics-argument-correctness) to run MCP evaluations on your single-turn test case.
...
✅ Done. You can now use the [MCP metrics](https://www.deepeval.com/docs/metrics-mcp-args-correctness) to run evaluations on your MCP based application.