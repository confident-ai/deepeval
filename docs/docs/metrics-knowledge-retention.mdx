...
To use the `KnowledgeRetentionMetric`, you'll have to provide the following arguments when creating an `LLMTestCase`s for each turn in a `ConversationalTestCase`:
