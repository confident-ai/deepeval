...
To use the `KnowledgeRetentionMetric`, you'll have to provide the following arguments when creating a [`ConversationalTestCase`](evaluation-test-cases#conversational-test-case):
...
<Equation formula="\text{Knowledge Retention} = \frac{\text{Number of Turns without Knowledge Attritions}}{\text{Total Number of Turns}}" />
