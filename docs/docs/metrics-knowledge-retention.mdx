...
<Equation formula="\text{Knowledge Retention} = \frac{\text{Number of Messages with Knowledge Retentions}}{\text{Total Number of Messages}}" />

The `KnowledgeRetentionMetric` first uses an LLM to extract knowledge gained throughout `messages`, before using the same LLM to determine whether each corresponding LLM responses indicates an ability to recall said knowledge.
