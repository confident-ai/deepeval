...
Here is an example of creating a custom Google VertexAI model through langchain's `ChatVertexAI` module for evaluation:
...
vertexai_gemini = GoogleVertexAIDeepEval(model=custom_model_gemini)
...
#Set the model to vertexai_gemini
