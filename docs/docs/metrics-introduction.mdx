...
:::note
By definition, [online metrics used in production](https://documentation.confident-ai.com/llm-observability/online-metrics) **cannot** be reference-based.
:::
...
Alternatively, if you find yourself running into JSON errors and would like to ignore it, use the [`-c` and `-i` flag during `deepeval test run`](/docs/evaluation-running-llm-evals#flags-for-deepeval-test-run):
