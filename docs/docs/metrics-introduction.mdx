...
For custom metrics (G-Eval and DAG), reference requirements depend on your evaluation criteria. For instance, users define answer correctness in G-Eval typically compares `generated_output` with `expected_output`. Check each metric's "Required Parameters" section in its documentation to see whether it is a referenceless metric or not.
...
- **RAG**: Focus on the `AnswerRelevancyMetric` (evaluates `generated_output` alignment with the `input`) and `FaithfulnessMetric` (checks for hallucinations against `retrieved_context`)
...
1. **Extract all factual claims** made in the `generated_output`
