...
:::warning
We **CANNOT** guarantee that evaluations will work as expected when using a custom model. This is because evaluation requires high levels of reasoning that are generally hard to achieve with custom models, and the ability to follow instructions such as outputing responses in JSON formats, which we find only a small number of LLMs are able to replicate.
:::
...
Using this metric, you can either execute it directly to get its score and reason:
