...
:::warning
Evaluations may not always work as expected when using a custom model. This is because evaluation requires:

- high levels of reasoning that are generally hard to achieve with custom models.
- the ability to follow instructions such as outputing responses in JSON formats.
