...
You can scroll to the bottom to see the [data model of a golden.](/docs/evaluation-datasets#golden-data-structure)
...
A dataset can either be a single-turn one, **or** a multi-turn one (but not both). Initializing supplying your dataset with a list of `Golden`s will make it a single-turn one, whereas supplying it with `ConversationalGolden`s will make it multi-turn:
...
If you haven't already, you can create a [free account on Confident AI here](https://documentation.confident-ai.com/docs/getting-started/setup#create-an-account).
