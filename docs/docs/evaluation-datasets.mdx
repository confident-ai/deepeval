...
- Don't require an `generated_output` when created
...
Think of `Golden`s as "pending test cases" - they contain all the input data and expected results, but are missing the dynamic elements (`generated_output`, `retrieval_context`, `tools_called`) that will be generated when your LLM processes them.
...
first_test_case = LLMTestCase(input="...", generated_output="...")
second_test_case = LLMTestCase(input="...", generated_output="...")
...
:::tip
A `Golden` and `LLMTestCase` contains almost an identical class signature, so technically you can also supply other parameters such as the `generated_output` when creating a `Golden`.
:::
...
:::info
Loading datasets as goldens are especially helpful if you're looking to generate LLM `generated_output`s at evaluation time. You might find yourself in this situation if you are generating data for testing or using historical data from production.
:::
...
Instead of running evaluations locally using your own evaluation LLMs via `deepeval`, you can choose to run evaluations on Confident AI's infrastructure instead. First, [login to Confident AI](https://documentation.confident-ai.com/docs/getting-started/create-account):
