...
The `RoleAdherenceMetric` first loops through each turn individually before using an LLM to determine which one of the `"assistant"` role `"content"` does not adhere to the specified `chatbot_role` using previous turns as context.
