...
The match is evaluated using Python's built-in regular expression engine (`re.fullmatch`), which ensures the entire `actual_output` matches the provided `pattern`.
