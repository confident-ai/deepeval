...
The Pattern Match metric measures whether your LLM application's `actual_output` **matches a given regular expression pattern**. This is useful for ensuring that your model produces outputs in a specific format, structure, or syntax.
...
- `expected_pattern`: a string representing the regular expression pattern that the actual_output must match.
- [Optional] `case_sensitive`: a boolean which when set to True, performs case-sensitive pattern matching. Defaulted to False.
...
<Equation formula="\text{Pattern Match Score} = \begin{cases} 1 & \text{if actual\_output fully matches the expected regex pattern}, \\ 0 & \text{otherwise} \end{cases}" />

The match is evaluated using Python's built-in regular expression engine (`re.fullmatch`), which ensures the entire `actual_output` matches the provided `expected_pattern`.
