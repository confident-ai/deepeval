...
The prompt alignment metric uses LLM-as-a-judge to measure whether your LLM application is able to generate `generated_output`s that aligns with any **instructions** specified in your prompt template. `deepeval`'s prompt alignment metric is a self-explaining LLM-Eval, meaning it outputs a reason for its metric score.
...
The `PromptAlignmentMetric` uses an LLM to classify whether each prompt instruction is followed in the `generated_output` using additional context from the `input`.
