...
title: Attack Enhancement Strategies
sidebar_label: Attack Enhancements
---

## Quick Summary

DeepEval's `RedTeamer` generates attacks by first synthesizing simple baseline prompts, before **progressively enhancing them to create more sophisticated versions** capable of eliciting harmful responses from your LLM application. You can specify these enhancements in `attack_enhancements` within the scan function.
:::info
DeepEval's `RedTeamer` offers **10 types of attack enhancements**, which are randomly sampled from the specified distribution each time an attack is enhanced.
:::
