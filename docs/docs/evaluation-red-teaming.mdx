...
- `target_model`: a [custom LLM model](guides-using-custom-llms) of type `DeepEvalBaseLLM` representing the model you wish to scan for risks and vulnerabilities for using said `RedTeamer`.
