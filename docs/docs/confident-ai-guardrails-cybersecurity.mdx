...
```python
print(input_guard_result.breached)
print(input_guard_result.score_breakdown)
```

`guard_result.breached` is a boolean that is `True` if the guard has been breached. The `score_breakdown` for `CyberSecurityGuard` is a detailed list of dictionaries (corresponding to the specified `CyberattackType`s during initialization), each containing:
