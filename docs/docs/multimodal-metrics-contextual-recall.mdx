...
:::info
The **Multimodal Contextual Recall** is the multimodal adaptation of DeepEval's [contextual recall metric](/docs/metrics-contextual-recall). It accepts images in addition to text for the `input`, `generated_output`, `expected_output`, and `retrieval_context`.
:::
...
:::info
We use the `expected_output` instead of the `generated_output` because we're measuring the quality of the RAG retriever for a given ideal output.
:::
