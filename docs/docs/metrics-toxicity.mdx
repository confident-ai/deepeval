...
test_case = LLMTestCase(
    ...
    generated_output="Sarah always meant well, but you couldn't help but sigh when she volunteered for a project."
)
...
The `ToxicityMetric` first uses an LLM to extract all opinions found in the `generated_output`, before using the same LLM to classify whether each opinion is toxic or not.
