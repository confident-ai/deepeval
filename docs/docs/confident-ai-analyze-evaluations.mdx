...
You **MUST** provide the `model` and `prompt_template` argument to `set_hyperparameters` in order for `deepeval` to know which LLM and prompt template you're evaluating.
