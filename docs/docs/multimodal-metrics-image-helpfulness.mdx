...
:::note
Remember that the `generated_output` of an `MLLMTestCase` is a list of strings and `Image` objects. If multiple images are provided in the generated output, The final score will be the average of each image's helpfulness score.
:::
