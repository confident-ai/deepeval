...
const travelAgent = observe({
  type: "agent",
  availableTools: ["..."],
  agentHandoffs: ["hotel_booking_agent"],
  fn: async (userRequest: string) => {
    // Orchestration logic goes here...
  },
});
```
...
const bookFlight = observe({
  type: "tool",
  name: "book_flight",
  description: "Book a selected flight by its ID",
  fn: async (flightId: string) => {
    return { status: "confirmed", bookingRef: "AB123" };
  },
});
```
...
const searchFlights = observe({
  ...
  fn: async (origin: string, destination: string, date: string) => {
    // Your API call here
    return [{ flightId: "AA123", price: 450 }];
  },
});
...
const travelAgent = observe({
  type: "agent",
  availableTools: ["search_flights", "book_flight"],
  metricCollection: "agent-task-completion-metrics",
  fn: async (userRequest: string) => {
    updateCurrentTrace({
      tags: ["travel-booking"],
      metadata: { agentVersion: "v3.1" },
    });
    const messages: any[] = [{ role: "user", content: userRequest }];
    while (true) {
      const decision = await reasonAndPlan(messages);
      if (decision.includes("search_flights")) {
        const results = await searchFlights("JFK", "LAX", "2025-01-15");
        messages.push({ role: "tool", content: JSON.stringify(results) });
      } else if (decision.includes("book_flight")) {
        const confirmation = await bookFlight("AA123");
        messages.push({ role: "tool", content: JSON.stringify(confirmation) });
      } else {
        return decision;
      }
    }
...
```typescript title="multi_agent.ts"
import { observe } from "deepeval/tracing";

const hotelAgent = observe({
  type: "agent",
  name: "hotel_agent",
  availableTools: ["..."],
  agentHandoffs: [],
...
const travelCoordinator = observe({
  type: "agent",
  availableTools: ["..."],
  agentHandoffs: ["hotel_agent"],
  fn: async (userRequest: string) => {
    // Coordinator logic
    const flightResult = await searchFlights("JFK", "LAX", "2024-12-01");

    // Sub-agent handoff — automatically becomes a child span
    const hotelResult = await hotelAgent("Need a hotel in LAX for Dec 1st");

    return `Flight: ${flightResult}, Hotel: ${hotelResult}`;
  },
});
```
...
const reasonAndPlan = observe({
  type: "llm",
  fn: async (messages: any[], expectedToolCalls?: ToolCall[]) => {
    const response = await client.chat.completions.create({ model: "gpt-4o", messages });

    // Provide ground truth for component-level evaluation
    if (expectedToolCalls) {
      updateCurrentSpan({ expectedTools: expectedToolCalls });
    }
...
const toolCorrectness = new metrics.ToolCorrectnessMetric({ threshold: 0.8 });
const taskCompletion = new metrics.TaskCompletionMetric({ threshold: 0.7 });
...
const reasonAndPlan = observe({
  type: "llm",
  metricCollection: "tool-correctness-metrics",
  fn: async (messages: any[]) => {
    // ...
  },
});
...
const travelCoordinator = observe({
  type: "agent",
  metricCollection: "agent-task-completion",
  fn: async (userRequest: string) => {
    // ...
  },
});
