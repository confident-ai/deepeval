...
:::note
The integrations shown here are minimal tracing examples. For full options — including attaching evaluation metrics to specific spans, running component-level evals, and setting up production `metric_collection`s — see the dedicated integration docs for [LlamaIndex](/integrations/frameworks/llamaindex), [LangGraph](/integrations/frameworks/langgraph), [CrewAI](/integrations/frameworks/crewai), [Pydantic AI](/integrations/frameworks/pydanticai), and [OpenAI Agents](/integrations/frameworks/openai-agents).
:::
