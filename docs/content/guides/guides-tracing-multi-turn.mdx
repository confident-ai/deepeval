...
const generateReply = observe({
  type: "llm",
  fn: async (history: { role: string; content: string }[], userMessage: string) => {
...
const handleTurn = observe({
  fn: async (userMessage: string, threadId: string, userId: string) => {
    updateCurrentTrace({
      threadId,
      userId, // Links the thread to a specific user on Confident AI
    });
    const history = conversations[threadId] ?? [];
    const response = await generateReply(history, userMessage);
...
const retrieveContext = observe({
  ...
  fn: async (userMessage: string) => {
    // Simulated database search
    const docs = ["DeepEval threads group traces by thread_id."];

    // Attach the context to this specific turn's retriever span
    updateCurrentSpan({ retrievalContext: docs });
...
const handleTurn = observe({
  fn: async (userMessage: string, threadId: string, userId: string) => {
    updateCurrentTrace({
      ...
      tags: ["customer-support", "billing"],
      metadata: {
        turnNumber: (conversations[threadId]?.length ?? 0) + 1,
        modelVersion: "v2.1",
        userPlan: "enterprise",
      },
    });
...
<VideoDisplayer
  ...
  description="Group conversational metrics to run on your production threads."
/>
...
```typescript
const handleTurn = observe({
  fn: async (userMessage: string, threadId: string) => {
    updateCurrentTrace({
      threadId,
      metricCollection: "multi-turn-metrics",
    });
    // ... rest of logic
  },
});
...
<VideoDisplayer
  ...
  description="Inspect every turn of a conversation exactly as your users saw it."
/>
...
// Trigger evaluation for a specific thread by its ID
evaluateThread({ threadId: "session-123", metricCollection: "my-thread-metrics" });
```
