...
For a broader overview of AI agent evaluation concepts and strategies, see the [AI Agent Evaluation guide](/guides/guides-ai-agent-evaluation). For the bigger picture of how these metrics fit into the validation layer of an agent, see [what an eval harness is](/blog/what-is-an-eval-harness).
...
const travelAgent = observe({
  ...
  fn: async (userInput: string) => {
    // Agent reasons: "I need to search for flights first, then book the cheapest"
    const flights = await searchFlights("NYC", "Paris", "2025-03-15");
    const cheapest = flights.reduce((a, b) => (a.price < b.price ? a : b));
    return `Found cheapest flight: ${cheapest.id} for $${cheapest.price}`;
  },
});

// Initialize metric
const planQuality = new metrics.PlanQualityMetric({ threshold: 0.7, model: "gpt-4o" });

// Evaluate agent with plan quality metric
const dataset = new EvaluationDataset({ goldens: [new Golden({ input: "Find me the cheapest flight to Paris" })] });
for await (const golden of dataset.evalsIterator({ metrics: [planQuality] })) {
...
const travelAgent = observe({
  ...
  fn: async (userInput: string) => {
    ...
    return `Booked flight ${cheapest.id}. Confirmation: ${booking.confirmation}`;
  },
});

// Initialize metric
const planAdherence = new metrics.PlanAdherenceMetric({ threshold: 0.7, model: "gpt-4o" });

// Evaluate whether agent followed its plan
const dataset = new EvaluationDataset({ goldens: [new Golden({ input: "Book the cheapest flight to Paris" })] });
for await (const golden of dataset.evalsIterator({ metrics: [planAdherence] })) {
  if (!("input" in golden)) continue;
  await travelAgent(golden.input);
}
```
...
// Initialize metric
const toolCorrectness = new metrics.ToolCorrectnessMetric({ threshold: 0.7 });
...
const callLlm = observe({
  ...
  fn: async (messages: { role: string; content: string }[]) => {
    ...
    updateCurrentSpan({
      ...
      output: `The weather is ${result.condition}, ${result.temp}`,
...
const weatherAgent = observe({
  type: "agent",
  fn: async (userInput: string) => {
    return callLlm([{ role: "user", content: userInput }]);
  },
});

// Evaluate
const dataset = new EvaluationDataset({
  goldens: [new Golden({ input: "What's the weather in Paris?", expectedTools: [new ToolCall({ name: "get_weather" })] })],
});
...
// Initialize metric
const argumentCorrectness = new metrics.ArgumentCorrectnessMetric({ threshold: 0.7, model: "gpt-4o" });
...
const callLlm = observe({
  ...
  fn: async (userInput: string) => {
    // LLM generates arguments for tool call
    const [origin, destination, date] = ["NYC", "London", "2025-03-15"];
    const flights = await searchFlights(origin, destination, date);

    // Update span with tool calling details for evaluation
    updateCurrentSpan({
      input: userInput,
      output: `Found ${flights.length} flights`,
    });
    return flights;
  },
});

const flightAgent = observe({
  type: "agent",
  fn: async (userInput: string) => {
...
// Evaluate - metric checks if arguments match what input requested
const dataset = new EvaluationDataset({
  goldens: [new Golden({ input: "Search for flights from NYC to London on March 15th" })],
});
for await (const golden of dataset.evalsIterator()) {
  if (!("input" in golden)) continue;
  await flightAgent(golden.input);
}
```
...
const travelAgent = observe({
  type: "agent",
  fn: async (userInput: string) => {
    const flights = await searchFlights("NYC", "LA", "2025-03-15");
    const cheapest = flights.reduce((a, b) => (a.price < b.price ? a : b));
    const booking = await bookFlight(cheapest.id);
...
// Initialize metric - task can be auto-inferred or explicitly provided
const taskCompletion = new metrics.TaskCompletionMetric({ threshold: 0.7, model: "gpt-4o" });
...
const inefficientAgent = observe({
  type: "agent",
  fn: async (userInput: string) => {
    // Inefficient: searches twice unnecessarily
    const flights1 = await searchFlights("NYC", "LA", "2025-03-15");
    const flights2 = await searchFlights("NYC", "LA", "2025-03-15"); // Redundant!
    const cheapest = flights1.reduce((a, b) => (a.price < b.price ? a : b));
    const booking = await bookFlight(cheapest.id);
    return `Booked: ${booking.confirmation}`;
  },
});

// Initialize metric
const stepEfficiency = new metrics.StepEfficiencyMetric({ threshold: 0.7, model: "gpt-4o" });

// Evaluate - metric will penalize the redundant search_flights call
const dataset = new EvaluationDataset({
  goldens: [new Golden({ input: "Book the cheapest flight from NYC to LA" })],
});
for await (const golden of dataset.evalsIterator({ metrics: [stepEfficiency] })) {
  if (!("input" in golden)) continue;
  await inefficientAgent(golden.input);
}
```
...
const callLlm = observe({
  ...
  fn: async (userInput: string) => {
    // LLM decides to search flights then book
    const [origin, destination, date] = ["NYC", "Paris", "2025-03-18"];
    const flights = await searchFlights(origin, destination, date);
    const cheapest = flights.reduce((a, b) => (a.price < b.price ? a : b));
    const booking = await bookFlight(cheapest.id);
    ...
    updateCurrentSpan({
      input: userInput,
      output: `Booked ${cheapest.id}`,
      expectedTools: [new ToolCall({ name: "search_flights" }), new ToolCall({ name: "book_flight" })],
    });
    return booking;
  },
});
...
const travelAgent = observe({
  type: "agent",
  fn: async (userInput: string) => {
    const booking = await callLlm(userInput);
    return `Flight booked! Confirmation: ${booking.confirmation}`;
  },
});
...
// Run evaluation with end-to-end metrics
for await (const golden of dataset.evalsIterator({
  metrics: [taskCompletion, stepEfficiency, planQuality, planAdherence],
})) {
  if (!("input" in golden)) continue;
  await travelAgent(golden.input);
}
