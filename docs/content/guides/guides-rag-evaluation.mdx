...
const contextualPrecision = new metrics.ContextualPrecisionMetric();
const contextualRecall = new metrics.ContextualRecallMetric();
const contextualRelevancy = new metrics.ContextualRelevancyMetric();
```
...
const testCase = new LLMTestCase({
  input: "I'm on an F-1 visa, how long can I stay in the US after graduation?",
  actualOutput: "You can stay up to 30 days after completing your degree.",
  expectedOutput: "You can stay up to 60 days after completing your degree.",
  retrievalContext: [
    `If you are in the U.S. on an F-1 visa, you are allowed to stay for 60 days after completing
    your degree, unless you have applied for and been approved to participate in OPT.`,
  ],
...
await contextualPrecision.measure(testCase);
console.log("Score: ", contextualPrecision.score);
console.log("Reason: ", contextualPrecision.reason);

await contextualRecall.measure(testCase);
console.log("Score: ", contextualRecall.score);
console.log("Reason: ", contextualRecall.reason);

await contextualRelevancy.measure(testCase);
console.log("Score: ", contextualRelevancy.score);
console.log("Reason: ", contextualRelevancy.reason);
```
...
const answerRelevancy = new metrics.AnswerRelevancyMetric();
const faithfulness = new metrics.FaithfulnessMetric();
```
...
await answerRelevancy.measure(testCase);
console.log("Score: ", answerRelevancy.score);
console.log("Reason: ", answerRelevancy.reason);

await faithfulness.measure(testCase);
console.log("Score: ", faithfulness.score);
console.log("Reason: ", faithfulness.reason);
```
...
await evaluate([testCase], [answerRelevancy, faithfulness]);
```
...
const darkHumor = new metrics.GEval({
  name: "Dark Humor",
  criteria: "Determine how funny the dark humor in the actual output is",
  evaluationParams: [SingleTurnParams.ACTUAL_OUTPUT],
});

await darkHumor.measure(testCase);
console.log("Score: ", darkHumor.score);
console.log("Reason: ", darkHumor.reason);
```
...
```typescript title="test_rag.ts"
import { evaluate, LLMTestCase } from "deepeval";
import { EvaluationDataset, Golden } from "deepeval/dataset";
// ...

const goldens = [new Golden({ input: "..." })];
...
// metrics is the list of RAG metrics as shown in previous sections
await evaluate(dataset.testCases, metrics);
```

</LangSwitch>

Then, simply execute `deepeval test run` in the CLI (or run the script directly for TypeScript):
...
const convoTestCase = new ConversationalTestCase({
  expectedOutcome: "User understands the visa policy and OPT options",
  turns: [
    new Turn({ role: "user", content: "I'm on an F-1 visa, how long can I stay after graduation?" }),
    new Turn({
      role: "assistant",
      content: "You can stay up to 60 days after completing your degree.",
      retrievalContext: [
    ...
    new Turn({ role: "user", content: "What is OPT and how do I apply?" }),
    new Turn({
      role: "assistant",
      content: "OPT is Optional Practical Training. You can apply through your school's international office up to 90 days before graduation.",
      retrievalContext: [
...
await evaluate(
  ...
  [
    ...
    new metrics.TurnContextualPrecisionMetric(),
    new metrics.TurnContextualRecallMetric(),
  ],
);
```
...
```typescript
import { ConversationSimulator, ConversationalGolden } from "deepeval";
import { Turn } from "deepeval/testCase";

const modelCallback = async ({ input, turns, threadId }: { input: string; turns: Turn[]; threadId: string }) => {
  const result = await yourRagApp(input, turns);
  return new Turn({
    role: "assistant",
    content: result.response,
    retrievalContext: result.retrievedChunks,
  });
};

const simulator = new ConversationSimulator({ modelCallback });
const testCases = await simulator.simulate({
  conversationalGoldens: [new ConversationalGolden({ scenario: "..." })],
});
```
