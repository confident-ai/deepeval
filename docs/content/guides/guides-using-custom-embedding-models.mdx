...
To use Google Gemini for embeddings, first configure your Google credentials (Gemini API key or
Vertex AI project/location) via `deepeval set-gemini`, then set the Gemini embedder:

```bash
deepeval set-gemini-embedding --model=gemini-embedding-001
```

To revert back to the default OpenAI embeddings run:

```bash
deepeval unset-gemini-embedding
```

You can also pass the embedder directly:

```python
from deepeval.models import GeminiEmbeddingModel
