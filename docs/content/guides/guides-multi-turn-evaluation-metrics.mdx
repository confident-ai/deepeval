...
```typescript
import { evaluate, metrics } from "deepeval";
import { Turn, ConversationalTestCase } from "deepeval/testCase";

const convoTestCase = new ConversationalTestCase({
  turns: [
    new Turn({ role: "user", content: "I need to cancel my subscription and get a refund." }),
    new Turn({ role: "assistant", content: "I've cancelled your subscription." }),
    new Turn({ role: "user", content: "What about the refund?" }),
    new Turn({ role: "assistant", content: "Your refund of $29.99 has been processed. It will appear in 3-5 business days." }),
  ],
});
const metric = new metrics.ConversationCompletenessMetric({ threshold: 0.7 });
...
const convoTestCase = new ConversationalTestCase({
  turns: [
    new Turn({ role: "user", content: "What's your return policy?" }),
    new Turn({ role: "assistant", content: "We offer a 30-day return policy with full refund." }),
    new Turn({ role: "user", content: "Great, and do you ship internationally?" }),
    new Turn({ role: "assistant", content: "Our return policy covers all items purchased in-store or online." }),
  ],
});
const metric = new metrics.TurnRelevancyMetric({ threshold: 0.7 });
...
const convoTestCase = new ConversationalTestCase({
  turns: [
    new Turn({ role: "user", content: "My name is Sarah and I'm allergic to peanuts." }),
    new Turn({ role: "assistant", content: "Nice to meet you, Sarah! I'll keep your peanut allergy in mind." }),
    new Turn({ role: "user", content: "Can you suggest a dessert for me?" }),
    new Turn({ role: "assistant", content: "How about our peanut butter brownies? They're delicious!" }),
  ],
});
const metric = new metrics.KnowledgeRetentionMetric({ threshold: 0.7 });
...
const convoTestCase = new ConversationalTestCase({
  chatbotRole: "A friendly restaurant booking assistant that only helps with reservations.",
  turns: [
    new Turn({ role: "user", content: "I'd like to book a table for two tonight." }),
    new Turn({ role: "assistant", content: "I'd be happy to help! What time works for you?" }),
    new Turn({ role: "user", content: "8pm. Also, what's the meaning of life?" }),
    new Turn({ role: "assistant", content: "The meaning of life is a deep philosophical question that many have pondered..." }),
  ],
});
const metric = new metrics.RoleAdherenceMetric({ threshold: 0.7 });
...
const convoTestCase = new ConversationalTestCase({
  turns: [
    new Turn({ role: "user", content: "How do I reset my password?" }),
    new Turn({ role: "assistant", content: "Go to Settings > Account > Reset Password and follow the prompts." }),
    new Turn({ role: "user", content: "Can you write me a poem about cats?" }),
    new Turn({ role: "assistant", content: "Sure! Roses are red, cats are great..." }),
  ],
});
const metric = new metrics.TopicAdherenceMetric({
  relevantTopics: ["account management", "technical support", "billing"],
  threshold: 0.7,
});
...
```typescript
import { evaluate, metrics } from "deepeval";
import { Turn, ConversationalTestCase, ToolCall } from "deepeval/testCase";

const convoTestCase = new ConversationalTestCase({
  turns: [
    new Turn({ role: "user", content: "Book me a flight from NYC to London for next Friday." }),
    new Turn({
      role: "assistant",
      content: "I'll search for available flights.",
      toolsCalled: [new ToolCall({ name: "search_flights", description: "Search available flights" })],
    }),
    new Turn({ role: "assistant", content: "I found 3 flights. The cheapest is $450 on British Airways. Shall I book it?" }),
    new Turn({ role: "user", content: "Yes, book it." }),
    new Turn({
      role: "assistant",
      content: "Done! Your flight is confirmed. Confirmation: BA-12345.",
      toolsCalled: [new ToolCall({ name: "book_flight", description: "Book a specific flight" })],
    }),
  ],
});
const metric = new metrics.GoalAccuracyMetric({ threshold: 0.7 });
...
const convoTestCase = new ConversationalTestCase({
  turns: [
    ...
    new Turn({
      ...
      toolsCalled: [new ToolCall({ name: "get_weather", description: "Get current weather", inputParameters: { city: "Paris" } })],
    }),
    new Turn({ role: "assistant", content: "It's 22°C and sunny in Paris right now." }),
  ],
});
const metric = new metrics.ToolUseMetric({
  availableTools: [
    new ToolCall({ name: "get_weather", description: "Get current weather for a city" }),
    new ToolCall({ name: "search_flights", description: "Search for available flights" }),
    new ToolCall({ name: "book_hotel", description: "Book a hotel room" }),
  ],
  threshold: 0.7,
});
...
const convoTestCase = new ConversationalTestCase({
  turns: [
    ...
    new Turn({ role: "user", content: "What about exchanges?" }),
    new Turn({
      role: "assistant",
      content: "Exchanges are available within 60 days of purchase.",
      retrievalContext: ["Exchanges can be made within 60 days. Items must be in original condition."],
    }),
  ],
});
const metric = new metrics.TurnFaithfulnessMetric({ threshold: 0.7 });
...
```typescript
import { evaluate, metrics } from "deepeval";
import { Turn, ConversationalTestCase, MultiTurnParams } from "deepeval/testCase";

const convoTestCase = new ConversationalTestCase({
  turns: [
    new Turn({ role: "user", content: "I'm really frustrated. My order has been delayed three times." }),
    new Turn({ role: "assistant", content: "Let me look into that. Your order was delayed due to weather." }),
    new Turn({ role: "user", content: "This is unacceptable! I want a refund." }),
    new Turn({
      role: "assistant",
      content: "I completely understand your frustration. Let me process that refund immediately and add a 15% discount for your next order as an apology.",
    }),
  ],
});

const empathy = new metrics.ConversationalGEval({
  name: "Empathy",
  criteria: "Evaluate whether the assistant shows genuine empathy when the user expresses frustration or dissatisfaction.",
  evaluationParams: [MultiTurnParams.ROLE, MultiTurnParams.CONTENT],
});

const deEscalation = new metrics.ConversationalGEval({
  name: "De-escalation",
  criteria: "Evaluate whether the assistant effectively de-escalates tense situations by acknowledging concerns and offering concrete solutions.",
  evaluationParams: [MultiTurnParams.ROLE, MultiTurnParams.CONTENT],
});

await evaluate([convoTestCase], [empathy, deEscalation]);
```
