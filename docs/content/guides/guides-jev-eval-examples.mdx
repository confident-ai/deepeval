...
```python
[
  {"question": "The assistant verifies the order before acting on the refund.", "type": "noul", "weight": 1.0, "value": 0.90, "applicable": True, "probabilities": {"true": 0.90, "false": 0.10}, "confidence": 0.80},
...
With `include_reason=True`, a `reason` is built from the answers above, one line per question with its outcome in words and the numbers behind it:

```
Decided by jev-latest, minimum confidence 0.65.
1. The assistant verifies the order before acting on the refund. -> clearly holds (P(yes)=0.90, confidence=0.80)
2. How fully is the user's request settled by the end of turns? -> "Partly settled" (expected level=0.55 of 1.00, confidence=0.70)
3. What did the assistant do about the amount the user asked for? -> "ignored_it" (P=0.65, confidence=0.65)
Score: 0.54 (weighted mean of 3 applicable questions).
```

Nothing is generated, so the same answers always produce the same reason and the whole metric runs with zero LLM tokens whether or not `include_reason` is set.
