...
const firstTestCase = new LLMTestCase({ input: "...", actualOutput: "..." });
const secondTestCase = new LLMTestCase({ input: "...", actualOutput: "..." });
const dataset = new EvaluationDataset();
dataset.testCases = [firstTestCase, secondTestCase];

const metric = new metrics.AnswerRelevancyMetric({ threshold: 0.5 });
await evaluate(dataset.testCases, [metric]);
```
...
To check that your test file is working, run `deepeval test run` (or execute the script directly for TypeScript):
