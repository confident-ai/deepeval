...
// Patches the client in place to auto-capture LLM spans
...
traceManager.configure({
  confidentApiKey: "your-confident-api-key",
});
```
...
traceManager.configure({
  confidentApiKey: "your-confident-api-key",
  environment: "production",
  samplingRate: 0.1, // Only trace 10% of requests
});
```
...
```typescript title="rag_pipeline.ts"
...
const generateResponse = observe({
  type: "llm",
  fn: async (query: string, context: string[]) => {
    // Simulated LLM generation (auto-patched)
    const response = await client.chat.completions.create({
      model: "gpt-4o",
      messages: [{ role: "user", content: `Context: ${context} Query: ${query}` }],
    });
    return response.choices[0].message.content;
  },
});

// Root span (no type required)
const answerUserQuery = observe({
  fn: async (userQuery: string) => {
    const context = await retrieveContext(userQuery);
    return generateResponse(userQuery, context);
  },
});
```
...
const retrieveContext = observe({
  type: "retriever",
  fn: async (query: string) => {
    const results = await vectorStore.search(query, { k: 3 });
    const documents = results.map((res) => res.text);

    // Attach the retrieved documents directly to the current span
    updateCurrentSpan({
      retrievalContext: documents,
      metadata: { chunkSize: 512, embedder: "text-embedding-3-small" },
    });
...
const answerUserQuery = observe({
  fn: async (userQuery: string, userPlan: string) => {
    updateCurrentTrace({
      tags: ["rag-v2"],
      metadata: { userPlan },
    });
...
const relevancyMetric = new metrics.AnswerRelevancyMetric({ threshold: 0.7 });
const faithfulnessMetric = new metrics.FaithfulnessMetric({ threshold: 0.8 });

const generateResponse = observe({
  type: "llm",
  metrics: [relevancyMetric, faithfulnessMetric],
  fn: async (query: string, context: string[]) => {
    const response = await client.chat.completions.create({
      model: "gpt-4o",
      messages: [{ role: "user", content: `Context: ${context} Query: ${query}` }],
...
for await (const golden of dataset.evalsIterator()) {
  ...
  await generateResponse(golden.input);
}
```
...
for await (const golden of dataset.evalsIterator({ metrics: [correctnessMetric] })) {
  if (!("input" in golden)) continue;
  await generateResponse(golden.input);
}
```
...
<VideoDisplayer
  ...
  description="Group RAG metrics to score retrievals and generations automatically."
/>
...
const generateResponse = observe({
  type: "llm",
  metricCollection: "my-production-metrics",
  fn: async (query: string, context: string[]) => {
    // ...
  },
});
...
<VideoDisplayer
  ...
  confidentUrl="/docs/llm-tracing/evaluations"
  label="Track tracing performance over time on Confident AI"
  description="Watch metric scores trend across your RAG pipeline in production."
/>
