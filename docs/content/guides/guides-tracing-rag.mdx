...
:::note
The integrations shown here are minimal tracing examples. For full options — including attaching evaluation metrics to specific spans, running component-level evals, and setting up production `metric_collection`s — see the [LlamaIndex integration docs](/integrations/frameworks/llamaindex) and [LangChain integration docs](/integrations/frameworks/langchain).
:::
...
:::note
Traces are sent asynchronously in a background worker thread. For short-lived scripts that exit before the worker finishes, set the `CONFIDENT_TRACE_FLUSH=1` environment variable to ensure all traces are flushed before the process exits. If you need to control when that happens, or to know whether it succeeded, call `deepeval.flush_traces()` (or `await deepeval.a_flush_traces()`) at your shutdown point instead. For long-running servers (FastAPI, Django), neither is needed.
:::
