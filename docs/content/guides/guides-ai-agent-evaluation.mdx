...
const tools = []; // See tools schema below

const searchFlights = observe({
  type: "tool",
  name: "search_flights",
  fn: async (origin: string, destination: string, date: string) => {
    // Simulated flight search
    return [{ id: "FL123", price: 450 }, { id: "FL456", price: 380 }];
  },
});

const bookFlight = observe({
  type: "tool",
  name: "book_flight",
  fn: async (flightId: string) => {
    // Simulated booking
    return { confirmation: "CONF-789", flightId };
  },
});

const callOpenai = observe({
  type: "llm",
...
const travelAgent = observe({
  ...
  fn: async (userInput: string) => {
    ...
    messages.push({ role: "assistant", content: `Found flights. Booking cheapest: ${cheapest.id}` });

    const booking = await bookFlight(cheapest.id);

    return `Booked flight ${cheapest.id} for $${cheapest.price}. Confirmation: ${booking.confirmation}`;
  },
});
```
...
const planQuality = new metrics.PlanQualityMetric();
const planAdherence = new metrics.PlanAdherenceMetric();
```
...
const dataset = new EvaluationDataset({
  goldens: [new Golden({ input: "Book a flight from NYC to London for next Monday" })],
});

// Loop through dataset with metrics
for await (const golden of dataset.evalsIterator({ metrics: [planQuality, planAdherence] })) {
  await travelAgent(golden.input);
}
```
...
const toolCorrectness = new metrics.ToolCorrectnessMetric();
const argumentCorrectness = new metrics.ArgumentCorrectnessMetric();
```
...
// Create dataset
const dataset = new EvaluationDataset({
  goldens: [new Golden({ input: "What's the weather like in San Francisco and should I bring an umbrella?" })],
});

// Evaluate with action layer metrics
for await (const golden of dataset.evalsIterator()) {
  await weatherAgent(golden.input);
}
```
...
const taskCompletion = new metrics.TaskCompletionMetric();
const stepEfficiency = new metrics.StepEfficiencyMetric();
```
...
// Create dataset
const dataset = new EvaluationDataset({
  goldens: [new Golden({ input: "Book the cheapest flight from NYC to LA for tomorrow" })],
});

// Evaluate with execution metrics
for await (const golden of dataset.evalsIterator({ metrics: [taskCompletion, stepEfficiency] })) {
  await travelAgent(golden.input);
}
```
...
// Reference your Confident AI metric collection by name
const callOpenai = observe({
  metricCollection: "my-agent-metrics",
  fn: async (messages: any[]) => {
    // ...
  },
});
...
<FAQs
  qas={[
    ...
    {
      question:
        "What is the difference between end-to-end and component-level agent evals?",
      answer: (
        <>
          End-to-end evals are passed to{" "}
          <code>evals_iterator(metrics=[...])</code> and score the entire
          trace—best for plan quality and task completion. Component-level evals
          are attached via <code>@observe(metrics=[...])</code> and score a
          specific span like the LLM tool-calling component—best for tool
