...
**AI agent evaluation** is the process of measuring how well an agent reasons, selects and calls tools, and completes tasks—at the scope that matches each failure mode—so you can pinpoint exactly what's broken. But first, what is an AI agent?
...
Since a successful agent outcome depends on both reasoning and action, AI agent evaluation uses three complementary scopes:

- **End-to-end (black-box output)** evaluates only the agent's observable input and final output, without inspecting internal execution.
- **Trajectory-based** evaluates the complete, ordered execution trace—including reasoning, tool calls, observations, and intermediate steps—to judge how the agent reached its outcome.
- **Component-level (step-wise)** evaluates a single span or decision in isolation, such as the LLM span that selected a tool and generated its arguments.

This guide uses trajectory metrics for full-trace planning and execution quality, and component-level metrics for individual tool-calling decisions. See [trajectory-based evaluation](/docs/evaluation-trajectory-based-llm-evals) for the full scope model.
...
_For a comprehensive breakdown of each agentic metric, see the [AI Agent Evaluation Metrics guide](/guides/guides-ai-agent-evaluation-metrics). To understand where this all fits in the agent stack, read [what an eval harness is and why it matters](/blog/what-is-an-eval-harness)._
...
<ImageDisplayer
  src="https://images.ctfassets.net/otwaplf7zuwf/U833Rl3xfX0xq7UCDbpgA/b57e854f9f8444639b12773f9cee77f8/ai-agent.png"
  alt="AI Agent"
/>
...
Evaluating agents in development is all about benchmarking with datasets and metrics. Your metrics can score black-box outputs, complete trajectories, or individual components, while datasets make sure you're comparing different iterations of your agents on the [same set of goldens.](/docs/evaluation-datasets)
...
<VideoDisplayer
  src={ASSETS.gettingStartedAgentEvalsEndToEnd}
  confidentUrl="https://www.confident-ai.com/docs/llm-evaluation/dashboards/testing-reports"
  label="Agent evals test reports on Confident AI"
  description="Run agent evals locally and get a test report on the platform."
/>

### Evaluating the Reasoning Layer

`deepeval` offers two trajectory metrics to evaluate your agent's reasoning and planning capabilities:
...
Finally, loop your traced AI agent over a [dataset](/docs/evaluation-datasets) you've prepared while defining `PlanAdherenceMetric` and `PlanQualityMetric` as trajectory metrics. They require tracing because they inspect planning and execution across the complete ordered trace:
...
`deepeval` offers two trajectory metrics to evaluate your agent's overall execution:

- [`TaskCompletionMetric`](/docs/metrics-task-completion): evaluates whether your agent **successfully accomplishes the intended task** by analyzing the complete ordered execution trace.

- [`StepEfficiencyMetric`](/docs/metrics-step-efficiency): evaluates whether your agent **completes tasks efficiently** by checking the complete ordered trace for unnecessary or redundant steps.
...
These metrics analyze the complete ordered agent trace to assess execution quality:
...
# Evaluate the complete trajectory with execution metrics
...
:::info
Both `TaskCompletionMetric` and `StepEfficiencyMetric` are trajectory metrics. They cannot be used standalone and **MUST** be used with tracing through `evals_iterator` or the `observe` decorator.
:::

## End-to-End vs Trajectory vs Component-Level Evals

AI agent evaluation can operate at three different scopes:

- **End-to-end evals** — Score the final black-box output from the agent's input and output, without requiring or inspecting its internal trace.

- **Trajectory evals** — The reasoning metrics (`PlanQualityMetric`, `PlanAdherenceMetric`) and execution metrics (`TaskCompletionMetric`, `StepEfficiencyMetric`) are passed to `evals_iterator(metrics=[...])`. They require tracing and evaluate the complete ordered sequence of reasoning, tool calls, observations, and other steps.

- **Component-level evals** — The action layer metrics (`ToolCorrectnessMetric`, `ArgumentCorrectnessMetric`) are attached directly to the `@observe` decorator on the LLM component via `@observe(metrics=[...])`. They evaluate the single span where the tool-calling decision is made.

This distinction matters because different metrics need different scopes:

| Metric Type                  | Scope                  | Why                                                                                         |
| ---------------------------- | ---------------------- | ------------------------------------------------------------------------------------------- |
| Final-output quality         | End-to-end (black-box) | Scores the result without depending on internal implementation details                      |
| Reasoning & execution        | Trajectory             | Needs the complete ordered trace to assess planning, adherence, completion, and efficiency |
| Tool selection and arguments | Component-level        | Scores the individual LLM span where the tool-calling decision happens                      |

You can learn more about when to use each approach in the [end-to-end evals](/docs/evaluation-end-to-end-llm-evals), [trajectory-based evals](/docs/evaluation-trajectory-based-llm-evals), and [component-level evals](/docs/evaluation-component-level-llm-evals) documentation.
...
This is where [`GEval`](/docs/metrics-llm-evals) comes in. G-Eval is a framework that uses LLM-as-a-judge to evaluate outputs based on **any custom criteria** you define in plain English. Depending on the test case and evaluation target, it can be applied to black-box outputs or specific components.
...
Drop this command into a `.yml` workflow and failing metrics will fail the build, so agent regressions get caught before they ship. For the complete CI/CD walkthrough—including a ready-to-use GitHub Actions config—see the [AI Agent Evaluation Quickstart](/docs/getting-started-agents).

## Evaluating AI agents with coding assistants

More and more agents are being built _by_ coding assistants like Claude Code. But here's the catch: a coding agent's harness — `CLAUDE.md`, hooks, skills, plugins, MCP servers — is built to get the right context into the model and ship good code. It has **no native concept of measuring an agent's output against ground truth.** A test is green or red, but there's no `assertEqual` for "was this response faithful?"

<ClaudeCodeTerminal />

That gap is exactly what an **eval harness** fills, and DeepEval _is_ the eval harness for AI agents. Because skills load on demand, dropping it into your coding agent is a one-liner:

```bash
npx skills add confident-ai/deepeval --skill "deepeval"
```

From there, Claude Code can drive the whole loop itself — generate datasets, run `deepeval test run`, and inspect locally captured traces — turning "add evals and fix the failures" into something it can actually act on.

_For the full picture of where the eval harness sits in the agent stack and how it plugs into a coding agent's own harness, read [Eval harness: what it is, how to use it, and why you should care](/blog/what-is-an-eval-harness)._
...
<VideoDisplayer
  ...
  description="Create a metric collection to score production traffic automatically."
/>
...
<VideoDisplayer
  ...
  confidentUrl="/docs/llm-tracing/evaluations"
  label="Track agent performance over time on Confident AI"
  description="Monitor traces and metric scores as your agent runs in production."
/>
...
| ---------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------- |
| End-to-end (black-box) | Evaluate the final output only     | Output-focused metrics and custom `GEval` criteria                                        |
| Trajectory             | Evaluate the complete ordered trace | `PlanQualityMetric`, `PlanAdherenceMetric`, `TaskCompletionMetric`, `StepEfficiencyMetric` |
| Component-level        | Evaluate a single span or decision | `ToolCorrectnessMetric`, `ArgumentCorrectnessMetric`                                      |
  ...
  qas={[
    {
      ...
      answer:
        "AI agent evaluation measures how well an autonomous LLM system reasons, plans, selects and calls tools, and completes tasks. It can score the final black-box output end-to-end, the complete ordered execution trace as a trajectory, or a single component span such as an LLM tool-calling decision.",
    },
    {
      question:
        "How is AI agent evaluation different from regular LLM evaluation?",
      answer:
        "Standard LLM evaluation commonly scores one input-output pair. AI agent evaluation can additionally score the complete ordered trajectory of reasoning steps, tool calls, observations, and intermediate decisions, or isolate a single component—so you can pinpoint whether failures came from bad planning, wrong tool selection, incorrect arguments, or incomplete task execution.",
    },
    {
      ...
      answer: (
        <>
          ...
          <code>StepEfficiencyMetric</code> for trajectory-level execution
          quality. The planning and execution metrics require tracing and score
          the complete ordered trace; the tool metrics remain component-level.
        </>
      ),
    },
    {
      question:
        "What is the difference between end-to-end, trajectory, and component-level agent evals?",
      answer: (
        <>
          End-to-end evals score the final black-box output without inspecting
          internal execution. Trajectory evals use tracing and score the
          complete ordered trace—use them for plan quality, plan adherence, task
          completion, and step efficiency. Component-level evals are attached
          via <code>@observe(metrics=[...])</code> and score a specific span
          such as the LLM tool-calling decision—use them for tool selection and
    ...
    },
    {
      ...
      answer: (
        <>
          Trajectory and component-level agent metrics require tracing because
          they read either the complete ordered trace or a specific span. Wrap
          your agent functions with <code>@observe</code> and the trace is built
          automatically. Black-box end-to-end output evaluation does not need
          internal tracing.
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          ...
          <code>DAGMetric</code> for deterministic decision-tree logic. Apply
          custom metrics at the scope supported by their test case and
    ...
    },
    {
      ...
      answer: (
        <>
          Run development evaluations locally with DeepEval, then export traces
          to <a href="https://confident-ai.com">Confident AI</a> for
          asynchronous production evaluation. Attach metric collections to your
          agent and LLM spans so the platform scores live traffic without adding
...
2. **Understand trajectory evaluation** — Learn how to score complete ordered agent traces in the [trajectory-based evaluation guide](/docs/evaluation-trajectory-based-llm-evals)
3. **Explore the metrics** — Learn how each metric works, including calculation formulas and configuration options, in the [AI Agent Evaluation Metrics guide](/guides/guides-ai-agent-evaluation-metrics)
4. **Read the full guide** — For a deeper dive into single-turn vs multi-turn agents, common misconceptions, and best practices, check out [AI Agent Evaluation: The Definitive Guide](https://www.confident-ai.com/blog/definitive-ai-agent-evaluation-guide)
