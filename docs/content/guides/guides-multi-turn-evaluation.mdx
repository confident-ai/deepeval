...
1. A `ConversationalGolden` feeds the scenario and persona into the `ConversationSimulator`.
...
dataset = EvaluationDataset(goldens=[
    ConversationalGolden(
        ...
        persona=Persona(characteristics="Impatient customer who has already contacted support twice"),
    ),
    ConversationalGolden(
        ...
        persona=Persona(characteristics="Non-technical user, first time using the product"),
    ),
    ConversationalGolden(
        ...
        persona=Persona(characteristics="Senior software engineer integrating the product's REST API"),
    ),
])
```
...
The built-in metrics cover common quality dimensions, but your application likely has specific requirements. Use [`ConversationalGEval`](/docs/metrics-llm-evals) to define custom evaluation criteria in plain English:
