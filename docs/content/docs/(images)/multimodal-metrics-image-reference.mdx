...
const metric = new metrics.ImageReferenceMetric({
  threshold: 0.7,
});
...
<FAQs
  qas={[
    ...
    {
      question: "What does `max_context_size` control?",
      answer: (
        <>
          It caps how many characters above and below each image feed its
          reference score. Defaults to <code>None</code> (all surrounding text);
