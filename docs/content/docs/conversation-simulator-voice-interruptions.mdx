...
On a [full-duplex connector](/docs/conversation-simulator-voice-connectors#full-duplex-custom-connectors) this exchange runs for every conversation; the judge in step 3 only runs when the persona has an `interruption_behavior`.
...
Leave `Persona.interruption_behavior` unset (or set it to `None`) and the caller never barges in — `Turn.interrupted` stays unset.
...
- Every barge is a user `Turn` and counts toward `max_user_simulations`.
- `Turn.latency_ms` is clamped at zero when the agent was already talking before the caller finished.
