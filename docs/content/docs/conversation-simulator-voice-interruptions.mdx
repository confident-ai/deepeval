...
With interruptions enabled, the simulator no longer uses `exchange_turn()`. It drives the same connector with concurrent uplink and downlink through the **duplex exchange**:
...
The duplex exchange and the barge judge are separate things. The exchange is just the listen-while-speaking loop; the judge in step 3 only runs when the persona carries an `interruption_behavior`. On a [full-duplex connector](/docs/conversation-simulator-voice-connectors#full-duplex-custom-connectors) the simulator runs the exchange for every conversation with no policy attached, so the caller hears everything without ever cutting in. Attaching a policy is what switches the judge on — it does not change how audio moves.
...
Leave `Persona.interruption_behavior` unset (or set it to `None`) and the judge never runs: the caller never barges in and `Turn.interrupted` stays unset. On a full-duplex connector the caller still listens while it speaks; on other connectors the simulator falls back to `exchange_turn`.
...
- Every barge is a user `Turn` of its own, and every one of them counts toward `max_user_simulations`. A caller that interrupts often reaches the turn budget in fewer exchanges than a caller that waits.
- `Turn.latency_ms` on an assistant turn is clamped at zero. When the agent was already talking before the caller finished — because the caller listens while it speaks, backlogged agent speech is kept rather than drained — the agent's reply "started" before the caller's turn ended. That is a wait of zero, not a negative one.
