...
const metric = new metrics.TurnContextualRecallMetric({ threshold: 0.5 });
  ...
  qas={[
    {
      ...
      answer: (
        <>
          ...
          retrieved everything needed for the <code>expected_outcome</code>. The
          single-turn{" "}
    ...
    {
      question: "Why does recall require `expected_outcome` when relevancy doesn't?",
      answer: (
        <>
          Recall is measured against a reference: it breaks the{" "}
          <code>expected_outcome</code> into statements and checks how many map
          to a node in the <code>retrieval_context</code>. Without it there's
    ...
    },
    {
      ...
      answer: (
        <>
          Largely, yes. Low recall means the <code>retrieval_context</code>{" "}
          never contained the info the <code>expected_outcome</code> needed, so
