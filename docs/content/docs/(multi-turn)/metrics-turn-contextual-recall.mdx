...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), the <Term py="expected_outcome" ts="expectedOutcome"/> is split into sentences in code for each window, and Jev, a System One model, answers one yes/no question per sentence: can it be attributed to that window's <Term py="retrieval_context" ts="retrievalContext"/>? `P(yes) >= 0.5` counts as attributable. The equation and the LLM-written reasons are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the whole conversation (with each assistant turn's <Term py="retrieval_context" ts="retrievalContext"/>) and the <Term py="expected_outcome" ts="expectedOutcome"/> and asked three questions:
