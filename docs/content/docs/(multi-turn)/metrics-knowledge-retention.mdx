...
const metric = new metrics.KnowledgeRetentionMetric({ threshold: 0.5 });
...
<FAQs
  qas={[
    ...
    {
      question: "Why does `strict_mode` set the `threshold` to 0?",
      answer: (
        <>
          With <code>strict_mode</code> set to <code>True</code> the metric is
