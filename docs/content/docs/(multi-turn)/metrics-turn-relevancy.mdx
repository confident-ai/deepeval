...
There are **TEN** optional parameters when creating a `TurnRelevancyMetric`:
...
You can change how the `TurnRelevancyMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
...
:::note
Under `system_one` Jev judges the conversation as a whole, whereas the LLM chain scores each sliding window and averages the verdicts.
:::
