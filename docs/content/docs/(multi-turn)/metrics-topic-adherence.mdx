...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), the LLM still extracts the question-answer pairs, but each pair is classified by Jev, a System One model, with two yes/no questions: is the question on one of the <Term py="relevant_topics" ts="relevantTopics"/>, and does the response answer it? Together they give the same true/false positive/negative verdict, so the equation and the LLM-written reason are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the whole conversation and your <Term py="relevant_topics" ts="relevantTopics"/> and asked one question per topic, plus one about questions outside those topics. For two topics, the questions are:
