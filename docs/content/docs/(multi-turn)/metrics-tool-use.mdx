...
Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the whole conversation (with the tools called in each turn) and your `available_tools` and asked two questions:
