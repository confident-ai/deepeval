...
Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the whole conversation, each turn carrying its `role`, `content` and `retrieval_context`, and asked three questions:
