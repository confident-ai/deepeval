...
You can change how the `TurnFaithfulnessMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), step 3 is answered by Jev, a System One model, instead: one yes/no question per claim, with `P(yes) > 0.65` counted as truthful, `< 0.35` as contradictory, and anything in between as borderline. The extraction, the equation and the reason are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the whole conversation, each turn carrying its `role`, `content` and <Term py="retrieval_context" ts="retrievalContext"/>, and asked three questions:
