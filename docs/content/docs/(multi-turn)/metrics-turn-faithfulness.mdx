...
const metric = new metrics.TurnFaithfulnessMetric({ threshold: 0.5 });
  ...
  qas={[
    {
      ...
      answer: (
        <>
          ...
          checked against its window's <code>retrieval_context</code>. The
          single-turn{" "}
    ...
    {
      question: "Does every turn need its own `retrieval_context`?",
      answer: (
        <>
          Attach <code>retrieval_context</code> to turns generated from
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <code>retrieval_context</code>, not claims that merely happen to be
          true — correct-but-unretrieved facts count as unfaithful. Inspect
          per-claim verdicts with <code>verbose_mode</code>, and try{" "}
          <code>penalize_ambiguous_claims</code> for vague claims.
        </>
      ),
    },
    {
      question:
        "How do `window_size` and `truths_extraction_limit` affect long conversations?",
      answer: (
        <>
          <code>window_size</code> (default <code>10</code>) groups turns per
          evaluation; <code>truths_extraction_limit</code> caps truths pulled
