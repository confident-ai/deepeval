---
id: metrics-conversation-completeness
title: Conversation Completeness
sidebar_label: Conversation Completeness
languages: [python, typescript]
---
<MetricTagsDisplayer jev={true} multiTurn={true} chatbot={true} referenceless={true} />

The conversation completeness metric is a conversational metric that determines whether your LLM chatbot is able to complete an end-to-end conversation by satisfying user needs **throughout a conversation**.

:::note
The `ConversationCompletenessMetric` can be used as a proxy to measure user satisfaction throughout a conversation. Conversational metrics are particular useful for an LLM chatbot use case.
:::

## Required Arguments

To use the `ConversationCompletenessMetric`, you'll have to provide the following arguments when creating a [`ConversationalTestCase`](/docs/evaluation-multiturn-test-cases):

- `turns`

You must provide the `role` and `content` for evaluation to happen. Read the [How Is It Calculated](#how-is-it-calculated) section below to learn more.

## Usage

<include cwd>snippets/metrics/set-eval-mode.mdx</include>

The `ConversationCompletenessMetric()` can be used for [end-to-end](/docs/evaluation-end-to-end-llm-evals) multi-turn evaluation:

<Switch>

<Case id="python">

```python
from deepeval.test_case import Turn, ConversationalTestCase
from deepeval.metrics import ConversationCompletenessMetric
from deepeval import evaluate

convo_test_case = ConversationalTestCase(
    turns=[Turn(role="...", content="..."), Turn(role="...", content="...")]
)
metric = ConversationCompletenessMetric(threshold=0.5)

# To run metric as a standalone
# metric.measure(convo_test_case)
# print(metric.score, metric.reason)

evaluate(test_cases=[convo_test_case], metrics=[metric])
```

</Case>

<Case id="typescript">

```typescript
import { Turn, ConversationalTestCase } from "deepeval/test-case";
import { ConversationCompletenessMetric } from "deepeval/metrics";
import { evaluate } from "deepeval";

const convoTestCase = new ConversationalTestCase({
  turns: [new Turn({ role: "user", content: "..." }), new Turn({ role: "assistant", content: "..." })],
});
const metric = new ConversationCompletenessMetric({ threshold: 0.5 });

// To run metric as a standalone
// await metric.measure(convoTestCase);
// console.log(metric.score, metric.reason);

await evaluate([convoTestCase], [metric]);
```

</Case>

</Switch>

<Switch>

<Case id="python">

There are **NINE** optional parameters when creating a `ConversationCompletenessMetric`:

</Case>

<Case id="typescript">

There are **SIX** optional parameters when creating a `ConversationCompletenessMetric`:

</Case>

</Switch>

- [Optional] `threshold`: a number representing the minimum passing threshold. Can also be set to <Term py="None" ts="null"/> to run the metric in [score-only mode](/docs/metrics-introduction#metric-thresholds). Defaulted to `0.5`.
- [Optional] `model`: a string specifying which of OpenAI's GPT models to use, **OR** [any custom LLM model](/docs/metrics-introduction#using-a-custom-llm) of type `DeepEvalBaseLLM`. Defaulted to <DefaultLLMModel />.
- [Optional] <Term py="include_reason" ts="includeReason"/>: a boolean which when set to <Term py="True" ts="true"/>, will include a reason for its evaluation score. Defaulted to <Term py="True" ts="true"/>.
- [Optional] <Term py="strict_mode" ts="strictMode"/>: a boolean which when set to <Term py="True" ts="true"/>, enforces a binary metric score: 1 for perfection, 0 otherwise. It also overrides the current threshold and sets it to 1. Defaulted to <Term py="False" ts="false"/>.
- <Only id="python">
    [Optional] `async_mode`: a boolean which when set to `True`, enables [concurrent execution within the `measure()` method.](/docs/metrics-introduction#measuring-metrics-in-async) Defaulted to `True`.
  </Only>
- [Optional] <Term py="verbose_mode" ts="verboseMode"/>: a boolean which when set to <Term py="True" ts="true"/>, prints the intermediate steps used to calculate said metric to the console, as outlined in the [How Is It Calculated](#how-is-it-calculated) section. Defaulted to <Term py="False" ts="false"/>.
- [Optional] `flaky`: a boolean which when set to <Term py="True" ts="true"/>, [marks the metric as flaky](/docs/metrics-introduction#flaky-metrics). Defaulted to <Term py="False" ts="false"/>.
- [Optional] <Term py="system_one_model" ts="systemOneModel"/>: the Jev model to use, as a string or a `DeepEvalBaseSystemOneModel`. Only used under `hybrid` or `system_one` <Term py="eval_mode" ts="evalMode"/>. Defaulted to <DefaultSystemOneModel />.
- [Optional] <Term py="eval_mode" ts="evalMode"/>: `llm`, `hybrid` or `system_one`, choosing whether an LLM, Jev, or both judge. Defaulted to the configured [eval mode](/docs/evaluation-eval-modes) (`llm` unless set).

### As a standalone

You can also run the `ConversationCompletenessMetric` on a single test case as a standalone, one-off execution.

<Switch>

<Case id="python">

```python
...

metric.measure(convo_test_case)
print(metric.score, metric.reason)
```

</Case>

<Case id="typescript">

```typescript
...

await metric.measure(convoTestCase);
console.log(metric.score, metric.reason);
```

</Case>

</Switch>

:::caution
This is great for debugging or if you wish to build your own evaluation pipeline, but you will **NOT** get the benefits (testing reports, Confident AI platform) and all the optimizations (speed, caching, computation) the `evaluate()` function or `deepeval test run` offers.
:::

## How Is It Calculated?

You can change how the `ConversationCompletenessMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).

### LLM-as-a-judge

The `ConversationCompletenessMetric` score is calculated according to the following equation:

<Equation formula="\text{Conversation Completeness} = \frac{\text{Number of Satisfied User Intentions in Conversation}}{\text{Total Number of User Intentions in Conversation}}" />

The `ConversationCompletenessMetric` assumes that a conversion is only complete if user intentions, such as asking for help to an LLM chatbot, are met by the LLM chatbot.

Hence, the `ConversationCompletenessMetric` first uses an LLM to extract a list of high level user intentions found in `turns` (in `"user"` roles), before using the same LLM to determine whether each intention was met and/or satisfied throughout the conversation by the `"assistant"`.

### Hybrid

Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), the LLM still extracts the user intentions, but whether each one was satisfied is answered by Jev, a System One model: one yes/no question per intention, with `P(yes) >= 0.5` counted as satisfied. The equation and the LLM-written reason are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the whole conversation and asked three questions:

| Question                                                                                                          | Type  | Weight |
| ----------------------------------------------------------------------------------------------------------------- | ----- | ------ |
| Every intention the user expresses in `turns` is satisfied by the assistant turns by the end of the conversation. | Noul  | 2      |
| No user request in `turns` is ignored, refused without cause or left only partly addressed by the assistant.      | Noul  | 1      |
| Across `turns`, how many of the user's intentions does the assistant satisfy? (None satisfied → All satisfied)    | Score | 1      |

<include cwd>snippets/metrics/system-one-scoring.mdx</include>

## FAQs

<FAQs
  qas={[
    {
      question: "How is Conversation Completeness different from Goal Accuracy?",
      answer: (
        <>
          Completeness measures <em>breadth</em> — did the assistant satisfy{" "}
          <strong>every</strong> intention raised.{" "}
          <a href="/docs/metrics-goal-accuracy">Goal Accuracy</a> measures
          reaching a single overarching task. A bot can hit the main goal yet
          leave secondary requests unaddressed.
        </>
      ),
    },
    {
      question: "What does an incomplete conversation look like?",
      answer: (
        <>
          The user voiced several intentions but only some were met — e.g.
          asking to reset a password <em>and</em> update billing, where the bot
          only handled the reset. The unaddressed intention lowers the score.
        </>
      ),
    },
    {
      question: "How does the metric decide what the user wanted?",
      answer: (
        <>
          An LLM extracts high-level intentions from the <code>"user"</code>{" "}
          turns, then checks each against the <code>"assistant"</code> turns.
          The score is satisfied intentions over total intentions.
        </>
      ),
    },
    {
      question: "Can I use this as a user-satisfaction proxy?",
      answer: (
        <>
          Yes. Since it reflects how many user needs were met end to end,{" "}
          <code>ConversationCompletenessMetric</code> works well as a proxy for
          overall satisfaction in chatbot use cases.
        </>
      ),
    },
  ]}
/>

