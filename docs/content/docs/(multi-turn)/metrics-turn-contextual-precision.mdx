...
const metric = new metrics.TurnContextualPrecisionMetric({ threshold: 0.5 });
  ...
  qas={[
    {
      ...
      answer: (
        <>
          ...
          node ranking against the <code>expected_outcome</code>. The
          single-turn{" "}
    ...
    {
      question:
        "The right info is in my `retrieval_context` but precision is low — what's wrong?",
      answer: (
        <>
    ...
    {
      question: "Why does this metric need `expected_outcome`?",
      answer: (
        <>
          It uses the <code>expected_outcome</code> to decide which nodes in the{" "}
          <code>retrieval_context</code> were useful before scoring their
