...
- [Optional] `classifiers`: a list of [`Classifier`](/docs/classifiers-classifier)s to label each test case alongside the metrics. See [classifiers](/docs/classifiers-introduction).
- [Optional] `identifier`: a string label for this test run on Confident AI.
- [Optional] `async_config`: an `AsyncConfig` controlling concurrency. See [async configs](/docs/evaluation-flags-and-configs#async-configs).
- [Optional] `display_config`: a `DisplayConfig` controlling console output. See [display configs](/docs/evaluation-flags-and-configs#display-configs).
...
Pass `classifiers=` to label each test case in the same run. A classifier only affects a test case's verdict when the test case sets `expected_labels` for it; otherwise it just reports the label:
...
Classifiers are not yet supported on `evals_iterator()` or on traced components. See the [classifiers introduction](/docs/classifiers-introduction) for how verdicts and expected labels work.
:::
