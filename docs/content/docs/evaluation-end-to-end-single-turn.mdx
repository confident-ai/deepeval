...
await dataset.pull({ alias: "My dataset" });
```
...
import { yourLlmApp } from "./your_app"; // replace with your LLM app
...
await evaluate(
  testCases,
  [new metrics.AnswerRelevancyMetric(), new metrics.FaithfulnessMetric()]
);
```
