...
Drop `deepeval test run` into a GitHub Actions workflow to run your `test_*.py` evals on every push or pull request. Write tests with `assert_test()` or `evaluate()`, then wire the command into your pipeline with the secrets and flags below.
...
:::note
`evaluate()` does not raise when metrics fail, so a single `test_*` function can still pass in pytest even when scores are low. `--pass-rate` and `--required-metrics` on `deepeval test run` fail the workflow after all tests finish if those thresholds are not met. To fail pytest immediately on a bad case, use `assert_test()`.
:::
