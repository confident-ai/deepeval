...
Write your evals in a `test_*.py` file using `assert_test()` or `evaluate()`.
...
Drop `deepeval test run` into a workflow to run your unit tests on every push or PR. This example uses `poetry` for installation and `OPENAI_API_KEY` as your LLM judge. Add `CONFIDENT_API_KEY` to send results to Confident AI.
