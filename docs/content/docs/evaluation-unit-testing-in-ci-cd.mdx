...
Drop `deepeval test run` into a GitHub Actions workflow to run your `test_*.py` evals on every push or pull request. Use `assert_test()` in your tests, then wire the command into your pipeline with the secrets and flags below.
...
For traced apps, use `assert_test(golden=..., metrics=...)` after calling your `@observe`-decorated code inside the same `test_*` function (see the traced examples above).
      ...
      - name: Run `deepeval` unit tests
        run: |
          poetry run deepeval test run tests/evals/test_cicd.py \
            --pass-rate 0.80 \
            --required-metrics "Answer Relevancy"
```
...
The `--pass-rate` and `--required-metrics` flags (shown in the command above) apply only to `assert_test()` results. They fail the workflow when overall pass rate or named metrics do not meet your bar. See [`--pass-rate`](/docs/evaluation-flags-and-configs#pass-rate-github-actions) and [`--required-metrics`](/docs/evaluation-flags-and-configs#required-metrics-github-actions) for details.
:::

[Click here](/docs/evaluation-flags-and-configs#flags-for-deepeval-test-run) to learn about the other optional flags available to `deepeval test run`.
