...
There are **ONE** mandatory and **EIGHT** optional parameters when creating an `PromptAlignmentMetric`:
...
- <Only id="python">[Optional] `eval_mode`: `llm`, `hybrid` or `system_one`. Only decides the reason for an invalid output: written by an LLM, or under `system_one` the schema validation error itself. Defaulted to the configured [eval mode](/docs/evaluation-eval-modes) (`llm` unless set).</Only>
