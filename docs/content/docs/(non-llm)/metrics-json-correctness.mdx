...
- [Optional] <Term py="eval_mode" ts="evalMode"/>: `llm`, `hybrid` or `system_one`. Only decides the reason for an invalid output: written by an LLM, or under `system_one` the schema validation error itself. Defaulted to the configured [eval mode](/docs/evaluation-eval-modes) (`llm` unless set).
