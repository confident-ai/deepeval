...
By default, built-in metric prompts are written in English. If you evaluate applications in another language, you can translate those prompts **once** with the CLI instead of rewriting them yourself.

Run `deepeval translate` with your target language and the metric class names you use (the same names as in Python, e.g. `FaithfulnessMetric`):

```bash
deepeval translate Spanish --metrics "FaithfulnessMetric, AnswerRelevancyMetric"
```

You can pass a comma-separated list in one value or repeat `--metrics` for each class. Optionally pass `--model` to choose which LLM performs the translation; otherwise `deepeval` uses the same default as your metric evaluations.

`deepeval` sends each metric’s English template to your configured **evaluation model**, translates the wording into your target language, and saves the result under `.deepeval/templates.json` in your project. On every later `measure`, `evaluate`, or `assert_test`, those saved templates are loaded automatically.

:::tip
Translate only the metrics you actually run. You can run the command again later to add more metrics or refresh translations. To switch back to the built-in English prompts, run `deepeval translate English --metrics "YourMetric"` this removes your saved overrides and defaults to original template for these metrics.
:::
