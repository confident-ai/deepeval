...
await evaluate(
  [new LLMTestCase({ input: "What's `deepeval`?", actualOutput: "Your favorite eval framework's favorite evals framework." })],
  [new metrics.AnswerRelevancyMetric()]
);
```
...
```typescript
import { LLMTestCase, SingleTurnParams } from "deepeval/testCase";
import { metrics } from "deepeval";

const testCase = new LLMTestCase({ input: "...", actualOutput: "...", expectedOutput: "..." });
const correctness = new metrics.GEval({
  name: "Correctness",
  criteria: "Correctness - determine if the actual output is correct according to the expected output.",
...
```typescript
import { Turn, MultiTurnParams, ConversationalTestCase } from "deepeval/testCase";
import { metrics } from "deepeval";

const convoTestCase = new ConversationalTestCase({ turns: [new Turn({ role: "user", content: "..." }), new Turn({ role: "assistant", content: "..." })] });
...
const relevancy = new metrics.AnswerRelevancyMetric({ threshold: 0.5 });
...
const convoTestCase = new ConversationalTestCase({ turns: [new Turn({ role: "user", content: "..." }), new Turn({ role: "assistant", content: "..." })] });
const roleAdherence = new metrics.RoleAdherenceMetric({ threshold: 0.5 });
...
```typescript
import { LLMTestCase, MLLMImage } from "deepeval/testCase";
import { metrics } from "deepeval";

const testCase = new LLMTestCase({ input: `What does this image say? ${new MLLMImage({ url: "..." })}`, actualOutput: "No idea!" });
const imageCoherence = new metrics.ImageCoherenceMetric({ threshold: 0.5 });
...
const testCase = new LLMTestCase({ input: "...", actualOutput: "..." });
const bias = new metrics.BiasMetric({ threshold: 0.5 });
...
await evaluate([testCase], [new metrics.AnswerRelevancyMetric()]);
```
...
const nestedComponent = observe({
  metrics: [new metrics.AnswerRelevancyMetric()],
  fn: async () => {
    // 3. Create test case at runtime
    updateCurrentSpan({ testCase: new LLMTestCase({ input: "...", actualOutput: "..." }) });
  },
});
...
// 4. Create dataset
const dataset = new EvaluationDataset({ goldens: [new Golden({ input: "Test input" })] });
...
// Initialize metric with threshold
const metric = new metrics.AnswerRelevancyMetric({ threshold: 0.5 });
...
// measure() is already async — these will all run at the same time
await Promise.all([
  metric1.measure(testCase),
  metric2.measure(testCase),
  metric3.measure(testCase),
  metric4.measure(testCase),
]);
...
const metric = new metrics.AnswerRelevancyMetric({ verboseMode: true });
await metric.measure(testCase);
```
...
Every metric accepts a `threshold` parameter (defaulted to `0.5`) that decides whether it passes or fails: `metric.is_successful()` is only `True` if `metric.score` reaches the `threshold` (or stays below it, for "lower is better" metrics such as `BiasMetric`).
...
const metric = new metrics.AnswerRelevancyMetric({ threshold: 0.7 });
```

</LangSwitch>

You can also set `threshold=None` to run a metric in **score-only mode**. The score and reason are still computed, recorded, and reported, but the metric has no pass/fail opinion — `metric.is_successful()` returns `None`, and the metric never contributes to its test case's pass/fail status.
...
:::caution
Because every test case must be able to pass or fail, each `evaluate()` or `assert_test()` call requires **at least one non-flaky metric with a `threshold`**.
:::

## Flaky Metrics

Every metric also accepts a `flaky` parameter (defaulted to `False`). A flaky metric behaves exactly like a normal one — its score, reason, and verdict are all computed and reported — but its failure **never decides its test case's pass/fail status**.
...
:::info
Test cases can also be [marked as flaky](/docs/evaluation-test-cases#mark-test-cases-as-flaky) — a failing flaky test case makes `assert_test()` print a warning instead of raising an `AssertionError`.
:::
  ...
  qas={[
    {
      ...
      answer: (
        <>
          ...
          (e.g. RAG or agent metrics) and <strong>1-2</strong> custom use-case
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          live LLM <a href="/docs/evaluation-llm-tracing">traces</a> rather than
          test cases. For continuous online evals, dashboards, and alerts on
          live traffic, run them on{" "}
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          score spans and traces automatically. Wire them up on{" "}
