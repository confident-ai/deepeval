...
Almost all predefined metrics on `deepeval` use **LLM-as-a-judge**, with techniques such as **QAG** (question-answer-generation), **DAG** (deep acyclic graphs), and **G-Eval** (with the exception of [**JevEval**](/docs/metrics-jev-eval), which is scored by Jev, a System One model, instead of an LLM).

Most score [test cases](/docs/evaluation-test-cases) representing atomic interactions, while [trajectory metrics](/docs/evaluation-trajectory-based-llm-evals) score the complete ordered trace produced by an AI agent.
...
Custom metrics allow you to define your **custom criteria** using SOTA implementations of LLM-as-a-Judge (and Jev-as-a-Judge) metrics in everyday language:
...
- <Only id="python">
    Support frontier, SOTA eval techniques such as Jev-as-a-Judge (`JevEval`),
    scored by a System One model with calibrated probabilities instead of
    generated text
  </Only>
...
:::tip
If you want a **categorical** LLM-as-a-judge, one that assigns a label from a set you define instead of a score, check out [classifiers](/docs/classifiers-introduction). You can use metrics and classifiers together in the same `evaluate()` call.
:::
...
`deepeval` provides G-Eval, a state-of-the-art LLM evaluation framework for anyone to create a custom LLM-evaluated metric using natural language, and JevEval, which scores the bounded questions you define with Jev, a System One model, instead of an LLM. Both are available for single-turn and multi-turn evals.

<Tabs items={["G-Eval", "JevEval"]}>
<Tab value="G-Eval">
...
const correctness = new JevEval({
  ...
  questions: [
    ...
    new Score({
      question: "How closely does actual_output match expected_output?",
      levels: ["Contradicts it", "Mostly wrong", "Mostly right", "Fully matches"],
    }),
  ],
});
...
Under the hood, G-Eval first generates a series of evaluation steps, before using these steps in conjunction with information in an `LLMTestCase` for evaluation; JevEval skips generation entirely and sends your questions to Jev, which returns calibrated probabilities that `deepeval` turns into a score. For more information, visit the [G-Eval](/docs/metrics-llm-evals) and [JevEval](/docs/metrics-jev-eval) documentation pages.
...
- **[Task Completion](/docs/metrics-task-completion):** Assesses whether the agent successfully completed its task.
- **[Step Efficiency](/docs/metrics-step-efficiency):** Assesses whether the agent completed the task without unnecessary or redundant steps.
- **[Plan Adherence](/docs/metrics-plan-adherence):** Assesses whether the agent followed its generated plan during execution.
- **[Plan Quality](/docs/metrics-plan-quality):** Assesses whether the generated plan was logical, complete, and efficient.

**Component-level action metrics** evaluate individual LLM tool-calling decisions:

- **[Tool Correctness](/docs/metrics-tool-correctness):** Assesses whether the agent selected the correct tools.
- **[Argument Correctness](/docs/metrics-argument-correctness):** Assesses whether the agent supplied the correct arguments to those tools.
...
Voice agents get seven more conversational metrics: [Voice Naturalness](/docs/metrics-voice-naturalness), [Voice Consistency](/docs/metrics-voice-consistency), [Voice Reliability](/docs/metrics-voice-reliability), [Speech Intelligibility](/docs/metrics-speech-intelligibility), [Audio Integrity](/docs/metrics-audio-integrity), [Turn-Taking Naturalness](/docs/metrics-turn-taking-naturalness), and [Agent Responsiveness](/docs/metrics-agent-responsiveness). See [voice evaluation](/docs/evaluation-voice) for the setup.
...
## Choose Who Judges With Eval Modes

<Only id="python">

Every built-in LLM-as-a-judge metric can be judged by an LLM alone, by an LLM and [Jev](/integrations/models/typesafe-ai) together, or by Jev alone. Jev is a System One model that answers bounded questions with calibrated probabilities instead of generating text, and the [**eval mode**](/docs/evaluation-eval-modes) picks which of the three you get:

| Eval mode       | Who does what                                                                                                                                         |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `llm` (default) | The LLM runs the whole metric.                                                                                                                        |
| `hybrid`        | The LLM extracts and writes the reason, and Jev makes the decisions. If a Jev call fails, the LLM makes that decision instead.                         |
| `system_one`    | Jev runs the whole metric in **one request**, and the reason is built from its answers. No LLM is called, so no LLM API key is needed. |

Switch modes for every metric from the CLI, or pass `eval_mode` to a single metric to override it:
...
Set your TypeSafe AI key to use Jev under the `hybrid` and `system_one` [eval modes](/docs/evaluation-eval-modes), and in [`JevEval`](/docs/metrics-jev-eval):

```bash
export TYPESAFE_API_KEY=<your-typesafe-api-key>
```

Or save it with the CLI, which also lets you pick the Jev model:

```bash
deepeval set-typesafe --prompt-api-key --save=dotenv
```

See the [TypeSafe AI integration](/integrations/models/typesafe-ai) for the available Jev models.
...
Every metric accepts a `threshold` parameter (defaulted to `0.5`) that decides whether it passes or fails: <Term py="metric.is_successful()" ts="metric.isSuccessful()"/> is only <Term py="True" ts="true"/> if `metric.score` reaches the `threshold`. Every metric scores in the same direction: 1 is a pass, 0 is a failure, and `threshold` is always a minimum.
...
:::info
[Classifiers](/docs/classifiers-introduction) have the same informational mode: a classifier whose `name` has no entry in the test case's [`expected_labels`](/docs/classifiers-introduction#expected-labels) records its label and reason but never influences the test case's pass/fail status.
:::
...
If you want custom criteria judged without a language model at all, use [`JevEval`](/docs/metrics-jev-eval). You write the questions, Jev, a System One model, answers each with calibrated probabilities, and `deepeval` turns them into a score with a fixed equation, so there's no chain-of-thought, no generated text, and no score to parse.
...
<FAQs
  qas={[
    ...
    },
    {
      ...
      answer: (
        <>
          Most do — they're <strong>LLM-as-a-judge</strong> (QAG, G-Eval).{" "}
          <a href="/docs/metrics-jev-eval">JevEval</a> is the exception: it is
          scored by Jev, a System One model, with no LLM in the scoring path.
          The <a href="/docs/metrics-dag">DAGMetric</a> can be fully
          deterministic, and for statistical scorers like ROUGE/BLEU you build a{" "}
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          run them on <a href="https://confident-ai.com">Confident AI</a>, the
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          trace are{" "}
          <a href="/docs/evaluation-trajectory-based-llm-evals">
            trajectory-based
          </a>
          , while metrics on individual spans are <a href="/docs/evaluation-component-level-llm-evals">
            component-level
          </a>. Wire them up on <a href="https://confident-ai.com">
            Confident AI
          </a> to track metric scores on production traffic over time.
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          ...
          <code>StepEfficiencyMetric</code>, <code>PlanAdherenceMetric</code>,
          and <code>PlanQualityMetric</code>. They require tracing and are
          passed to the evals iterator or test assertion rather than measured
          against one isolated component.
        </>
      ),
    },
  ]}
