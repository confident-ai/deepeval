...
In your project, install `deepeval` alongside [Vitest](https://vitest.dev), which `deepeval` plugs into to collect and run your eval files:

```bash
npm install --save-dev deepeval vitest
```

Vitest is an optional peer dependency, so it isn't pulled in for you — `deepeval test run` needs it present.
...
import { SingleTurnParams } from "deepeval/testCase";

it("answers health questions with medically sound advice", async () => {
  const correctnessMetric = new metrics.GEval({
    name: "Correctness",
    criteria: "Determine if the 'actual output' is correct based on the 'expected output'.",
  ...
  const testCase = new LLMTestCase({
    input: "I have a persistent cough and fever. Should I be worried?",
    // Replace this with the actual output from your LLM application
    actualOutput: "A persistent cough and fever could be a viral infection or something more serious. See a doctor if symptoms worsen or don't improve in a few days.",
    expectedOutput: "A persistent cough and fever could indicate a range of illnesses, from a mild viral infection to more serious conditions like pneumonia or COVID-19. You should seek medical attention if your symptoms worsen, persist for more than a few days, or are accompanied by difficulty breathing, chest pain, or other concerning signs.",
  });
...
The file name matters: `deepeval test run` only picks up files your test runner already discovers, which by default means `.test.ts` or `.spec.ts`.
...
import { it, expect } from "vitest";
import "deepeval/vitest";
import { Turn, ConversationalTestCase, metrics } from "deepeval";
import { MultiTurnParams } from "deepeval/testCase";

it("answers user questions in a professional manner", async () => {
  const professionalismMetric = new metrics.ConversationalGEval({
    name: "Professionalism",
    criteria: "Determine whether the assistant has acted professionally based on the content.",
  ...
  const testCase = new ConversationalTestCase({
    turns: [
      ...
      new Turn({ role: "assistant", content: "DeepEval is an open-source LLM eval package." }),
    ],
  });
...
Keep single-turn and multi-turn metrics in separate `toPass` calls — a metric can only score the kind of test case it was built for.
...
```typescript
const correctnessMetric = new metrics.GEval({ ..., model: "o1" });
```
...
Set `CONFIDENT_API_KEY` before your test run and the results are posted as they finish, with a link to the report printed at the end:
...
Results are only kept for a run that had the key set — there's no deferred upload of an earlier run yet, so set it before you run rather than after.
...
const dataset = new EvaluationDataset({ goldens: [new Golden({ input: "Why is the sky blue?" })] });
```
...
To do the same thing inside your test suite instead of a standalone script, invoke your app and hand the golden to <Term py="assert_test()" ts="toPass()"/>. The trace your app just produced is scored against the metrics on its spans, and any gaps in it are filled from the golden:
...
it.each(goldens)("explains the answer using what it retrieved: $input", async (golden) => {
  await myAgent(golden.input);
  await expect(golden).toPass();
});
```

Reach for this form rather than `evalsIterator()` when you're inside a test file — both capture traces through the same channel, so running one inside the other will have them compete.
...
:::tip
Every <Term py="evals_iterator()" ts="evalsIterator()"/> run is snapshotted to disk, so you can open it in a trace-tree TUI — with per-span scores and metric reasons — by running bare `deepeval inspect`. See the [`deepeval inspect` reference](/docs/command-line-interface#inspect) for full details.
    ...
    },
    {
      ...
      answer: (
        <>
          <Switch>
            <Case id="python">
              Put it anywhere Pytest can discover it, usually alongside your app
              or in a <code>tests/</code> folder. Then run{" "}
              <code>deepeval test run path/to/test_file.py</code>.
            </Case>
            <Case id="typescript">
              Put it anywhere your test runner can discover it, usually
              alongside your app or in a <code>tests/</code> folder. The name
              has to end in <code>.test.ts</code> or <code>.spec.ts</code>,
              otherwise it is never collected. Then run{" "}
              <code>npx deepeval test run path/to/file.test.ts</code>.
            </Case>
          </Switch>
        </>
      ),
...
You can find the full example [here on our Github](https://github.com/confident-ai/deepeval/tree/main/typescript/examples/vitest).
