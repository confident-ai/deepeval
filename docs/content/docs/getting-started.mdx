...
const dataset = new EvaluationDataset({
  goldens: [new Golden({ input: "Why is the sky blue?" })],
});
```
...
- <Switch><Case id="python">Your integration's adapter (or `@observe`) created spans</Case><Case id="typescript">`observe` created spans</Case></Switch> for the components inside the trace.
- The <Term py="metrics=[...]" ts="metrics: [...]"/> you attached to one of those spans scored it once the trace finished.
...
To do the same thing inside your test suite instead of a standalone script, invoke your app and hand the golden to the assertion. The trace your app just produced is scored against the metrics on its spans, and any gaps in it are filled from the golden:
...
const goldens = [new Golden({ input: "Why is the sky blue?" })];

it.each(goldens)("answers the user's question: $input", async (golden) => {
...
:::tip
Every <Term py="evals_iterator()" ts="evalsIterator()"/> run is snapshotted to disk, so you can open it in a trace-tree TUI — with per-span scores and metric reasons — by running bare <Term py="deepeval inspect" ts="npx deepeval inspect"/>. See the [`deepeval inspect` reference](/docs/command-line-interface#inspect) for full details.
