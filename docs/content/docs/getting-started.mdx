...
To keep your testing reports in a centralized place on the cloud, use [Confident AI](https://www.confident-ai.com), an AI observability and evals platform that DeepEval integrates with natively:
...
For CI or other non-interactive environments, pass an existing key with
...
<Tabs items={["LLM-as-a-judge", "Jev-as-a-judge"]}>
<Tab value="LLM-as-a-judge">
...
<NotImplemented id="typescript" feature="JevEval">

Use the LLM-as-a-judge example until `JevEval` is available in TypeScript.
...
Confident AI is an AI observability and evals platform that DeepEval integrates with natively, and helps you build the best LLM evals pipeline.
...
Simply set the `DEEPEVAL_RESULTS_FOLDER` environment variable to your relative path of choice. Everything `deepeval` stores locally, including the SQLite alternative, is explained [here](/docs/evaluation-local-backend-storage).
...
- <Term py="evals_iterator()" ts="evalsIterator()" /> looped through your dataset,
...
<ImageDisplayer
  src={ASSETS.componentLevelEvals}
  alt="component-level metrics attached to individual spans within an agent trace"
/>
...
<include cwd>
  snippets/integrations/framework-integration-card-examples.mdx
</include>
