...
For CI or other non-interactive environments, pass an existing key with <Term py="deepeval login --api-key ..." ts="npx deepeval login --api-key ..."/>.
...
Set your `OPENAI_API_KEY` as an env variable (or [configure any other LLM](/docs/metrics-introduction#configure-llm-judges)) before running this example.
...
Set your `TYPESAFE_API_KEY` as an env variable ([configure Jev](/docs/metrics-introduction#configure-jev)) before running this example. No LLM key is needed.
...
import { it, expect } from "vitest";
import { LLMTestCase, SingleTurnParams } from "deepeval/test-case";
import { JevEval, Noul, Score } from "deepeval/metrics";
import "deepeval/vitest";

it("gives medically sound advice", async () => {
  const correctnessMetric = new JevEval({
    name: "Correctness",
    evaluationParams: [
      SingleTurnParams.ACTUAL_OUTPUT,
      SingleTurnParams.EXPECTED_OUTPUT,
    ...
    questions: [
      new Noul({
        statement:
          "No fact in actual_output contradicts a fact in expected_output.",
        weight: 2,
      }),
      new Score({
        question:
...
import { ConversationalJevEval, Noul, Score } from "deepeval/metrics";
import "deepeval/vitest";

it("stays professional across the conversation", async () => {
  const professionalismMetric = new ConversationalJevEval({
    name: "Professionalism",
    evaluationParams: [MultiTurnParams.CONTENT],
    questions: [
      new Noul({
        statement:
          "Every assistant turn in turns is polite and professional in tone.",
        weight: 2,
      }),
      new Score({
        question: "How professional is the assistant across turns?",
        levels: [
          "Unprofessional",
