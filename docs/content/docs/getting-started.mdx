...
To do the same thing inside your test suite instead of a standalone script, invoke your app and hand the golden to `assert_test()`. The trace your app just produced is scored against the metrics on its spans, and any gaps in it are filled from the golden:
