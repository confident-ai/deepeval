...
Run <Term py="touch test_example.py" ts="touch test_example.ts"/> in your terminal and paste in the following code:
...
```typescript title="test_example.ts"
...
Then, run <Term py="deepeval test run" ts="npx tsx"/> from the root directory of your project to evaluate your LLM app **end-to-end**:
...
```typescript title="test_example.ts"
import { evaluate, Turn, ConversationalTestCase, metrics } from "deepeval";
...
await evaluate([testCase], [professionalismMetric]);
```
...
After you've logged in, Confident AI will **generate testing reports and automate regression testing** whenever you run a test run to evaluate your LLM application inside any environment, at any scale, anywhere.
