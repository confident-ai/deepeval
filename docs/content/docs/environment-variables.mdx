...
| <Only id="python">`DEEPEVAL_EVAL_MODE`</Only> | <Only id="python">`llm` / `hybrid` / `system_one` / unset</Only> | <Only id="python">Who decides in LLM-as-a-judge metrics: the LLM end to end (`llm`, default), the LLM extracts and a System One model decides (`hybrid`), or a System One model runs the whole metric with no LLM at all (`system_one`). See [DeepEval eval mode](#deepeval-eval-mode).</Only> |
...
`DEEPEVAL_EVAL_MODE` sets the [eval mode](/docs/evaluation-eval-modes), which picks who decides in `deepeval`'s LLM-as-a-judge metrics and classifiers, and is how [Jev](/integrations/models/typesafe-ai), TypeSafe AI's System One model, takes part in an eval:
...
Unset, it means `llm`, so existing setups behave as before; the `DEEPEVAL_MODE` feature channel has no effect on it. An `eval_mode=` argument on a metric overrides it, and `deepeval set-eval-mode` sets it for you. Unrecognised values mean unset. G-Eval metrics are LLM-as-a-judge by definition and ignore it.
