...
| `CONFIDENT_REGION`           | <Switch><Case id="python">`US` / `EU` / unset</Case><Case id="typescript">`US` / `EU` / `AU` / unset</Case></Switch> | Confident AI data region. When unset, the region is inferred from your API key prefix.                                                                        |
...
| `DEEPEVAL_LOCAL_STORE`       | `json` / `sqlite` / unset  | Backend for saving finished test runs locally. `json` (default) keeps the JSON exports; `sqlite` appends each run to a `deepeval.db` database instead. See [SQLite local store](#sqlite-local-store). |
| `DEEPEVAL_MODE`              | `stable` / `experimental` / unset | Feature channel. `stable` (default) only runs finalised behaviour; `experimental` opts into the latest features, which may change or break between releases. Unrecognised values mean `stable`. See [DeepEval mode](#deepeval-mode). |
| `DEEPEVAL_EVAL_MODE`         | `llm` / `hybrid` / `system_one` / unset | Who decides in LLM-as-a-judge metrics: the LLM end to end (`llm`, default), the LLM extracts and a System One model decides (`hybrid`), or a System One model runs the whole metric with no LLM at all (`system_one`). See [DeepEval eval mode](#deepeval-eval-mode). |
| `DEEPEVAL_SQLITE_INCLUDE_ROW_JSON`   | `1` / `0` / unset          | SQLite store only. Also store the full JSON object on every test case, trace and span row (`payload_json`) so unpromoted fields can be queried per row. Off by default: roughly doubles the database size. See [here](/docs/evaluation-local-backend-storage#schema). |
...
By default every finished run is written as JSON. Set `DEEPEVAL_LOCAL_STORE=sqlite` (or run <Term py="deepeval set-local-store sqlite --save=dotenv" ts="npx deepeval set-local-store sqlite --save=dotenv"/>) to keep a queryable history of runs in a single SQLite database instead. A full comparison of the two backends, where each file lands, and how to export the database to other platforms is [here](/docs/evaluation-local-backend-storage).
...
The database lives at `deepeval.db` inside `DEEPEVAL_RESULTS_FOLDER` (or `DisplayConfig(results_folder=...)`) when set, otherwise inside the `.deepeval` cache folder. No JSON files are written in this mode, and nothing is uploaded — it is purely local storage.
...
The database lives at `deepeval.db` inside `DEEPEVAL_RESULTS_FOLDER` when set, otherwise inside the `.deepeval` cache folder. The timestamped `test_run_*.json` export is skipped in this mode; `.deepeval/.latest_test_run.json` is still written because `npx deepeval view` uploads from it.

:::caution[Node.js 24 or newer]
The TypeScript store is built on the runtime's built-in `node:sqlite` module, which is only stable from Node 24. On an older Node, setting `DEEPEVAL_LOCAL_STORE=sqlite` is rejected with an error telling you the version you are on and to either upgrade or switch back to `json` — `deepeval` never silently falls back, so your results are not quietly written somewhere else.
:::
...
Stored runs can be listed and opened in the [<Term py="deepeval inspect" ts="npx deepeval inspect"/>](/docs/command-line-interface#inspect) TUI with the `--list` and `--run-id <id>` flags, or loaded back in code:
...
The store uses only Python's built-in `sqlite3` module, so there is nothing extra to install and it works the same on macOS, Linux, Windows, and CI.
...
const db = resolveDbPath();                    // honours DEEPEVAL_RESULTS_FOLDER
const runs = listTestRuns(db, 10);             // newest first, summary columns only
const { runId, payload } = loadTestRunPayload(db); // latest run
```

The store uses only Node's built-in `node:sqlite` module, so there is no native addon to compile and it works the same on macOS, Linux, Windows, and CI.
...
`DEEPEVAL_MODE` picks the feature channel. Leave it unset (or set it to `stable`) to get only finalised behaviour. Set it to `experimental` (or run <Term py="deepeval set-mode experimental --save=dotenv" ts="npx deepeval set-mode experimental --save=dotenv"/>) to enrol in the newest deepeval features before they are finalised — anything gated on it may change or break between releases, so keep it off in CI unless you are deliberately testing those features.
...
In code, gate on the channel with `is_experimental()`:

```python
from deepeval.config.mode import is_experimental
...
In code, gate on the channel with `isExperimental()`:

```typescript
import { isExperimental } from "deepeval";
...
The value is case-insensitive. Anything other than `stable` or `experimental` falls back to `stable` with a one-time warning on stderr, so a typo can never silently enrol you. <Term py="deepeval diagnose" ts="npx deepeval diagnose"/> shows the channel currently in effect and where it was set.

#### DeepEval eval mode

`DEEPEVAL_EVAL_MODE` sets the [eval mode](/docs/evaluation-eval-modes), which picks who decides in `deepeval`'s LLM-as-a-judge metrics<Only id="python"> and classifiers</Only>, and is how [Jev](/integrations/models/typesafe-ai), TypeSafe AI's System One model, takes part in an eval:

- `llm` (the default): the LLM runs the whole metric.
- `hybrid`: the LLM extracts and writes the reason; Jev answers the decision points. If a Jev call fails mid-run, the LLM makes that decision instead.
- `system_one`: Jev runs the whole metric in one request over the raw test case and the reason is built from its answers. No LLM is used, so no LLM key is needed.
...
```bash
export DEEPEVAL_EVAL_MODE=system_one
...
Unset, it means `llm`, so existing setups behave as before; the `DEEPEVAL_MODE` feature channel has no effect on it. An <Term py="eval_mode" ts="evalMode"/> argument on a metric overrides it, and <Term py="deepeval set-eval-mode" ts="npx deepeval set-eval-mode"/> sets it for you. Unrecognised values mean unset. G-Eval metrics are LLM-as-a-judge by definition and ignore it.
...
Text-to-speech and speech-to-text are two independent families used by [voice simulation](/docs/multi-turn-conversation-simulation). One `USE_*_TTS` flag and one `USE_*_STT` flag can be on at the same time, and each family falls back to OpenAI when no flag is set. Set them with [`deepeval set-tts`](/docs/command-line-interface#model-provider-configs) and `deepeval set-stt`.
