...
| `USE_ATLASCLOUD_MODEL`             | `1` / `0` / unset | Prefer Atlas Cloud as the default LLM provider (where applicable). |
| `ATLASCLOUD_API_KEY`               | `string` / unset  | Atlas Cloud API key.                                               |
| `ATLASCLOUD_MODEL_NAME`            | `string` / unset  | Optional default model name passed to Atlas Cloud.                 |
| `ATLASCLOUD_BASE_URL`              | `string` / unset  | Optional Atlas Cloud base URL.                                     |
| `ATLASCLOUD_COST_PER_INPUT_TOKEN`  | `float` / unset   | Optional input-token cost used for cost reporting.                 |
| `ATLASCLOUD_COST_PER_OUTPUT_TOKEN` | `float` / unset   | Optional output-token cost used for cost reporting.                |
