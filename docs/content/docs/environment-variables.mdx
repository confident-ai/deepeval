...
| `USE_GEMINI_EMBEDDING`            | `1` / `0` / `unset`      | Prefer Google Gemini embeddings as the default embeddings provider (where applicable). |
| `GEMINI_EMBEDDING_MODEL_NAME`     | `string` / unset | Gemini embedding model name (default `gemini-embedding-001`).                          |
