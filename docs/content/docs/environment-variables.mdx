...
| `DEEPEVAL_SQLITE_PAYLOADS`   | `1` / `0` / unset          | SQLite store only. Also store the full JSON object on every test case, trace and span row (`payload_json`) so unpromoted fields can be queried per row. Off by default: roughly doubles the database size. See [here](/docs/evaluation-local-backend-storage#schema). |
...
By default every finished run is written as JSON. Set `DEEPEVAL_LOCAL_STORE=sqlite` to keep a queryable history of runs in a single SQLite database instead. A full comparison of the two backends, where each file lands, and how to export the database to other platforms is [here](/docs/evaluation-local-backend-storage).
...
runs = list_test_runs(db, limit=10)    # newest first, no payloads
...
const runs = listTestRuns(db, 10);             // newest first, no payloads
