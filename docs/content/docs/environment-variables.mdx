...
| `CONFIDENT_BASE_URL`              | `string` / unset        | Base URL for the Confident AI API server (set only when using a custom/self-hosted endpoint).                                      |
| `CONFIDENT_DISABLE_SSL`           | `1` / `0` / `unset`             | Disable SSL/TLS certificate verification for requests to the Confident AI API server. Only use with self-hosted endpoints that have self-signed certificates. |
...
| Variable                | Values           | Effect                                                         |
| ----------------------- | ---------------- | -------------------------------------------------------------- |
| `USE_ORCAROUTER_MODEL`     | `1` / `0` / `unset`      | Prefer OrcaRouter as the default LLM provider (where applicable). |
| `ORCAROUTER_API_KEY`       | `string` / unset | OrcaRouter API key.                                               |
| `ORCAROUTER_MODEL_NAME`    | `string` / unset | Optional default model name passed to OrcaRouter.                 |
| `ORCAROUTER_BASE_URL`      | `string` / unset | Optional OrcaRouter base URL.                                     |
| `ORCAROUTER_COST_PER_INPUT_TOKEN`  | `float` / unset  | Optional input-token cost used for cost reporting.              |
| `ORCAROUTER_COST_PER_OUTPUT_TOKEN` | `float` / unset  | Optional output-token cost used for cost reporting.              |
