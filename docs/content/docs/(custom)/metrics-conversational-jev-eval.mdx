...
The conversational JevEval is the multi-turn version of [`JevEval`](/docs/metrics-jev-eval): a custom metric that scores an **entire conversation** as the **weighted mean** of the answers to bounded **questions** you write about its turns. Each question is answered by Jev, a System One model, as a calibrated probability rather than generated text, and `deepeval` combines those probabilities into a score with a fixed equation.
...
`ConversationalJevEval` has the same three question types as `JevEval`, drawn directly from Jev's decision primitives. Each maps Jev's probabilities onto a value $v \in [0, 1]$ with the same fixed rule as in `JevEval`:
...
See [Noul](/docs/metrics-jev-eval#noul), [Score](/docs/metrics-jev-eval#score) and [Choice](/docs/metrics-jev-eval#choice) for how to write each one. What changes in the conversational metric is **what the questions are about**:
