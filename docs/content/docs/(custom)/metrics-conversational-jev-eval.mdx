...
There are **THREE** mandatory and **SEVEN** optional parameters when creating a `ConversationalJevEval`:
...
`ConversationalJevEval` needs Jev in every mode; there is no LLM to fall back to. Install the SDK and set your key once:
...
Constructing the metric without them raises immediately with these instructions. See the [TypeSafe AI integration](/integrations/models/typesafe-ai) for the CLI (`deepeval set-typesafe`), model aliases and cost settings. Unlike the rest of the Jev integration, `ConversationalJevEval` does **not** require `DEEPEVAL_MODE=experimental`.

## Question Types

The three question types are the same as in `JevEval`; see [Noul](/docs/metrics-jev-eval#noul), [Score](/docs/metrics-jev-eval#score) and [Choice](/docs/metrics-jev-eval#choice) for the value each one maps onto. What changes in the conversational metric is **what the questions are about**:
...
With `include_reason=True`, the evaluation LLM then receives the turns, the conversation-level fields and each question's outcome **in words** (never the probabilities or the score) and writes a reason that cites turns by position and role. With `include_reason=False`, the metric finishes without any LLM call.
...
With `include_reason=True`, the evaluation LLM now gets the turns, the conversation-level fields and the three questions, each with its outcome **in words** ("clearly holds", `"Partly settled"`, `"ignored_it"`), never the numbers. It is instructed to cover every question, point at the turn by position and role, quote the span that decided it, and never justify anything by a score or a confidence:
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          decisions to Jev. <code>ConversationalJevEval</code> is Jev-native
          and works in stable mode; it only needs <code>typesafe-sdk</code>{" "}
          installed and <code>TYPESAFE_API_KEY</code> set.
        </>
      ),
    },
  ]}
