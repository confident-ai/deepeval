...
You'll also need to supply any additional arguments such as <C id="field::retrieval_context"/> and <C id="field::tools_called"/> in your `turns` if your evaluation criteria depends on these parameters.
...
<C id="field::root_nodes"/> contains the nodes where evaluation begins. A root can be a `ConversationalTaskNode`, `ConversationalBinaryJudgementNode`, or `ConversationalNonBinaryJudgementNode`.
...
Any task or judgement node can focus on a slice of the conversation instead of every turn. Pass a <C id="field::turn_window"/> of two inclusive indices to restrict what the node sees:
...
A DAG has no single exit node. Instead, every path through the graph ends at one of the outcomes you register with <C id="method::add_verdict"/> on a judgement node:
...
A multi-turn DAG uses three node types. Define the nodes first, then connect them with <C id="method::add_node"/> and <C id="method::add_verdict"/>.
...
- <C id="field::output_label"/>: the name used to present this node's output to child nodes.
- [Optional] <C id="field::evaluation_params"/>: a list of `MultiTurnParams` available to the node.
- [Optional] <C id="field::turn_window"/>: two inclusive indices restricting the node to a slice of the conversation.
- [Optional] `label`: a name displayed in verbose logs.

After both nodes are initialized, connect a task to another task or judgement node with <C id="method::add_node"/>. This adds an outgoing edge; it does not define an outcome. A `ConversationalTaskNode` cannot end the graph on its own — only judgement nodes define outcomes.
