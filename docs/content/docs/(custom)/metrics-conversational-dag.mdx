...
<MetricTagsDisplayer jev={true} multiTurn={true} custom={true} />
...
There are **TWO** mandatory and **EIGHT** optional parameters when creating a `ConversationalDAGMetric`:
...
- [Optional] `system_one_model`: the Jev model to use, as a string or a `DeepEvalBaseSystemOneModel`. Only used under `hybrid` or `system_one` `eval_mode`. Defaulted to <DefaultSystemOneModel />.
- [Optional] `eval_mode`: `llm`, `hybrid` or `system_one`, choosing whether an LLM or Jev decides judgement nodes. `system_one` runs as `hybrid`. Defaulted to the configured [eval mode](/docs/evaluation-eval-modes) (`llm` unless set).
...
- [Optional] `systemOneModel`: the Jev model to use, as a string or a `DeepEvalBaseSystemOneModel`. Only used under `hybrid` or `system_one` `evalMode`. Defaulted to <DefaultSystemOneModel />.
- [Optional] `evalMode`: `llm`, `hybrid` or `system_one`, choosing whether an LLM or Jev decides judgement nodes. `system_one` runs as `hybrid`. Defaulted to the configured [eval mode](/docs/evaluation-eval-modes) (`llm` unless set).
...
You can change how the `ConversationalDAGMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), the judgement nodes are decided by Jev, a System One model, instead of the LLM:

- A `ConversationalBinaryJudgementNode` becomes one yes/no question on the node's criteria (`P(yes) >= 0.5` is <Term py="True" ts="true"/>).
- A `ConversationalNonBinaryJudgementNode` becomes one choice over its child verdicts.
- `ConversationalTaskNode` and `ConversationalVerdictNode` stay on the LLM.

A node whose Jev call fails is decided by the LLM instead. You'll need to set `TYPESAFE_API_KEY` and [install the SDK](/integrations/models/typesafe-ai#setting-up-your-api-key).

### Jev-as-a-judge

Task nodes always need the LLM, so under the `system_one` [eval mode](/docs/evaluation-eval-modes) a `ConversationalDAGMetric` runs the same way as under `hybrid`.
  ...
  qas={[
    {
      ...
      answer: (
        <>
          ...
          <a href="/docs/metrics-llm-evals">ConversationalGEval</a>{" "}
