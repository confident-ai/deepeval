...
You'll also need to supply any additional arguments such as <Term py="expected_output" ts="expectedOutput"/> and <Term py="tools_called" ts="toolsCalled"/> if your evaluation criteria depends on these parameters.
...
There are **TWO** mandatory and **EIGHT** optional parameters when creating a `DAGMetric`:
...
<Term py="root_nodes" ts="rootNodes"/> contains the nodes where evaluation begins. A root can be a `TaskNode`, `BinaryJudgementNode`, or `NonBinaryJudgementNode`.
...
Here, the heading-order judgement depends on the extracted headings and the successful <Term py="True" ts="true"/> branch. The graph tracks both incoming dependencies and executes the shared node only when the active path reaches it.
...
A single-turn DAG uses three node types. Define the nodes first, then connect them with <Term py="add_node" ts="addNode"/> and <Term py="add_verdict" ts="addVerdict"/>.
...
There are **TWO** mandatory and **ONE** optional parameters when creating a `TaskNode`:
...
- [Optional] <Term py="evaluation_params" ts="evaluationParams"/>: test-case parameters available to the node.
- [Optional] `label`: a name displayed in verbose logs.

After both nodes are initialized, connect a task to another task or judgement node with <Term py="add_node" ts="addNode"/>. This adds an outgoing edge; it does not define an outcome. A `TaskNode` cannot end the graph on its own — only judgement nodes define outcomes.
...
A `BinaryJudgementNode` evaluates one criterion and returns either <Term py="True" ts="true"/> or <Term py="False" ts="false"/>.
...
- [Optional] <Term py="evaluation_params" ts="evaluationParams"/>: additional test-case parameters available to the node.
- [Optional] `label`: a name displayed in verbose logs.
...
You can change how the `DAGMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
...
- A `BinaryJudgementNode` becomes one yes/no question on the node's criteria (`P(yes) >= 0.5` is <Term py="True" ts="true"/>).
- A `NonBinaryJudgementNode` becomes one choice over its child verdicts.
- `TaskNode` and `VerdictNode` stay on the LLM.
...
Task nodes always need the LLM, so under the `system_one` [eval mode](/docs/evaluation-eval-modes) a `DAGMetric` runs the same way as under `hybrid`.
...
<FAQs
  qas={[
    ...
    },
    {
      ...
      answer: (
        <>
          Yes. Pass a <code>GEval</code> or another <code>BaseMetric</code> as
          the <code>then</code> argument when registering a verdict. It runs
    ...
    },
    {
      ...
      answer: (
        <>
          Enable verbose mode and inspect which judgements and verdicts are
          selected. Repeated scores usually mean a criterion consistently
          selects the same branch or several terminal verdicts use the same
          score.
        </>
      ),
    },
