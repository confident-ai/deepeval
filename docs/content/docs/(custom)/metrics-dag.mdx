...
4. `VerdictNode`s: this node represents one outcome of a judgement node — it either assigns a final score (making it a leaf) or continues evaluation down another path, so every path eventually ends at a score-assigning `VerdictNode`.
