...
```typescript
import { evaluate, LLMTestCase, metrics } from "deepeval";
import { SingleTurnParams } from "deepeval/testCase";

const correctness = new metrics.BinaryJudgementNode({
  criteria: "Is the actual output correct for the input?",
  evaluationParams: [
    SingleTurnParams.INPUT,
    SingleTurnParams.ACTUAL_OUTPUT,
...
const metric = new metrics.DAGMetric({
  name: "Correctness",
  dag: new metrics.DeepAcyclicGraph({ rootNodes: [correctness] }),
});
...
```typescript
// Define the processing and judgement nodes
const extract = new metrics.TaskNode({
  instructions: "Extract every heading.",
  outputLabel: "Extracted headings",
  evaluationParams: [SingleTurnParams.ACTUAL_OUTPUT],
});
const headingsPresent = new metrics.BinaryJudgementNode({
  criteria: "Are all required headings present?",
});
const headingOrder = new metrics.NonBinaryJudgementNode({
  criteria: "Classify the ordering of the headings.",
});
...
// Validate and create the completed DAG
const dag = new metrics.DeepAcyclicGraph({ rootNodes: [extract] });
```
...
Here, the heading-order judgement depends on the extracted headings and the successful `True` branch. The graph tracks both incoming dependencies and executes the shared node only when the active path reaches it.
...
const task = new metrics.TaskNode({
  instructions: "Extract every heading from the actual output.",
  outputLabel: "Extracted headings",
  evaluationParams: [SingleTurnParams.ACTUAL_OUTPUT],
  label: "Heading extraction",
...
const judgement = new metrics.BinaryJudgementNode({
  criteria: "Are all required headings present?",
  evaluationParams: [SingleTurnParams.ACTUAL_OUTPUT],
  label: "Required headings",
});
...
const judgement = new metrics.NonBinaryJudgementNode({
  criteria: "Classify the ordering of the headings.",
  evaluationParams: [SingleTurnParams.ACTUAL_OUTPUT],
  label: "Heading order",
});
...
const subjectiveQuality = new metrics.GEval({
  name: "Writing Quality",
  criteria: "Determine whether the response is clear and well written.",
  evaluationParams: [SingleTurnParams.ACTUAL_OUTPUT],
});
...
```typescript
const policyCheck = new metrics.BinaryJudgementNode({
  criteria: "Does the response violate the refund policy?",
  evaluationParams: [SingleTurnParams.ACTUAL_OUTPUT],
});
...
const dag = new metrics.DeepAcyclicGraph({ rootNodes: [policyCheck] });
```
