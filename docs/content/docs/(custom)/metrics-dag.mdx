...
4. `VerdictNode`s: this node represents one outcome of a judgement node — it either assigns a final score (making it a leaf) or continues evaluation down another path, so every path eventually ends at a score or metric-based `VerdictNode`.
...
- **`TaskNode.add_node(child)`** — attaches a downstream node (another `TaskNode`, `BinaryJudgementNode`, or `NonBinaryJudgementNode` — never a `VerdictNode`). Returns the child. Only `TaskNode`s have `add_node`, since only they fan out to plain downstream nodes.
- **`BinaryJudgementNode.add_verdict(...)` / `NonBinaryJudgementNode.add_verdict(verdict, *, score=None, then=None)`** — attaches one outcome to a judgement node (their children are always verdicts). Provide **either** a `score` (0–10, making it a leaf) **or** `then` (continue evaluation with another node, or a [`GEval`](/docs/metrics-llm-evals)/[`BaseMetric`](/docs/metrics-introduction) to run). Returns the created `VerdictNode`.
...
3. All leaf nodes are `VerdictNode`s, but not every `VerdictNode` is a leaf — a `VerdictNode` created with `then` continues to another node instead of scoring, and every path eventually terminates at a leaf `VerdictNode` (one created with a `score` or passes onto a metric).
...
There are **TWO** mandatory and **THREE** optional parameters when creating a `TaskNode`:
...
- `output_label`: a string representing the final output. Child nodes will use the `output_label` to reference the output from the current `TaskNode`.
- [Optional] `children`: downstream nodes. Prefer wiring these with `add_node(child)` after construction. There **must not** be a `VerdictNode` among them.
...
Attach downstream nodes with **`add_node(child)`**, which returns the child so you can keep a reference:

```python
task = TaskNode(instructions="...", output_label="Sentences", evaluation_params=[SingleTurnParams.ACTUAL_OUTPUT])
judgement = task.add_node(BinaryJudgementNode(criteria="Are there more than 3 sentences?"))
```
...
There is **ONE** mandatory and **THREE** optional parameters when creating a `BinaryJudgementNode`:
...
- [Optional] `children`: exactly two `VerdictNode`s, one with `verdict=True` and one with `verdict=False`. Prefer adding these with `add_verdict(...)`.
- [Optional] `evaluation_params`: a list of type `SingleTurnParams`. Include only the parameters that are relevant for evaluation.
- [Optional] `label`: a string that will be displayed in the verbose logs if `verbose_mode` is `True`.

Add its two outcomes with **`add_verdict(verdict, *, score=None, then=None)`** — one `True` and one `False`:

```python
node = BinaryJudgementNode(criteria="Does the summary contain all three headings?")
node.add_verdict(verdict=False, score=0)               # leaf: score
node.add_verdict(verdict=True, then=correct_order_node)  # continue evaluating or have a score
```

:::tip
If you have a `TaskNode` as a parent node, you can base your `criteria` on its output by referencing the parent's `output_label`. For example, if the parent `TaskNode`'s `output_label` is "Extracted Sentences", you can set the `criteria` as: "Is the number of extracted sentences greater than 3?".
:::
...
There is **ONE** mandatory and **THREE** optional parameters when creating a `NonBinaryJudgementNode`:
...
- [Optional] `children`: `VerdictNode`s whose `verdict` values (unique strings) are the possible outcomes. Prefer adding these with `add_verdict(...)`.
- [Optional] `evaluation_params`: a list of type `SingleTurnParams`. Include only the parameters that are relevant for evaluation.
- [Optional] `label`: a string that will be displayed in the verbose logs if `verbose_mode` is `True`.
...
```python
node = NonBinaryJudgementNode(criteria="Are the headings in the correct order?")
node.add_verdict(verdict="Yes", score=10)
node.add_verdict(verdict="Two are out of order", score=4)
node.add_verdict(verdict="All out of order", score=2)
```

### `VerdictNode`

A `VerdictNode` represents one outcome of a judgement node. You rarely construct it directly — `add_verdict(...)` creates it for you. A `VerdictNode` with a `score` is a leaf that sets the final score; one with a `child` continues evaluation down the graph, so **not all `VerdictNode`s are leaves.**
...
- `verdict`: a string **OR** boolean matching an outcome of the parent judgement node. String if the parent is a `NonBinaryJudgementNode`, boolean if a `BinaryJudgementNode`.
- [Optional] `score`: an integer between 0 - 10 that sets the final `DAGMetric` score when this verdict is taken. Provide this to make the verdict a leaf.
- [Optional] `then` (`child` of this node): a `BaseNode` **OR** any [`BaseMetric`](/docs/metrics-introduction), including [`GEval`](/docs/metrics-llm-evals) metric instances. If the `score` is not provided, the `DAGMetric` will use this provided `child` to run the provided `BaseMetric` instance to calculate a score, **OR** propagate the DAG execution to the `BaseNode` `child`.

:::caution
You must provide `score` **or** a child using `then`, but not both.
:::
  ...
  qas={[
    {
      ...
      answer: (
        <>
          Use <code>DAGMetric</code> when criteria are explicit rules or a
          sequence of decisions and you need the same score every run.{" "}
          <a href="/docs/metrics-llm-evals">GEval</a> is better for fuzzy,
          subjective judgement where one holistic score is fine.
        </>
      ),
    },
    {
      question: "How does a DAG actually reduce LLM-judge variance?",
      answer: (
        <>
          It breaks evaluation into atomic steps: a <code>TaskNode</code>{" "}
          extracts structure, and each <code>BinaryJudgementNode</code> /{" "}
          <code>NonBinaryJudgementNode</code> answers one narrow question. The
          score comes from the deterministic path taken, not the LLM picking a
          number — more decomposition, less room to hallucinate.
        </>
      ),
    },
    {
      question: "Can I run a G-Eval (or other metric) inside a DAG?",
      answer: (
        <>
          Yes. A <code>VerdictNode</code>'s <code>child</code> can be a{" "}
          <code>GEval</code> instance (or any{" "}
          <a href="/docs/metrics-introduction">BaseMetric</a>), letting you gate
          a subjective check behind deterministic branches so it runs only on the
          intended path.
        </>
      ),
    },
    {
      question: "My `DAGMetric` keeps returning the same score — what's wrong?",
      answer: (
        <>
          Check your wiring: each leaf must be a <code>VerdictNode</code> with a
          distinct <code>score</code>. If traversal always lands on one branch,
          a judgement node's <code>criteria</code> is likely too vague — set{" "}
          <code>verbose_mode=True</code> to print each node's verdict.
        </>
      ),
    },
    {
      question: "Do I have to provide both `score` and `child` on a `VerdictNode`?",
      answer: (
        <>
          No — provide <strong>exactly one</strong>. Use <code>score</code> (an
          integer 0-10) for a true leaf, or <code>child</code> to keep traversing
          into another <code>BaseNode</code> or a metric like{" "}
          <code>GEval</code>. Both raises an error.
        </>
      ),
    },
  ]}
