...
```typescript
import { compare, ArenaTestCase, Contestant, LLMTestCase, metrics } from "deepeval";
...
const metric = new metrics.ArenaGEval({
  name: "Friendly",
  criteria: "Choose the winner of the more friendly contestant based on the input and actual output",
  evaluationParams: [
    SingleTurnParams.INPUT,
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          no <code>threshold</code> or <code>strict_mode</code>. Read{" "}
