...
You'll also need to supply any additional arguments such as `actual_output`, `tools_called`, `expected_output` or `retrieval_context` if your questions refer to them.
...
- [Optional] `system_one_model`: a string specifying which System One model to use, **OR** [any custom System One model](/integrations/models/typesafe-ai) of type `DeepEvalBaseSystemOneModel`. Defaulted to <DefaultSystemOneModel />.
