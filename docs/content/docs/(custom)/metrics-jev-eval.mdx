...
<MetricTagsDisplayer jev={true} singleTurn={true} multiTurn={true} custom={true} />
...
There are **THREE** mandatory and **EIGHT** optional parameters when creating a JevEval metric:
...
- [Optional] `include_reason`: a boolean which when set to `True`, has your evaluation LLM write a reason grounded in the test case (for conversations, one that cites specific turns). When `False`, JevEval makes **no LLM call at all**. Defaulted to `True`.
