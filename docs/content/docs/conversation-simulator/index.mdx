...
import { Turn, ConversationSimulator, ConversationalGolden } from "deepeval";
...
// Define chatbot callback
const chatbotCallback = async ({ input }: { input: string }) => {
  return new Turn({ role: "assistant", content: `Chatbot response to: ${input}` });
};

// Run Simulation
const simulator = new ConversationSimulator({ modelCallback: chatbotCallback });
const conversationalTestCases = await simulator.simulate({ conversationalGoldens: [conversationGolden] });
console.log(conversationalTestCases);
```
...
const modelCallback = async ({ input }: { input: string }): Promise<Turn> => {
  return new Turn({ role: "assistant", content: `I don't know how to answer this: ${input}` });
};
...
```typescript
import { Turn, ConversationalGolden } from "deepeval";

const golden = new ConversationalGolden({ scenario: "...", turns: [new Turn({ role: "assistant", content: "Hi! How can I help you today?" })] });
```
