...
<ClassifierTagsDisplayer jev={true} custom={true} singleTurn={true} multiTurn={true} />
...
There are **TWO** mandatory and **SIX** optional parameters when creating a `Classifier`:
...
You can change how a `Classifier` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
