...
The simulator carries `scenario` and <Term py="expected_outcome" ts="expectedOutcome"/> over from the golden, and fills in `turns`:
...
There are **TWO** mandatory and **SIX** optional parameters when calling `evaluate()` for multi-turn end-to-end evaluation:
...
- `metrics`: a list of metrics of type `BaseConversationalMetric`. See the [multi-turn metrics](/docs/metrics-introduction#multi-turn-metrics) for the full list (e.g. `TurnRelevancyMetric`, `KnowledgeRetentionMetric`, `RoleAdherenceMetric`, `ConversationCompletenessMetric`). Can be omitted when `classifiers` is provided.
- [Optional] `classifiers`: a list of [`Classifier`](/docs/classifiers-classifier)s to label each conversation alongside the metrics. See [classifiers](/docs/classifiers-introduction).
...
The same `Classifier` that labels an `LLMTestCase` also labels a `ConversationalTestCase`, reading every turn plus the scenario and expected outcome. Pass `classifiers=` next to your metrics; a classifier only affects a conversation's verdict when `expected_labels` is set on the test case:

```python
from deepeval.classifiers import Classifier
from deepeval import evaluate

resolution = Classifier(name="resolution", labels=["resolved", "unresolved", "handed_over"])

evaluate(
    test_cases=conversational_test_cases,
    metrics=[TurnRelevancyMetric()],
    classifiers=[resolution],
)
```

See the [classifiers introduction](/docs/classifiers-introduction) for how verdicts and expected labels work.
:::
...
<FAQs
  qas={[
    ...
    },
    {
      ...
      answer: (
        <>
          No. You define <code>ConversationalGolden</code>s with a scenario and a
          user persona, then let <code>ConversationSimulator.simulate()</code>{" "}
