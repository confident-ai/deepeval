...
- [Optional] `classifiers`: a list of [`Classifier`](/docs/classifiers-custom)s to label each conversation alongside the metrics. See [classifiers](/docs/classifiers-introduction).
...
The same `Classifier` that labels an `LLMTestCase` also labels a `ConversationalTestCase`, reading every turn plus the scenario and expected outcome. Pass `classifiers=` next to your metrics; a classifier only passes or fails a conversation when `expected_labels` is set on the test case:
...
See the [classifiers introduction](/docs/classifiers-introduction#expected-labels) for how expected labels work.
:::
