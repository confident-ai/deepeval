...
The simulator carries `scenario` and `expected_outcome` over from the golden, flattens its `persona` onto `user_description`, and fills in `turns`:

</Only>

<Only id="typescript">

The simulator carries `scenario`, `expectedOutcome`, and `userDescription` over from the golden, and fills in `turns`:
