...
const metric = new metrics.FaithfulnessMetric({
  threshold: 0.7,
  model: "gpt-4.1",
  includeReason: true,
});
  ...
  qas={[
    {
      ...
      answer: (
        <>
          Generation. It grades <code>actual_output</code> against the
          already-retrieved <code>retrieval_context</code>, so a low score means
    ...
    },
    {
      ...
      answer: (
        <>
          Faithfulness checks the <code>actual_output</code> against the
          retrieved <code>retrieval_context</code> ("is this grounded in what I
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          by claims your <code>retrieval_context</code> doesn't support. The
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <code>retrieval_context</code> — a real-world truth absent from the
          retrieved text still counts as unfaithful. Use{" "}
          <code>truths_extraction_limit</code> to focus on key truths and{" "}
          <code>penalize_ambiguous_claims</code> to control borderline scoring.
        </>
      ),
    },
  ]}
