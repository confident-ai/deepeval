...
const metric = new metrics.ContextualRecallMetric({
  threshold: 0.7,
  model: "gpt-4",
  includeReason: true,
});
  ...
  qas={[
    {
      ...
      answer: (
        <>
          Yes — exactly what it measures. It breaks <code>expected_output</code>{" "}
          into statements and checks how many trace back to a node in{" "}
          <code>retrieval_context</code>. Any statement that can't signals
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <code>retrieval_context</code>. Usual levers: increase top-K, adjust
    ...
    {
      question: "Why does it use `expected_output` instead of `actual_output`?",
      answer: (
        <>
          Recall measures whether the retriever fetched everything needed for
          the ideal answer, so it attributes <code>expected_output</code>{" "}
          (ground truth) to the <code>retrieval_context</code>. Using{" "}
          <code>actual_output</code> would let a bad answer unfairly drag down a
          good retriever.
        </>
      ),
    },
