...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), the `expected_output` is split into sentences in code, and Jev, a System One model, answers one yes/no question per sentence: can it be attributed to the `retrieval_context`? `P(yes) >= 0.5` counts as attributable. The equation and the LLM-written reason are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the raw `expected_output` and `retrieval_context` and asked three questions:
