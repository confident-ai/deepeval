...
There are **TEN** optional parameters when creating a `ContextualRecallMetric`:
...
You can change how the `ContextualRecallMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), the <Term py="expected_output" ts="expectedOutput"/> is split into sentences in code, and Jev, a System One model, answers one yes/no question per sentence: can it be attributed to the <Term py="retrieval_context" ts="retrievalContext"/>? `P(yes) >= 0.5` counts as attributable. The equation and the LLM-written reason are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the raw <Term py="expected_output" ts="expectedOutput"/> and <Term py="retrieval_context" ts="retrievalContext"/> and asked three questions:

| Question                                                                                                      | Type  | Weight |
| ------------------------------------------------------------------------------------------------------------- | ----- | ------ |
| Every sentence in `expected_output` can be attributed to the facts in `retrieval_context`.                    | Noul  | 2      |
| `retrieval_context` contains the key facts needed to write `expected_output`.                                 | Noul  | 1      |
| How much of `expected_output` can be attributed to the facts in `retrieval_context`? (None of it → All of it) | Score | 1      |
