...
When `retrieval_context` contains `RetrievedContextData` entries, contextual precision groups entries with the same `source` before generating LLM verdicts. The grouped node is placed at the first position where that `source` appears, so a duplicate chunk appearing at ranks 2 and 6 is evaluated as one node at rank 2. Plain string entries are not grouped and remain independent nodes.

Choose the `source` granularity based on the retrieval unit you want to score. Use a section or span ID when overlapping chunks from the same section should count as one retrieval unit. Use a chunk-level hash when the same chunk can appear multiple times after hybrid dense and sparse retrieval fusion. Avoid using a source that is too broad, such as an entire file, if unrelated chunks from that file should still be scored separately. Empty sources are treated as independent nodes and emit a warning, while sources shared by most retrieved nodes may warn that grouping granularity is too coarse.
