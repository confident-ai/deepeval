...
There are **TEN** optional parameters when creating a `ContextualRelevancyMetricMetric`:
...
You can change how the `ContextualRelevancyMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), each node in the <Term py="retrieval_context" ts="retrievalContext"/> is split into sentences in code, and Jev, a System One model, answers one yes/no question per statement: is it relevant to the `input`? `P(yes) >= 0.5` counts as relevant. The equation and the LLM-written reason are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the raw `input` and <Term py="retrieval_context" ts="retrievalContext"/> and asked three questions:

| Question                                                                                          | Type  | Weight |
| ------------------------------------------------------------------------------------------------- | ----- | ------ |
| Every statement in `retrieval_context` is relevant to addressing `input`.                         | Noul  | 2      |
| Every node in `retrieval_context` contains at least one statement relevant to addressing `input`. | Noul  | 1      |
| How much of `retrieval_context` is relevant to addressing `input`? (None of it → All of it)       | Score | 1      |
