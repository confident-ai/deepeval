...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), each node in the `retrieval_context` is split into sentences in code, and Jev, a System One model, answers one yes/no question per statement: is it relevant to the `input`? `P(yes) >= 0.5` counts as relevant. The equation and the LLM-written reason are unchanged. If a Jev call fails, the LLM makes that decision instead.

### Jev-as-a-judge

Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the raw `input` and `retrieval_context` and asked three questions:
