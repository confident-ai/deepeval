...
<NotImplemented id="typescript" feature="save_as">

It writes the goldens (and optionally the test cases) in your dataset to a local JSON or CSV file. Until it lands, serialize `dataset.goldens` yourself, or [push the dataset to Confident AI](#save-dataset) where it is versioned for you.
...
`deepeval` offers support for loading datasets stored on Confident AI or in a CSV file into an `EvaluationDataset`. The remaining local file formats are on their way, and each tab below says where things stand.
...
<NotImplemented id="typescript" feature="add_goldens_from_json_file">

It reads an array of objects out of a `.json` file and turns each one into a golden or a test case. Until it lands, `JSON.parse` the file yourself and pass the rows to `new Golden({ ... })`, then hand them to `new EvaluationDataset({ goldens })`.
...
<NotImplemented id="typescript" feature="add_goldens_from_jsonl_file">

It reads a `.jsonl` file one line at a time, mapping each line to a golden. Until it lands, split the file on newlines and `JSON.parse` each line into `new Golden({ ... })` yourself.
...
You can build single-turn `LLMTestCase`s out of a `.csv` file by supplying a `filePath` along with the columns to read them from. Columns such as `context` hold a list of strings, so pass a `contextDelimiter` to tell `deepeval` how to split those cells.
...
dataset.testCases = await dataset.addTestCasesFromCSV({
  // filePath is the absolute path to your .csv file
...
:::note
Since `expectedOutput`, `context`, `retrievalContext`, `toolsCalled`, and `expectedTools` are optional parameters for an `LLMTestCase`, these columns are similarly **optional** when adding test cases from an existing dataset.
:::

<NotImplemented id="typescript" feature="add_goldens_from_csv_file">

It reads the same file into goldens instead of test cases, which is what you want when the `actualOutput` is generated at evaluation time. Until it lands, read the CSV yourself and construct `new Golden({ ... })` per row.
    ...
    },
    {
      ...
      answer: (
        <Switch>
          ...
          <Case id="typescript">
            Partly, for now. You can read test cases straight out of a CSV file
            with <code>addTestCasesFromCSV()</code>; loading goldens from JSON,
            JSONL, or CSV and saving a dataset back to disk are still
            TypeScript-only gaps, so store datasets on Confident AI in the
            meantime.
          </Case>
        </Switch>
      ),
