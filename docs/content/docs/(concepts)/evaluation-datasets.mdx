...
:::caution
`user_description` is deprecated in favor of [`persona`](/docs/conversation-simulator-voice-personas). Goldens and dataset files that still carry `user_description` keep loading — the string is promoted to `Persona(characteristics=...)`, and the two stay in sync — but the parameter will be removed in a future release.
:::
