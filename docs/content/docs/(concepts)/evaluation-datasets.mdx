...
Blank lines are skipped, and a line that doesn't parse fails with its line number. `saveAs({ fileType: "jsonl" })` writes `context` as an array. A `context` written as one delimited string rather than an array is still split on `contextDelimiter`, which defaults to `|`.
...
Remember, parameters such as `context` should be a list of strings. `save_as("csv")` writes those lists as JSON array cells so a `|` inside an item (markdown tables, shell pipelines) survives a reload. Loaders try JSON first and fall back to splitting on `context_col_delimiter` (default `|`) so files written before this change still load. Supply a delimiter only if your file uses something else.
...
Remember, parameters such as `context` hold a list of strings. `saveAs({ fileType: "csv" })` writes those lists as JSON array cells so a `|` inside an item (markdown tables, shell pipelines) survives a reload. Loaders try JSON first and fall back to splitting on `contextDelimiter` (default `|`) so files written before this change still load. Pass a delimiter only if your file uses something else.
...
class Golden(BaseModel):
    ...
    # Classifier name -> expected label, see /docs/classifiers-introduction
...
`expected_labels` is carried from a golden onto the test case built from it, so a dataset can hold the label each golden should receive per [classifier](/docs/classifiers-introduction) in the same way it holds `expected_output` for metrics.
...
:::caution
<Term py="user_description" ts="userDescription"/> is deprecated in favor of [`persona`](/docs/conversation-simulator-voice-personas). Goldens and dataset files that still carry it keep loading — the string is promoted to a persona built from those characteristics, and the two stay in sync — but the parameter will be removed in a future release.
:::
