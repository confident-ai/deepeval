...
```typescript title="main.ts" {4}
import { evaluate, metrics } from "deepeval";
// ...

await evaluate(dataset.testCases, [new metrics.AnswerRelevancyMetric()]);
```
...
```typescript title="main.ts" {9}
import { ConversationalTestCase } from "deepeval";
// ...
  ...
  qas={[
    {
      ...
      answer: (
        <>
          ...
          <code>actual_output</code>, <code>retrieval_context</code>, and{" "}
          <code>tools_called</code> at evaluation time.
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      answer:
        "Yes. You can save and load datasets locally, including importing goldens from JSON and JSONL files, and convert them to test cases when you run evals.",
    },
    {
      question: "Can I add custom columns to a golden?",
      answer: (
        <>
          Yes. Goldens support <code>additional_metadata</code>,{" "}
          <code>comments</code>, and <code>custom_column_key_values</code> for
