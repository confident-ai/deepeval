...
### Edit Or Delete Goldens

You can update or delete individual goldens in a dataset on Confident AI using the `update_golden()` and `delete_golden()` methods. Goldens must first be pulled from Confident AI so they carry the `id` assigned by the platform.

To update a golden, mutate it and pass it to `update_golden()`:
...
golden.expected_output = "An updated expected output"
dataset.update_golden(golden)
```

There is **ONE** mandatory and **ONE** optional parameter when calling the `update_golden()` method:

- `golden`: the `Golden` or `ConversationalGolden` to update, which must carry an `id` from a previous `pull()`.
- [Optional] `finalized`: a boolean which when set to `False`, marks the golden as unfinalized on Confident AI. Defaulted to `True`.

To delete a golden, pass either the golden itself or its id to `delete_golden()`:
...
dataset.delete_golden("your-golden-id")
```

:::tip
Both methods work for multi-turn datasets exactly the same way.
:::
