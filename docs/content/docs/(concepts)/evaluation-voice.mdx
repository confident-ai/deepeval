...
- **What was said** remains the responsibility of existing conversational metrics such as `TurnRelevancyMetric`, `GoalAccuracyMetric`, and `ToolUseMetric`.
- **How it sounded** is evaluated by `VoiceNaturalnessMetric`, `SpeechIntelligibilityMetric`, `VoiceConsistencyMetric`, and `AudioIntegrityMetric`.
- **When it happened** is evaluated by `TurnTakingNaturalnessMetric` and `AgentResponsivenessMetric`.
