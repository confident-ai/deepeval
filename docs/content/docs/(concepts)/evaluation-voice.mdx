...
- **SIP / PSTN (telephony)** — the universal path. Any agent behind a phone number or SIP URI can be called, regardless of vendor: Vapi, Retell, Bland, LiveKit deployments, ElevenLabs agents with phone numbers. `deepeval` has no carrier of its own, so there is no built-in SIP connector: you dial through your own LiveKit SIP trunk by subclassing `LiveKitConnector` (see [Phone numbers](/docs/conversation-simulator-voice-connectors#phone-numbers)).
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          in-house agents are WebSocket; anything with a phone number is SIP,
          reached through your own LiveKit trunk (see{" "}
          <a href="/docs/conversation-simulator-voice-connectors#phone-numbers">
            Phone numbers
          </a>
          ). If you're still building the pipeline, start with{" "}
          <code>CALLBACK</code> — no transport at all.
        </>
      ),
    },
    {
