...
- <Switch><Case id="python">[Optional] `user_description`</Case><Case id="typescript">[Optional] `userDescription`</Case></Switch>
- [Optional] `context`
...
:::note
The `retrieval_context` parameter accepts a list of strings or `RetrievedContextData` objects.
:::
...
Use `source` to identify the retrieval unit that the context belongs to. Contextual precision metrics group `RetrievedContextData` entries with the same `source`, so you can use a section ID for section-level evaluation or a chunk hash when the same chunk can appear multiple times after hybrid search fusion. Plain string entries remain independent retrieval nodes. Empty sources are treated as independent nodes and will emit a warning, while overly broad sources may warn that grouping granularity is too coarse.
...
:::tip
If you are [simulating conversations](/docs/conversation-simulator), describe the user on the `ConversationalGolden` instead, using a [`Persona`](/docs/conversation-simulator-voice-personas) — it carries the same profile plus voice, background noise, and interruption behavior. The simulator flattens it onto the `user_description` of each test case it produces.
:::
...
const testCase = new ConversationalTestCase({
  userDescription: "John Smith, lives in NYC, has a dog, divorced.",
  turns: [new Turn({ role: "user", content: "..." })],
});
```
  ...
  qas={[
    {
      ...
      answer: (
        <>
          A <code>ConversationalTestCase</code> represents an entire multi-turn
          conversation through a list of <code>Turn</code>s, while an{" "}
          <a href="/docs/evaluation-test-cases">
            <code>LLMTestCase</code>
          </a>{" "}
          represents a single atomic interaction. Use the conversational one for
          chatbots and assistants where context spans multiple turns.
        </>
      ),
    },
    {
      question: "What does a `Turn` contain?",
      answer: (
        <>
          Each <code>Turn</code> has a <code>role</code> (<code>user</code> or{" "}
          <code>assistant</code>) and <code>content</code>, and can optionally
          carry retrieval context, tools called, and MCP primitives so
          per-turn behavior can be evaluated.
        </>
      ),
    },
    {
      question: "What are `scenario` and `expected_outcome` for?",
      answer: (
        <>
          They describe the conversation at a high level: <code>scenario</code>{" "}
          sets up the situation the user is in, and{" "}
          <code>expected_outcome</code> describes what a successful conversation
          should achieve. They're especially useful for simulating turns and
          for outcome-based conversational metrics.
        </>
      ),
    },
    {
      question: "Do I have to write every turn by hand?",
      answer: (
        <>
          No. You can author turns manually, or generate them automatically with
          the{" "}
          <a href="/docs/conversation-simulator">conversation simulator</a> from
          a <code>ConversationalGolden</code>'s scenario and user description.
        </>
      ),
    },
    {
      question: "Can multi-turn test cases include images?",
      answer: (
        <>
          Yes. You can embed <code>MLLMImage</code> objects inside a turn's
          content to evaluate multimodal conversations.
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          ...
          transcripts in a terminal doesn't scale. Logging into{" "}
          <a href="https://www.confident-ai.com">Confident AI</a> (the platform
          from the <code>deepeval</code> team) renders the same conversations
          and their
          per-turn scores into a shared cloud UI, so a team can review,
          annotate, and track them over time — with no change to your test case
          code. It's entirely optional.
        </>
      ),
    },
  ]}
