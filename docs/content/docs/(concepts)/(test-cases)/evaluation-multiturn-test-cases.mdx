...
- <Only id="python">[Optional] `expected_labels`</Only>
...
The `turns` parameter is a list of `Turn`s and is basically a list of messages/exchanges in a user-LLM conversation. If you're using [`ConversationalGEval`](/docs/metrics-llm-evals), you might also want to supply different parameters to a `Turn`. A `Turn` is made up of the following parameters:
