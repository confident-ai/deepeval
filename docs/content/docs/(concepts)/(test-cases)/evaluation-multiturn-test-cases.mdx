...
:::caution
`user_description` is deprecated on `ConversationalGolden` in favor of [`persona`](/docs/conversation-simulator-voice-personas), which carries the same profile plus voice, background noise, and interruption behavior. Goldens that still set `user_description` keep working — the string is promoted to `Persona(characteristics=...)` — but the parameter will be removed in a future release.

It is **not** deprecated here: `user_description` remains the field on `ConversationalTestCase`, and is what the simulator flattens a golden's persona onto for every test case it produces.
:::
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          a <code>ConversationalGolden</code>'s scenario and user persona.
        </>
      ),
    },
    {
