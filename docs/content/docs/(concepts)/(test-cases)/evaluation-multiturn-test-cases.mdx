...
For **voice conversations**, a `Turn` carries two additional fields that record what was actually spoken and when:
...
The `audio` parameter holds the spoken audio for that turn as an `Audio` object, and `latency_ms` records the assistant's response time. A `ConversationalTestCase` whose turns carry audio has its `voice` flag set to `True` automatically. See [Voice](/docs/evaluation-voice) in concepts for what these fields mean and the `Audio` data model.
