...
The `expected_labels` parameter is an **optional** `Dict[str, str]` that maps a [classifier](/docs/classifiers-introduction)'s `name` to the label this conversation should receive. A classifier only produces a pass/fail verdict for a test case when its name appears here; otherwise it still labels the conversation but leaves the verdict untouched. Metrics never read this parameter.
