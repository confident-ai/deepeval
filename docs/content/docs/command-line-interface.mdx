...
| `--official`, `-o`               | Mark this test run as the official baseline on Confident AI after it completes.                                                                    |
| `--mode`                         | Test run mode. Use `regression` to compare against the latest official run on Confident AI and exit non-zero on regression. Defaults to `default`. |
...
Use `deepeval diagnose` to print the configuration your environment actually resolves to — and where each value comes from. Reach for it when evals are using the wrong model, credentials, or data region, or when you're not sure which `.env` file is winning.
...
:::tip
`deepeval diagnose --json` is handy in CI or bug reports — it's safe to share since all secrets are masked.
:::
