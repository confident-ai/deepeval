...
There are **FIFTEEN** optional parameters when creating a `ContextConstructionConfig`:
...
- [Optional] `allow_cross_file_contexts`: a boolean that specifies if context construction is allowed to be sourced from different files. Defaulted to `False`.
- [Optional] `target_files_per_context`: an integer specifying the exact number of files to be used per context. It cannot be less than 2. Defaulted to `None`.
- [Optional] `max_files_per_context`: an integer specifying the maximum number of files that can be used per context. It cannot be less than 2. Defaulted to `3`.
...
<FAQs
  qas={[
    {
      question: "What document types does `generate_goldens_from_docs` support?",
      answer: (
        <>
          <code>.txt</code>, <code>.docx</code>, <code>.pdf</code>,{" "}
          <code>.md</code>, <code>.markdown</code>, and <code>.mdx</code> files,
          passed as a list to the <code>document_paths</code> parameter.
        </>
      ),
    },
    {
      question: "How are contexts constructed from my documents?",
      answer: (
        <>
          <code>deepeval</code> parses documents into token-based chunks (via{" "}
          <code>chunk_size</code> and <code>chunk_overlap</code>), selects and
          quality-filters chunks, then groups semantically similar chunks by
          cosine similarity into contexts. Tune this with a{" "}
          <code>ContextConstructionConfig</code>.
        </>
      ),
    },
    {
      question: "When should I use this instead of `generate_goldens_from_contexts`?",
      answer: (
        <>
          Use <code>generate_goldens_from_docs</code> when you have raw documents
          and want <code>deepeval</code> to extract contexts for you. If you
          already control chunking, use{" "}
    ...
    {
      question: "Can my team generate goldens from our documents without code?",
      answer: (
        <>
