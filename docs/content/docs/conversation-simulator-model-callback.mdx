...
const modelCallback = async ({ input }: { input: string }): Promise<Turn> => {
  const response = await yourLlmApp(input);
...
const modelCallback = async ({ input, turns }: { input: string; turns: Turn[] }): Promise<Turn> => {
...
const modelCallback = async ({ input, turns, threadId }: { input: string; turns: Turn[]; threadId: string }): Promise<Turn> => {
  const res = await yourLlmApp({ input, turns, threadId });
  return new Turn({ role: "assistant", content: res });
};
```
