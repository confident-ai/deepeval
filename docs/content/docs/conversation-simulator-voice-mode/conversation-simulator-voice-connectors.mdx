...
There are **ONE** mandatory and **TWO** optional parameters when creating an `ElevenLabsConnector`:
...
Joins a LiveKit room as a participant and exchanges audio with your agent over WebRTC. Requires the `livekit` extra:
...
- [Optional] `end_of_turn_silence_ms`: silence window (ms) used to detect the end of an agent reply. Defaulted to `800`.
- [Optional] `max_turn_timeout_s`: hard cap (seconds) on waiting for an agent reply. Defaulted to `30.0`.
...
There are **ONE** mandatory and **ELEVEN** optional parameters when creating a `WebSocketConnector`:
...
There are **ONE** mandatory and **TWO** optional parameters when creating a `CallbackVoiceConnector`:
