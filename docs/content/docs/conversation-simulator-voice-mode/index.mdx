...
On a **full-duplex** connector such as `LiveKitConnector` or `WebRTCConnector`, the caller keeps listening while it speaks, so nothing the agent says mid-turn is lost; on connectors that can't carry both voices at once, such as `CallbackVoiceConnector`, each side takes a turn. Neither makes the caller interrupt — for barge-in, see [Interruptions](/docs/conversation-simulator-voice-interruptions).
...
:::info
`combine_audio_files` stitches the clips attached to each `Turn`, so it contains exactly what the metrics see. `record_call` tapes every frame that crossed the wire — including agent speech that never became a turn and only the part of a barge the caller actually sent — with the caller on the left channel and your agent on the right.
:::
...
- When `record_call=True`, `call_recording_path` points to a stereo WAV of the entire call — caller on the left channel, your agent on the right.
...
<FAQs
  qas={[
    ...
    },
    {
      ...
      answer: (
        <>
          Yes. Every user turn counts against{" "}
          <code>max_user_simulations</code>, including barge turns, so an
          interrupting persona reaches the limit in fewer exchanges.
        </>
      ),
    },
    {
