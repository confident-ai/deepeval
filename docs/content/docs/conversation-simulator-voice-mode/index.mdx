...
On a **full-duplex** connector such as `LiveKitConnector`, the caller keeps listening while it speaks, so nothing the agent says mid-turn is lost; on connectors that can't carry both voices at once, such as `CallbackVoiceConnector`, each side takes a turn. Neither makes the caller interrupt — for barge-in, see [Interruptions](/docs/conversation-simulator-voice-interruptions).
