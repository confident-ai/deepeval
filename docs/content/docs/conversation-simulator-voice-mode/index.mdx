...
The diagram shows the loop as a **speak-then-listen** exchange (`exchange_turn`): the caller finishes speaking, then the connector waits for the reply. Which loop actually runs depends on the connector:

- On a **full-duplex** connector — one that can carry both voices at once, such as `LiveKitConnector` for LiveKit rooms and SIP calls — the caller always listens while it speaks, whether or not the persona interrupts. Anything the agent says while the caller is talking or thinking is heard and recorded; nothing is lost between turns.
- On a connector that **cannot** carry both voices at once, such as `CallbackVoiceConnector`, the speak-then-listen exchange above is used instead.

Neither loop makes the caller interrupt. To exercise barge-in, give the persona an `interruption_behavior` — see [Interruptions](/docs/conversation-simulator-voice-interruptions).
...
There are **ONE** mandatory and **FIVE** optional parameters when creating a `VoiceConfig`:
...
- [Optional] `record_call`: a boolean which when set to `True`, records the whole call as it happened — both sides, in real time — and exposes it as [`call_recording_path`](#what-a-voice-simulation-produces) on each `ConversationalTestCase`. Defaulted to `False`.
...
- When `record_call=True`, `call_recording_path` points to a stereo WAV of the **entire call** — the caller on the left channel, your agent on the right. Each side is placed by wall-clock time, so the overlaps and silences you hear are the ones that actually happened, not a stitch of per-turn clips. It is `None` when recording is off or nothing was heard.
...
`deepeval-conversation.wav` and `deepeval-call-recording.wav` answer different questions. The first is stitched from the turn clips and is the cleanest way to hear what was said. The second is the tape of the call: if a metric flags an awkward pause or double-talk, this is the file to listen to.

Every conversation also ends with one `INFO` log line from the `deepeval.simulator` logger summarizing reply times on both sides:

```text
Conversation 0 voice latency: agent replied after avg 1.42s, max 2.87s over 6 turns; caller replied after avg 0.91s, max 1.30s over 6 turns
```

The agent's figure is your agent's `latency_ms` averaged across the conversation. The caller's figure is the simulator's own reply time — TTS synthesis plus turn generation — and is worth reading alongside it: a slow caller stretches the whole call and makes the agent look more patient than it is. This log line is the quickest way to read latency without opening every `Turn`.
...
<FAQs
  qas={[
    ...
    {
      question: "Do interruptions count toward the turn limit?",
      answer: (
        <>
          Yes. Every user turn the simulated caller produces counts against{" "}
          <code>max_user_simulations</code>, including barge turns made by an
          interrupting persona. A caller who cuts in three times spends three
          turns of the budget doing it, so a{" "}
          <code>&quot;frequent&quot;</code> interrupter may reach the limit in
          fewer exchanges than a polite one.
        </>
      ),
    },
    {
