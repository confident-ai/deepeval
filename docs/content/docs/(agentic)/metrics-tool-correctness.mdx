...
```typescript
import { evaluate, LLMTestCase, ToolCall, metrics } from "deepeval";
...
const metric = new metrics.ToolCorrectnessMetric();
...
```typescript
import { evaluate, LLMTestCase, ToolCall, metrics } from "deepeval";
import { MLLMImage } from "deepeval/testCase";
  ...
  qas={[
    {
      ...
      answer: (
        <>
          Mostly not. The core score is deterministic — <code>tools_called</code>{" "}
          vs <code>expected_tools</code>, counting matches. An LLM is only used
          if you pass <code>available_tools</code> to judge optimality, and the
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <code>ToolCallParams.OUTPUT</code> to <code>evaluation_params</code> to
          also match inputs/outputs. Use{" "}
          <code>should_consider_ordering=True</code> for call order, or{" "}
          <code>should_exact_match=True</code> for identical lists (overrides
          ordering).
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          ...
          deterministically against <code>expected_tools</code>.{" "}
    ...
    },
    {
      ...
      answer: (
        <>
          Yes — pass <code>available_tools</code> and an LLM judges whether the{" "}
          <code>tools_called</code> were optimal. Without it, the metric only
          checks calls against <code>expected_tools</code> deterministically.
        </>
      ),
    },
    {
