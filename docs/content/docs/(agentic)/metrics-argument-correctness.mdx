...
const metric = new metrics.ArgumentCorrectnessMetric({
  threshold: 0.7,
  model: "gpt-4",
  includeReason: true,
});
...
const metric = new metrics.ArgumentCorrectnessMetric({ verboseMode: true });
await metric.measure(testCase);
```
...
<FAQs
  qas={[
    ...
    {
      question: "Do I need `expected_tools` for Argument Correctness?",
      answer: (
        <>
          No — it's referenceless. Provide only <code>input</code>,{" "}
          <code>actual_output</code>, and <code>tools_called</code> on the{" "}
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          "right tools?" deterministically against <code>expected_tools</code>;
