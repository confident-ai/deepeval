...
Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the raw `input` and the structured `tools_called` and asked three questions:
