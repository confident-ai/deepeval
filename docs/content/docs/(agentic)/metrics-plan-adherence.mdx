...
There are **NINE** optional parameters when creating a `PlanAdherenceMetric`:
...
You can change how the `PlanAdherenceMetric` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
...
Under the `hybrid` [eval mode](/docs/evaluation-eval-modes), the LLM still extracts the task and plan, but the adherence score is given by Jev, a System One model, as a five-level rating mapped onto 0 to 1. The reason states Jev's score and confidence. If a Jev call fails, the LLM makes that decision instead.
