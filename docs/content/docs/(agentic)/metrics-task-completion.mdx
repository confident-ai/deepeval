...
for await (const golden of dataset.evalsIterator({ metrics: [taskCompletion] })) {
  ...
  await tripPlannerAgent(golden.input);
}
```
  ...
  qas={[
    {
      ...
      answer: (
        <>
          Inferred by default. <code>task</code> defaults to <code>None</code>{" "}
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          outputs to label. Set <code>include_reason=True</code> for the judge's
          explanation.
        </>
      ),
    },
