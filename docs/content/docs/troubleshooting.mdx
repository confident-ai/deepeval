...
`CONFIDENT_DISABLE_SSL=1` turns off certificate verification for requests from `deepeval` to the Confident AI API. Only use it as a temporary workaround for custom or self-hosted Confident AI endpoints where you control the network and understand why certificate verification is failing.
...
- That endpoint uses a self-signed certificate, private CA, or certificate chain that is not trusted by your Python environment.
- You have confirmed the failure is certificate verification related, for example with `SSLCertVerificationError` or `CERTIFICATE_VERIFY_FAILED`.
