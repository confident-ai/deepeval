...
<ClassifierTagsDisplayer jev={true} singleTurn={true} multiTurn={true} rag={true} />
...
There are **SEVEN** optional parameters when creating a `AbstentionClassifier`:
...
You can change how the `AbstentionClassifier` is calculated by setting the [eval mode](/docs/evaluation-eval-modes).
