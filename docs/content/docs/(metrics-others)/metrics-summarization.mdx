...
const metric = new metrics.SummarizationMetric({
  threshold: 0.5,
  model: "gpt-4",
  assessmentQuestions: [
    "Is the coverage score based on a percentage of 'yes' answers?",
...
const testCase = new LLMTestCase({ input: "...", actualOutput: "..." });
const metric = new metrics.SummarizationMetric({ threshold: 0.5 });
  ...
  qas={[
    {
      ...
      answer: (
        <>
          The score is{" "}
          <code>min(alignment_score, coverage_score)</code>, so an accurate
          summary that drops key details is capped by a low{" "}
          <code>coverage_score</code>. Check <code>metric.score_breakdown</code>{" "}
          to see which half is dragging it down.
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          Two separate checks. <code>alignment_score</code> flags summary claims
          that contradict or invent detail (hallucination);{" "}
          <code>coverage_score</code> uses yes/no questions to catch key points
          left out (omission).
        </>
      ),
    },
    {
      question:
        "When should I supply `assessment_questions` instead of letting deepeval generate them?",
      answer: (
        <>
          Supply <code>assessment_questions</code> when you know what a good
          summary must answer — they pin <code>coverage_score</code> to the
    ...
    },
    {
      ...
      answer: (
        <>
          Set <code>truths_extraction_limit</code> to cap how many factual
          ...
          details. Defaults to <code>None</code>.
        </>
      ),
    },
  ]}
