...
const taskCompletionMetric = new metrics.MCPUseMetric({ model: "gpt-4.1" });
```
...
```typescript title="main.ts" showLineNumbers {3,15-21}
...
```typescript title="main.ts" showLineNumbers {1,21-27}
...
const mcpUseMetric = new metrics.MCPUseMetric();
```
...
```typescript title="main.ts" {7,13}
import { MCPToolCall, Turn } from "deepeval/testCase";
...
const mcpUseMetric = new metrics.MultiTurnMCPUseMetric();
const mcpTaskCompletion = new metrics.MCPTaskCompletionMetric();
```
