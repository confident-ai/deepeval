...
```python title="main.py" showLineNumbers {8,14-20,27-31}
import os
import httpx
...
async def main():
    async with AsyncExitStack() as stack:
        ...
        # Run the host and track its interactions before leaving this block.
```
...
The Python sample sends bearer credentials in an authorization header. Never
include API keys in the server URL. The exit stack keeps the connection open
while the host runs and closes every transport afterward.
...
Set `actual_output` to the final model response after all MCP tool results have
returned. The first response can contain only tool calls and is not a complete
answer.

<Only id="python">

The [complete Xquik example](https://github.com/confident-ai/deepeval/blob/main/examples/mcp_evaluation/mcp_eval_single_turn.py)
applies this pattern to public X search. It includes a bounded tool loop,
secure bearer authentication, connection cleanup, and test-case creation.
