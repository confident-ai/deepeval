...
await evaluate(testCases, [
  ...
  new metrics.FaithfulnessMetric(),
]);
```
...
const generator = observe({
  ...
  fn: async (query: string, chunks: string[]) => {
    ...
    updateCurrentSpan({
      testCase: new LLMTestCase({ input: query, actualOutput: answer }),
    });
    return answer;
  },
});
...
await evaluate(testCases, [
  new metrics.TurnFaithfulnessMetric(),
  new metrics.TurnContextualRelevancyMetric(),
]);
```
