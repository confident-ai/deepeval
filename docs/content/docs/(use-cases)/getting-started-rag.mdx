...
```typescript
import { Golden, EvaluationDataset } from "deepeval";

const goldens = [
  new Golden({ input: "How do I reset my password?" }),
  new Golden({ input: "What's your refund policy?" }),
  // ...
];
...
```typescript title="test_rag.ts" showLineNumbers
import { evaluate, LLMTestCase, metrics } from "deepeval";
import { ragPipeline } from "./your_app"; // returns [answer, retrievedChunks]
...
await evaluate(testCases, [new metrics.AnswerRelevancyMetric(), new metrics.FaithfulnessMetric()]);
```
...
const ragPipeline = observe({
  fn: async (query: string) => {
    ...
    updateCurrentTrace({ input: query, output: answer, retrievalContext: chunks });
    return answer;
  },
});

for await (const golden of dataset.evalsIterator({
  metrics: [new metrics.AnswerRelevancyMetric(), new metrics.FaithfulnessMetric()],
})) {
...
```typescript title="main.ts" showLineNumbers
import { EvaluationDataset, Golden, LLMTestCase, metrics } from "deepeval";
...
const retriever = observe({
  ...
  fn: async (query: string) => {
    ...
    updateCurrentSpan({ testCase: new LLMTestCase({ input: query, retrievalContext: chunks }) });
    return chunks;
  },
});
...
const generator = observe({
  ...
  fn: async (query: string, chunks: string[]) => {
    const answer = "..."; // your LLM call here
    updateCurrentSpan({ testCase: new LLMTestCase({ input: query, actualOutput: answer }) });
    return answer;
  },
});
...
const dataset = new EvaluationDataset({ goldens: [new Golden({ input: "How do I reset my password?" })] });
...
const dataset = new EvaluationDataset({
  goldens: [
    new ConversationalGolden({
      scenario: "User can't log in and wants to reset their password.",
      expectedOutcome: "Receives clear reset steps grounded in the help docs.",
    }),
  ],
});
```
...
```typescript title="main.ts" showLineNumbers
import { Turn, ConversationSimulator } from "deepeval";

const modelCallback = async ({ input, turns, threadId }) => {
  // Replace with your RAG chatbot; returns [answer, retrievedChunks]
  const [answer, retrievedChunks] = await yourRagChatbot(input, turns, threadId);
  return new Turn({ role: "assistant", content: answer, retrievalContext: retrievedChunks });
};
...
await evaluate(testCases, [new metrics.TurnFaithfulnessMetric(), new metrics.TurnContextualRelevancyMetric()]);
```
