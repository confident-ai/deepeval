...
const taskCompletionMetric = new metrics.TurnRelevancyMetric({ model: "gpt-4.1" });
```
...
const testCase = new ConversationalTestCase({
  turns: [
    ...
    new Turn({ role: "assistant", content: "I'd like to buy a ticket to a Coldplay concert." }),
  ],
});
```
...
await evaluate([testCase], [new metrics.TurnRelevancyMetric(), new metrics.KnowledgeRetentionMetric()]);
```
...
const dataset = new EvaluationDataset({
  goldens: [
    ...
    new ConversationalGolden({ scenario: "Couple booking two VIP Coldplay tickets" }),
  ],
});
```
...
```typescript title=main.ts showLineNumbers={true} {6}
import { Turn } from "deepeval";
import OpenAI from "openai";

const client = new OpenAI();

const modelCallback = async ({ input, turns }) => {
  const messages = [
    { role: "system", content: "You are a ticket purchasing assistant" },
  ...
  return new Turn({ role: "assistant", content: response.choices[0].message.content });
};
```
...
await evaluate(conversationalTestCases, [new metrics.TurnRelevancyMetric()]);
```
