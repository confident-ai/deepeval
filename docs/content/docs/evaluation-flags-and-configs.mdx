...
The `--pass-rate` flag sets the minimum fraction of `assert_test()` cases that must pass (e.g., `0.8` for 80%). On GitHub Actions, the job fails if the overall pass rate falls below this value. Cases from `evaluate()` or `evals_iterator()` are not counted.
...
The `--required-metrics` flag accepts a comma-separated list of metric **display names** (from each metric's `__name__` property) that must pass on every `assert_test()` case. Enforced on GitHub Actions alongside `--pass-rate`.

```
deepeval test run test_example.py --required-metrics "Answer Relevancy, Contextual Precision"
```
...
env:
  GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
run: poetry run deepeval test run test_example.py --pass-rate 0.8 --required-metrics "Answer Relevancy"
```
