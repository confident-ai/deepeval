...
The `--required-metrics` flag accepts a comma-separated list of metric names that must pass on every test case. Enforced on GitHub Actions alongside `--pass-rate`.

```
deepeval test run test_example.py --required-metrics Correctness,Answer Relevancy
```
...
env:
  GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
run: poetry run deepeval test run test_example.py --pass-rate 0.8 --required-metrics Correctness
```
