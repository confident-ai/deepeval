...
There is at least **ONE** required and **FIVE** optional parameters when creating a `ToolPermissionMetric`:
...
- [Optional] `threshold`: a float representing the minimum passing threshold. Can also be set to `None` to run the metric in [score-only mode](/docs/metrics-introduction#metric-thresholds). Defaulted to `1.0` (any unauthorized call fails).
