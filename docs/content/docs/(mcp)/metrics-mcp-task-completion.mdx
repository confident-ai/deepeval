...
import { Turn, ConversationalTestCase, MCPServer } from "deepeval/testCase";
...
const metric = new metrics.MCPTaskCompletionMetric({ threshold: 0.5 });
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <code>metric.reason</code>. Keep <code>include_reason</code> set to{" "}
          <code>True</code> (default) to see which interaction failed.
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          ...
          with <code>turns</code> and <code>mcp_servers</code>, plus{" "}
          <code>mcp_tools_called</code>, <code>mcp_resources_called</code>, and{" "}
          <code>mcp_prompts_called</code> inside the relevant turns.
        </>
      ),
    },
  ]}
