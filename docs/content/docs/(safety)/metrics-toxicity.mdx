...
const metric = new metrics.ToxicityMetric({ threshold: 0.5 });
...
- [Optional] `threshold`: a float representing the maximum passing threshold. Can also be set to `None` to run the metric in [score-only mode](/docs/metrics-introduction#metric-thresholds). Defaulted to `0.5`.
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <code>actual_output</code>, and a quoted or cited statement isn't the
          model's own opinion (see the{" "}
          <a href="/docs/metrics-bias#definition-of-opinion">
            definition of opinion
          </a>
          ). Enable <code>verbose_mode</code> to confirm each classification.
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          ...
          <code>include_reason</code> to see <em>why</em> something was marked
          toxic.
        </>
      ),
    },
