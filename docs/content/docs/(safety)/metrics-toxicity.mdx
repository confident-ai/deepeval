...
Under the `system_one` [eval mode](/docs/evaluation-eval-modes), Jev judges the whole metric in one request. It is sent the raw `actual_output` and asked two questions, phrased so that a higher probability means less toxicity:
