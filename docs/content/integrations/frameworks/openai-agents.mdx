...
```bash
npm install deepeval @openai/agents zod
```
...
```typescript title="openai_agents_app.ts" showLineNumbers
import { Agent, run, tool, addTraceProcessor } from "@openai/agents";
import { z } from "zod";
