...
- **Evaluate the complete ordered [agent trajectory](/docs/evaluation-trajectory-based-llm-evals)** across its agent, LLM, tool, and retriever spans.
...
Trajectory evaluation scores the complete ordered trace: every agent decision, LLM call, tool interaction, and retrieval step in sequence across the applicable spans. This is distinct from output-only evaluation, which scores only the final response, and individual component evaluation, which scores one span at a time.
