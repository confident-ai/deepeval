  ...
  qas={[
    {
      ...
      answer: (
        <>
          ...
          <code>with next_llm_span(metrics=[...])</code> and the{" "}
          <code>CallbackHandler</code> drains the metric onto the span that
          sub-agent node emits — scoring it on its own. It's one-shot per run,
          so for every loop turn either drive the loop yourself or score
