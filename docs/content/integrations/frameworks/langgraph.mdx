...
Use `EvaluationDataset` + `evals_iterator(...)`. Each `Golden` becomes one LangGraph run, and metrics passed to the iterator score the resulting trace end-to-end.
...
Passing `metrics=[...]` to `evals_iterator` evaluates the overall LangGraph run. To evaluate a component instead, attach metrics where the graph creates that component.

### Agent spans

Wrap the `graph.invoke(...)` in `with next_agent_span(metrics=[...]):`. The `CallbackHandler` drains the staged metric onto the **root agent span** opened by `invoke(...)` — useful when you want a span-level score on the graph run itself rather than the whole trace.
...
The same one-shot semantic as `next_llm_span` applies: only the root agent span picks up the staged metric.
...
`next_llm_span` stages a metric for the **first** LLM span the graph emits inside the `with` block. Later loop iterations through the `chatbot` node won't pick it up. To score every LLM call, drive the loop yourself (`next_llm_span` per `graph.invoke(...)`) or score the run end-to-end with trace-level metrics on `evals_iterator(metrics=[...])`.
:::
...
| `metrics`           | `list`      | Trace-level metrics for the run. Prefer `evals_iterator(metrics=[...])` in eval scripts. |
