...
const retrieveDocs = observe({
  type: "retriever",
  fn: async (query: string) => ["Paris is the capital of France."],
});

const respondToUser = observe({
  fn: async (prompt: string) => {
    const docs = await retrieveDocs(prompt);
    updateCurrentTrace({ userId: "user-123", tags: ["openai", "rag"] });
...
for await (const golden of dataset.evalsIterator()) {
  if (!("input" in golden)) continue;
  await respondToUser(golden.input);
}
```
...
const planThenRespond = observe({
  name: "plan_then_respond",
  fn: async (prompt: string) => {
