...
- **Evaluate the complete ordered [generation/tool-loop trajectory](/docs/evaluation-trajectory-based-llm-evals)** across all applicable LLM, tool, and retriever spans.
...
| AI SDK package | Compatible `deepeval` versions |
| --- | --- |
| `ai==6.0.97`; `@ai-sdk/openai==3.0.30` | `0.9.11` |
...
For AI SDK, trajectory evaluation scores the complete ordered generation/tool-loop trace: the root generation and every applicable model, tool, and retriever span in execution order. There is no separate agent span. This differs from output-only evaluation, which judges only the final response, and component evaluation, which scores one span without considering the full tool loop.

Pass trajectory metrics to `evalsIterator(...)`, then call the instrumented `ask(...)` function from above once per golden. Its `experimental_telemetry` configuration keeps the generation and tool loop in one trace.
