...
To use Anthropic models for `deepeval` metrics, define an `AnthropicModel` and specify the model you want to use. By default, the `model` is set to `claude-sonnet-4-6`.
...
Every option is backed by an environment variable, so you can set `ANTHROPIC_MODEL_NAME` and `ANTHROPIC_API_KEY` in your `env` and construct the model with no arguments:
...
There are **ZERO** mandatory and **SEVEN** optional parameters when creating an `AnthropicModel`:

- [Optional] `model`: A string specifying which Claude model to use. Defaults to `ANTHROPIC_MODEL_NAME` if not passed; falls back to `claude-sonnet-4-6` if unset.
- [Optional] `apiKey`: A string specifying your Anthropic API key. Defaults to `ANTHROPIC_API_KEY` if not passed; raises an error at runtime if unset.
...
- [Optional] `maxTokens`: A number specifying the maximum number of tokens to generate. Defaults to `4096`.
...
- [Optional] `generationKwargs`: An object of additional generation parameters forwarded to the Anthropic `messages.create(...)` call.

:::tip
Pass generation parameters that have no first-class option, such as `top_p`, via `generationKwargs` (they are forwarded to `messages.create(...)`).
:::
