...
Any other key you pass is forwarded as-is to the Anthropic `messages.create(...)` call, so generation parameters with no first-class option go inline alongside the rest:
