...
:::info
The CLI command above sets Azure OpenAI as the default provider for all metrics, unless overridden in code. To use a different default model provider, you must first unset Azure OpenAI:
...
import { AzureOpenAIModel } from "deepeval/models";
...
`deepeval` autoloads `.env.local` then `.env` at import time, so you can keep your Azure OpenAI configuration out of your code entirely:
...
- [Optional] `model`: A string specifying the name of the Azure OpenAI model your deployment serves. Used to look up pricing and capabilities in `deepeval`'s model registry, and used as the deployment name when `deployment` is not passed.
- [Optional] `deployment`: A string specifying the name of your Azure OpenAI deployment, which requests are routed by. Defaults to `model`, then `AZURE_DEPLOYMENT_NAME`, then `AZURE_MODEL_NAME` if not passed; raises an error if unset.
- [Optional] `endpoint`: A string specifying your Azure OpenAI endpoint URL. Defaults to `AZURE_OPENAI_ENDPOINT` if not passed; raises an error if unset.
- [Optional] `apiKey`: A string specifying your Azure OpenAI API key. Defaults to `AZURE_OPENAI_API_KEY` if not passed; raises an error at runtime if unset.
- [Optional] `apiVersion`: A string specifying the OpenAI API version used in your deployment. Defaults to `OPENAI_API_VERSION` if not passed.
...
- [Optional] `generationKwargs`: An object of additional generation parameters forwarded to the Azure OpenAI `chat.completions.create(...)` call.
...
:::tip
Any additional parameters you would like to use for your model can be passed through the generation kwargs parameter. However, we recommend that you double check the params supported by the model and your model provider in their [official docs](https://learn.microsoft.com/en-us/azure/ai-foundry/openai/reference#request-body).
:::
