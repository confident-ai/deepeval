...
There are **ZERO** mandatory and **EIGHT** optional parameters when creating an `AzureOpenAIModel`:
...
Any other key you pass is forwarded as-is to the Azure OpenAI `chat.completions.create(...)` call, so generation parameters `deepeval` does not name go inline alongside the rest:
...
:::tip
Before passing additional generation parameters, we recommend that you double check the params supported by the model and your model provider in their [official docs](https://learn.microsoft.com/en-us/azure/ai-foundry/openai/reference#request-body).
:::
