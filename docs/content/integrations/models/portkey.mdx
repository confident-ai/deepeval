...
There are **ZERO** mandatory and **NINE** optional parameters when creating a `PortkeyModel`:
...
- [Optional] `generationKwargs`: An object of additional generation parameters forwarded to Portkey's `chat.completions.create(...)` call.
...
:::tip
Pass generation parameters such as `max_tokens` via `generationKwargs` (they are forwarded to `chat.completions.create(...)`).
:::
