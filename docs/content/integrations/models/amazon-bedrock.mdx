...
- [Optional] `model`: A string specifying the bedrock model identifier to call (e.g. `anthropic.claude-sonnet-4-5-20250929-v1:0`). Defaults to `AWS_BEDROCK_MODEL_NAME` if not passed; raises an error at runtime if unset.
...
- [Optional] `temperature`: A number specifying the model temperature. Not sent unless you set it, since some Bedrock models (e.g. OpenAI GPT-6) reject the field.
