...
There are **ZERO** mandatory and **NINE** optional parameters when creating an `AmazonBedrockModel`:
...
- [Optional] `generationKwargs`: An object of generation parameters that will be sent to Bedrock as `inferenceConfig`. Available keys may vary by the Bedrock model you choose. See the [AWS Bedrock inference parameters docs](https://docs.aws.amazon.com/bedrock/latest/userguide/inference-parameters.html).

:::tip
Pass generation parameters like `topP` or `maxTokens` via `generationKwargs` (they are sent as `inferenceConfig`).
:::
