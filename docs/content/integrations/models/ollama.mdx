...
There are **ZERO** mandatory and **SIX** optional parameters when creating an `OllamaModel`:
...
- [Optional] `generationKwargs`: An object of additional generation parameters merged into Ollama's `chat(..., options: {...})` call.
...
:::tip
Any additional parameters you would like to use for your model can be passed through the generation kwargs parameter. However, we request you to double check the params supported by the model and your model provider in their [official docs](https://ollama.readthedocs.io/en/api/#parameters).
:::
