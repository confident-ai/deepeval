...
First point `deepeval` at Ollama. `LOCAL_MODEL_API_KEY` must be the literal string `ollama` — that value is what routes a plain model name to your Ollama server instead of a hosted provider:
...
Then pass the name of your desired model as a string in your metric initialization:
...
Running `deepeval set-ollama --model=deepseek-r1:1.5b --save` writes these same variables for you.
