...
Alternatively, you can specify your model directly in code using `GeminiModel` from DeepEval's model collection. By default, `model` is set to `gemini-2.5-flash`.
...
There are **ZERO** mandatory and **EIGHT** optional parameters when creating a `GeminiModel` through Vertex AI:

- [Optional] `model`: A string specifying the name of the Gemini model to use. Defaults to `GEMINI_MODEL_NAME` if not passed; falls back to `gemini-2.5-flash` if unset.
...
- [Optional] `generationKwargs`: An object of additional generation parameters merged into the `generateContent` request `config`.
...
:::tip
Any additional parameters you would like to use for your model can be passed through the `generationKwargs` parameter. However, we request you to double check the params supported by the model and your model provider in their [official docs](https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/content-generation-parameters).
:::
