...
Most [custom LLM-as-a-Judge metrics](/blog/llm-as-a-judge) begin with a sentence:
...
Beyond the questions themselves, three settings shape how a JevEval metric behaves: what it records, whether it explains the result, and whether partial credit is allowed.
...
By default, JevEval attaches a `reason` built from the outcomes themselves: one line per question with its result in words, the probability and confidence behind it, and a closing line showing how the score follows. Nothing is generated. The same answers always produce the same reason, and there is no LLM anywhere in the metric, so there is no `model` argument to configure.

Set `include_reason=False`, as in the example above, and you skip the reason text; the metric still returns `score`, `success`, `confidence`, and `score_breakdown`.

This boundary matters. A reason that is assembled from the decision cannot drift from it, and passing or failing a regression test stays a decision task with no generative step in the loop.
...
It turns the same question set from a quality score into an executable acceptance test you can use in an [eval-driven development loop](/blog/eval-driven-development).
...
[`ConversationalJevEval`](/docs/metrics-jev-eval) applies the same Noul, Score, and Choice model to an entire [`ConversationalTestCase`](/docs/evaluation-multiturn-test-cases). The conversation becomes a structured `turns` list, and you can add fields such as `scenario`, `expected_outcome`, and `chatbot_role` to the state.

The scoring equation stays the same. The only difference is what the questions can see.

If you do not have conversation test cases yet, start with the [conversation simulation workflow](/blog/how-to-evaluate-conversational-agent) and use `ConversationalJevEval` to score the generated conversations.
...
The full [JevEval documentation](/docs/metrics-jev-eval) covers question design, strict mode, score calculation, and a worked example down to every probability. If you're evaluating multi-turn agents, switch to the Multi-Turn tabs on that same page for `ConversationalJevEval`.
...
You write the evaluation specification as bounded questions. Jev returns calibrated decisions. DeepEval turns them into a traceable score with fixed math, and the explanation is read straight off those decisions, so nothing generative ever gets a vote.
