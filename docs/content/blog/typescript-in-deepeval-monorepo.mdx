...
:::info
DeepEval for TypeScript is out in beta. Read the launch post, [DeepEval for TypeScript, now fully open-source](/blog/introducing-deepeval-typescript), or keep track of it on [our GitHub](https://github.com/confident-ai/deepeval).
:::
...
The first is docs. Prose is written once and rendered per-language: an [inline term component](https://github.com/confident-ai/deepeval/blob/main/docs/lib/lang/term.ts) carries the Python and TypeScript spelling of every code span side by side — `test_case` ↔ `testCase`, `actual_output` ↔ `actualOutput`, `touch test_example.py` ↔ `touch test_example.ts` — so the documentation never silently describes one language while showing the other.
