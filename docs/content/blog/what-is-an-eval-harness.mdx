...
Validation loops in the assurance layer is which is where **the eval harness lives.**
...
Both of which are custom to your agentic use case. The in one verification loop within an eval harness, it typically involves:
...
3. Use all collected responses an traces to run your metric suite on it
...
You can jump ahead and get started [here](/docs/getting-started), but do first finish the article because its about to get interesting.
...
This is a big deal because, although Claude Code's own harness requires concepts of Skills and Hooks - there exist no eval harness for Claude Code building AI agents, not deterministic software.
...
There's no assert equal for "was this response faithful?" The output is non-deterministic — the one thing hooks and lint can't catch. Claude Code's harness can validate the software it writes, but not the agent it writes.
