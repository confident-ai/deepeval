...
These three techniques all score a single output. A fourth pattern, pairwise comparison, judges two or more outputs against each other instead, which is useful when you want to know which prompt or model version is better rather than how good one output is on its own.
