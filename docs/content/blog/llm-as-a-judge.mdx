---
title: "LLM-as-a-Judge in 2026: Top evaluation techniques and best practices"
description: LLM-as-a-Judge uses an LLM to score, classify, or compare another LLM's outputs. Here's what it is, the main techniques, and best practices for reliable evaluation.
date: 2026-06-15
authors: [penguine]
category: community
---
...
This guide explains what LLM-as-a-Judge is, and how to use it in an open-source LLM evaluation framework like DeepEval through three main techniques:
...
These techniques can judge different scopes. Most score one output, component, or interaction, while trajectory-based agent metrics inspect a complete ordered trace. A separate pattern, pairwise comparison, judges two or more outputs against each other, which is useful when you want to know which prompt or model version is better rather than how good one output is on its own.

DeepEval also supports a decision model alternative for bounded decisions: [`JevEval`](/blog/introducing-jev-as-a-judge). It uses Jev, a System One decision model rather than an LLM judge, to answer explicit questions with calibrated probabilities. JevEval is therefore not technically LLM-as-a-Judge, but it belongs in the same design conversation whenever you are choosing how a custom metric should judge an output.

But before we dive too deep, let's go through the fundamentals of what LLM-as-a-judge is.
...
- Did the agent follow a sound and efficient path through its plan, model decisions, tool calls, and handoffs? This requires trajectory-based evaluation.
- Which prompt or model version performed better? This is usually measured with pairwise preference.

This makes LLM-as-a-Judge especially useful for evaluating LLM applications where quality is semantic, subjective, or context-dependent. A customer support answer can be factually correct but too vague. A RAG answer can sound fluent while hallucinating. An AI agent can call tools successfully but still fail the user task—or reach the right answer through a wasteful or plan-breaking sequence. These are the kinds of failures that traditional exact-match metrics usually miss.
...
- Use [`JevEval`](/blog/introducing-jev-as-a-judge) when you can express a custom metric as explicit bounded questions and want the score calculated with fixed math.
- Use `DAGMetric` for strict multi-step scoring logic.
- Use built-in RAG metrics for grounding and retrieval quality.
- Use built-in agentic metrics for complete trajectories and component-level tool use.
...
DeepEval gives you multiple ways to turn LLM-as-a-Judge from a broad idea into a repeatable evaluation metric. The judging technique and evaluation scope are separate choices: a judge may inspect final output, one span, or an agent's complete ordered trajectory.
...
| Metric                                                            | What the judge checks                                          | Evaluation scope    | Required reference-like field          |
| ----------------------------------------------------------------- | -------------------------------------------------------------- | ------------------- | -------------------------------------- |
| [`AnswerRelevancyMetric`](/docs/metrics-answer-relevancy)         | Whether `actual_output` answers the `input`                    | Output              | None                                   |
| [`FaithfulnessMetric`](/docs/metrics-faithfulness)                | Whether `actual_output` is grounded in retrieved context       | Output              | `retrieval_context`                    |
| [`ContextualRelevancyMetric`](/docs/metrics-contextual-relevancy) | Whether retrieved chunks are relevant to the input             | Component/output    | `retrieval_context`                    |
| [`ContextualRecallMetric`](/docs/metrics-contextual-recall)       | Whether retrieval captured facts needed by the expected answer | Component/output    | `expected_output`, `retrieval_context` |
| [`TaskCompletionMetric`](/docs/metrics-task-completion)           | Whether the ordered agent execution achieved the task          | Trajectory          | Depends on metric setup                |
| [`StepEfficiencyMetric`](/docs/metrics-step-efficiency)           | Whether the agent avoided unnecessary steps                    | Trajectory          | None                                   |
| [`PlanAdherenceMetric`](/docs/metrics-plan-adherence)             | Whether execution followed the agent's plan                    | Trajectory          | Agent plan captured in trace           |
| [`PlanQualityMetric`](/docs/metrics-plan-quality)                 | Whether the agent's plan was suitable for the task              | Trajectory          | Agent plan captured in trace           |
| [`ToolCorrectnessMetric`](/docs/metrics-tool-correctness)         | Whether one decision selected the right tools                  | Component-level     | `expected_tools`                       |
| [`ArgumentCorrectnessMetric`](/docs/metrics-argument-correctness) | Whether one decision supplied correct tool arguments           | Component-level     | Expected tool arguments                |
...
Use built-in QAG-style metrics when your evaluation target is already covered by DeepEval. They give you stronger defaults than a one-sentence custom judge. For agents, `TaskCompletionMetric`, `StepEfficiencyMetric`, `PlanAdherenceMetric`, and `PlanQualityMetric` judge the complete trajectory; tool and argument metrics remain component-level checks on individual LLM spans.

## A Decision-Model Alternative: Jev-as-a-Judge

The techniques above all use an LLM somewhere in the judging path. If you already know the exact decisions your metric needs to make, generation may be unnecessary.

[`JevEval`](/blog/introducing-jev-as-a-judge) lets you define those decisions as three bounded question types:

- `Noul` for a true-or-false proposition.
- `Score` for ordered descriptive levels.
- `Choice` for one option from an unordered closed set.

Jev returns calibrated probabilities for every question in one request. DeepEval maps those probabilities onto `0–1` values and calculates a weighted mean. With `include_reason=False`, no LLM is called at all; with it enabled, an LLM only explains the result and cannot change the score.

Use `GEval` when you want an LLM to derive evaluation logic from broad criteria. Use `DAGMetric` when the logic branches. Use JevEval when you can write the decision points directly and want a complete probability-level breakdown. For the model architecture and its use inside built-in metrics, see [Introducing Jev for Evals](/blog/introducing-jev-in-deepeval).

## Choosing Between G-Eval, DAG, QAG, and JevEval
...
| Score explicit bounded questions with fixed math         | `JevEval`                       | Jev returns probabilities; DeepEval calculates the weighted score      |
...
| Evaluate a complete agent trajectory                     | Built-in trajectory metrics     | They inspect ordered plans, decisions, calls, and handoffs             |
| Evaluate one agent tool-use decision                     | Built-in component metrics      | Tool-specific metrics understand `tools_called` and `expected_tools`   |
...
- Use [`JevEval`](/blog/introducing-jev-as-a-judge) when the rubric can be decomposed into bounded questions and you do not need an LLM to generate the judgement.
...
LLM-as-a-Judge can be used anywhere you need repeatable quality checks. The most common workflows are regression testing before deployment, trajectory-based evaluation of complete agent traces, component-level evaluation of individual spans, and production monitoring after release.
...
### Component-Level Evaluation on Traces
...
A component judge sees one span. A [trajectory-based evaluation](/docs/evaluation-trajectory-based-llm-evals) instead gives the judge the complete ordered agent trace: plans, model decisions, tool calls, results, and sub-agent handoffs. This lets it assess whether the agent completed the task, followed its plan, and avoided unnecessary steps—not merely whether the final response looks correct.
...
Keep `ToolCorrectnessMetric` and `ArgumentCorrectnessMetric` on the relevant LLM span when you need to judge a single tool-use decision.
...
For `JevEval`, inspect `score_breakdown` instead. It records every question's probability distribution, mapped value, weight, and applicability, so the final score can be traced back to the individual decisions.
...
<FAQs
  qas={[
    {
      question: "What is the best framework to use LLM-as-a-Judge?",
      answer: (
        <>
          We recommend{" "}
          <a href="https://github.com/confident-ai/deepeval">DeepEval</a>, the
          open-source LLM evaluation framework with{" "}
          <strong>50+ ready-to-use LLM-as-a-Judge metrics</strong> like{" "}
          <code>GEval</code>, <code>DAGMetric</code>, <code>ArenaGEval</code>,
          and built-in RAG and agentic metrics, so you can define a judge in a
          few lines instead of hand-writing judge prompts and scoring logic from
          scratch.
        </>
      ),
    },
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          built-in metrics for common RAG, agent trajectory, or component
          workflows, and{" "}
    ...
    {
      question: "Can an LLM judge evaluate an AI agent's trajectory?",
      answer: (
        <>
          Yes. Trajectory metrics inspect the complete ordered trace, including
          plans, model decisions, tool calls, and handoffs. Use{" "}
          <code>TaskCompletionMetric</code>,{" "}
          <code>StepEfficiencyMetric</code>,{" "}
          <code>PlanAdherenceMetric</code>, or <code>PlanQualityMetric</code>{" "}
          for this scope; keep tool and argument metrics on individual LLM
          spans.
        </>
      ),
    },
    ...
    {
      question: "How is JevEval different from LLM-as-a-Judge?",
      answer: (
        <>
          JevEval does not use a generative LLM to make its decisions. You
          define bounded <code>Noul</code>, <code>Score</code>, and{" "}
          <code>Choice</code> questions; Jev returns calibrated probabilities;
          and DeepEval calculates the score with fixed math. An LLM is optional
          and can only write the explanation.
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          traces with <code>@observe</code>. For production monitoring over live
          traffic, use <a href="https://confident-ai.com">Confident AI</a> with
          DeepEval metrics to track referenceless scores such as answer
          relevancy, task completion, faithfulness, safety, or custom G-Eval
          metrics over time.
        </>
      ),
    },
...
- Read [Introducing JevEval](/blog/introducing-jev-as-a-judge) to build a custom Jev-as-a-Judge metric from bounded questions.
...
- Use [trajectory-based evaluation](/docs/evaluation-trajectory-based-llm-evals) to judge complete ordered agent traces.
