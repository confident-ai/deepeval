...
No LLM generates a verdict anywhere in that path. The only thing an LLM can do in JevEval is, optionally, write the reason. Set `include_reason=False` and the metric is one Jev request plus arithmetic.
