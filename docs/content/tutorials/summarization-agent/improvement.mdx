...
await dataset.pull({ alias: "MeetingSummarizer Dataset" });
```
...
import { MeetingSummarizer } from "./meetingSummarizer"; // import your summarizer here

const summarizer = new MeetingSummarizer(); // Initialize with your best config
const summaryTestCases: LLMTestCase[] = [];
const actionItemTestCases: LLMTestCase[] = [];
for (const golden of dataset.goldens) {
  if (!("input" in golden)) continue;
  const [summary, actionItems] = await summarizer.summarize(golden.input);
  const summaryTestCase = new LLMTestCase({
    input: golden.input,
    actualOutput: summary,
  });
  const actionItemTestCase = new LLMTestCase({
    input: golden.input,
    actualOutput: JSON.stringify(actionItems),
  });
  summaryTestCases.push(summaryTestCase);
  actionItemTestCases.push(actionItemTestCase);
}

console.log(summaryTestCases.length);
console.log(actionItemTestCases.length);
```
...
const summarySystemPrompt = "..."; // Use your new summary system prompt here

const actionItemSystemPrompt = "..."; // Use your new action item system prompt here

const models = ["gpt-3.5-turbo", "gpt-4o", "gpt-4-turbo"];

// Use the same metrics used before
const summaryConcision = new metrics.GEval({
  name: "Summary Concision",
  criteria: "Assess whether the summary is concise and focused only on the essential points of the meeting.",
  evaluationParams: [SingleTurnParams.INPUT, SingleTurnParams.ACTUAL_OUTPUT],
});
const actionItemCheck = new metrics.GEval({
  name: "Action Item Check",
  criteria: "Assess whether all action items from the meeting are captured accurately.",
  evaluationParams: [SingleTurnParams.INPUT, SingleTurnParams.ACTUAL_OUTPUT],
});
...
for (const model of models) {
  ...
  await evaluate(summaryTestCases, [summaryConcision]);
  await evaluate(actionItemTestCases, [actionItemCheck]);
}
```
...
class MeetingSummarizer {
  ...
  summarize = observe({
    fn: async (
      transcript: string,
      summaryModel: string = "gpt-4o",
      actionItemModel: string = "gpt-4-turbo",
    ) => {
      const summary = await this.getSummary(transcript, summaryModel);
      const actionItems = await this.getActionItems(transcript, actionItemModel);

      return [summary, actionItems] as [string, Record<string, any>];
    },
  });

  getSummary = observe({
    fn: async (transcript: string, model?: string) => {
      // ...
  ...
  getActionItems = observe({
    fn: async (transcript: string, model?: string) => {
      // ...
      const response = await this.client.chat.completions.create({
        model: model || this.model,
        messages: [
          { role: "system", content: this.actionItemSystemPrompt },
          { role: "user", content: transcript },
        ],
      });
      return JSON.parse(
