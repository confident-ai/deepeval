...
class MeetingSummarizer {
  ...
  constructor(
    ...
    this.actionItemSystemPrompt = actionItemSystemPrompt || (
      "..." // Use the action_item_system_prompt mentioned above
    );
  }
  ...
  getSummary = observe({
    ...
    fn: async (transcript: string, model?: string) => {
      try {
        ...
        updateCurrentSpan({ input: transcript, output: summary });
        return summary;
      } catch (e) {
        console.log(`Error generating summary: ${e}`);
        return `Error: Could not generate summary due to API issue: ${e}`;
      }
    },
  });
  ...
  getActionItems = observe({
    ...
    fn: async (transcript: string, model?: string) => {
      try {
        ...
        const actionItems = response.choices[0].message.content?.trim() ?? "";
        try {
          const parsed = JSON.parse(actionItems);
          updateCurrentSpan({ input: transcript, output: JSON.stringify(parsed) });
          return parsed;
        } catch {
          return { error: "Invalid JSON returned from model", raw_output: actionItems };
        }
      } catch (e) {
        console.log(`Error generating action items: ${e}`);
        return { error: `API call failed: ${e}`, raw_output: "" };
      }
    },
  });
}
...
```typescript title="meeting_summarizer_quality.ts"
import { EvaluationDataset } from "deepeval/dataset";
...
for await (const golden of dataset.evalsIterator()) {
  ...
  await summarizer.summarize(golden.input); // captures trace and evaluates spans
}
```
...
```bash
npx tsx meeting_summarizer_quality.ts
```
    ...
    steps:
      ...
      - name: Run DeepEval Unit Tests
        env:
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }} # Add your OPENAI_API_KEY
          CONFIDENT_API_KEY: ${{ secrets.CONFIDENT_API_KEY }} # Add your CONFIDENT_API_KEY
        run: npx tsx meeting_summarizer_quality.ts
```
