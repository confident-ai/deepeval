...
const testCase = new LLMTestCase({
  input: "...", // Your transcript
  actualOutput: "...", // The summary or action items
});
```
...
const documentsPath = "path/to/documents/folder";
const transcripts: string[] = [];

for (const document of readdirSync(documentsPath)) {
  if (document.endsWith(".txt")) {
    const filePath = join(documentsPath, document);
...
goldens.forEach((golden, i) => {
  console.log(`Golden ${i}: `, golden.input.slice(0, 20));
});
```
...
await dataset.push({ alias: "MeetingSummarizer Dataset" });
```
