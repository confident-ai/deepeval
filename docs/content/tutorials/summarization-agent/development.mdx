...
class MeetingSummarizer {
  ...
  constructor(model: string = "gpt-4", systemPrompt: string = "") {
    this.model = model;
...
const transcript = readFileSync("meeting_transcript.txt", "utf-8").trim();
...
class MeetingSummarizer {
  // ...
  async getActionItems(transcript: string): Promise<Record<string, any>> {
    try {
      const response = await this.client.chat.completions.create({
        model: this.model,
        messages: [
...
class MeetingSummarizer {
  // ...
  async summarize(transcript: string): Promise<[string, Record<string, any>]> {
    const summary = await this.getSummary(transcript);
    const actionItems = await this.getActionItems(transcript);
...
const [summary, actionItems] = await summarizer.summarize(transcript);
console.log(summary);
console.log("JSON:");
