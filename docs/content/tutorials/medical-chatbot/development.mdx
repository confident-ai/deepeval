...
```typescript title="main.ts"
const chatbot = new MedicalChatbot("gpt-4o-mini");
await chatbot.model.invoke([{ role: "user", content: "Hi!" }]);
```
...
class MedicalChatbot {
  ...
  constructor(model: string) {
    this.model = new ChatOpenAI({ model });
    // For RAG engine
    this.encoder = new OpenAIEmbeddings({ model: "text-embedding-3-small" });
    this.client = new QdrantClient({ url: "http://localhost:6333" });
  }

  async indexKnowledge(documentPath: string) {
    const documents = readFileSync(documentPath, "utf-8").split("\n");

    // Create namespace in qdrant
    await this.client.createCollection("gale_encyclopedia", {
      vectors: { size: 1536, distance: "Cosine" },
    });

    // Vectorize and index into qdrant
    const vectors = await this.encoder.embedDocuments(documents);
    await this.client.upsert("gale_encyclopedia", {
...
```typescript title="main.ts"
const chatbot = new MedicalChatbot("gpt-4o-mini");
await chatbot.indexKnowledge("path-to-your-encyclopedia.txt");
```
...
class MedicalChatbot {
  ...
  retrieveKnowledge = tool(
    ...
    },
    {
      ...
      description: "A tool to retrieve data on various diagnosis methods from gale encyclopedia",
...
```typescript title="main.ts"
const chatbot = new MedicalChatbot("gpt-4o-mini");
await chatbot.retrieveKnowledge.invoke({ query: "Cough, fever, and diarrhea." });
```
...
class MedicalChatbot {
  ...
  createAppointment = tool(
    async ({ name, email, date }: { name: string; email: string; date: string }) => {
      const parsedDate = new Date(date);
      if (isNaN(parsedDate.getTime())) {
        return "Invalid date format. Please use YYYY-MM-DD format.";
      }
      const appointment: Appointment = { id: randomUUID(), name, email, date: parsedDate };
      this.appointments.push(appointment);
      return `Created new appointment with ID: ${appointment.id} for ${name} on ${date}.`;
    },
    {
      name: "create_appointment",
      description: "Create a new appointment with the given ID, name, email, and date",
      schema: z.object({ name: z.string(), email: z.string(), date: z.string() }),
    },
  );
  ...
  retrieveKnowledge = tool(
    ...
    { name: "retrieve_knowledge", schema: z.object({ query: z.string() }) },
  );
...
// Simple in-memory store for chat histories
const chatStore: Record<string, InMemoryChatMessageHistory> = {};
function getSessionHistory(sessionId: string): InMemoryChatMessageHistory {
  if (!chatStore[sessionId]) {
    chatStore[sessionId] = new InMemoryChatMessageHistory();
  }
...
class MedicalChatbot {
  ...
  // For managing appointments
  appointments: Appointment[] = [];
  agentWithMemory: RunnableWithMessageHistory<Record<string, any>, any>;
  ...
  setupAgent() {
    ...
    // Create agent
    const tools = [this.retrieveKnowledge, this.createAppointment];
    const agent = createToolCallingAgent({ llm: this.model, tools, prompt });
    const agentExecutor = new AgentExecutor({ agent, tools });
    this.agentWithMemory = new RunnableWithMessageHistory({
...
async function startSession(sessionId?: string) {
  // Start an interactive session with the chatbot
  console.log("Hello! I am Baymax, your personal healthcare companion.");
  console.log("How are you feeling today? (type 'exit' to quit.)");
  ...
  while (true) {
    ...
    const response = await chatbot.agentWithMemory.invoke(
      { input: userInput },
      { configurable: { sessionId } },
    );
    console.log("Baymax:", response.output);
  }
...
// Initialize chatbot and start session
const chatbot = new MedicalChatbot(MODEL, SYSTEM_PROMPT);
await chatbot.indexKnowledge("path-to-your-encyclopedia.txt");
await startSession();
```
