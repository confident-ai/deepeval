...
```typescript title="main.ts"
const chatbot = new MedicalChatbot("gpt-4o-mini");
await chatbot.retrieveKnowledge.invoke({
  query: "Cough, fever, and diarrhea.",
});
```
...
class MedicalChatbot {
  ...
  // For managing appointments
  appointments: Appointment[] = [];
  agentWithMemory: ReturnType<typeof createAgent>;
  ...
  setupAgent = () => {
    ...
    this.agentWithMemory = createAgent({
      model: this.model,
      tools,
