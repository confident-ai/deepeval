...
await dataset.pull({ alias: "Medical Chatbot Dataset" });
```
...
class MedicalChatbot {
  ...
  createAppointment = async (appointmentId: string) => {
    // ...
  };
...
```typescript
import { MedicalChatbot } from "./medical_chatbot";
