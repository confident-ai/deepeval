...
const testCase = new ConversationalTestCase({
  turns: [
    new Turn({ role: "user", content: "I've a sore throat." }),
    new Turn({ role: "assistant", content: "Thanks for letting me know?" }),
  ],
});
```
...
// Example: Fetch conversations from your database
const conversations = await fetchConversationsFromDb(); // Your database query here

const testCases = [];
for (const conv of conversations) {
  const turns = conv.messages.map((msg) => new Turn({ role: msg.role, content: msg.content }));
...
// Initialize test case list
const testCases: ConversationalTestCase[] = [];

async function startSession(chatbot: MedicalChatbot) {
  ...
  while (true) {
    ...
    turns.push(new Turn({ role: "assistant", content: response.output }));
...
const goldens = [
  new ConversationalGolden({
    scenario: "User with a sore throat asking for paracetamol.",
    expectedOutcome: "Gets a recommendation for panadol.",
  }),
  new ConversationalGolden({
    scenario: "Frustrated user looking to rebook their appointment.",
    expectedOutcome: "Gets redirected to a human agent",
  }),
  new ConversationalGolden({
    scenario: "User just looking to talk to somebody.",
    expectedOutcome: "Tell them this chatbot isn't meant for this use case.",
  }),
];

// Create dataset and optionally push to Confident AI
const dataset = new EvaluationDataset({ goldens });
await dataset.push({ alias: "Medical Chatbot Dataset" });
```
...
const modelCallback = async ({ input, turns, threadId }: { input: string; turns: Turn[]; threadId: string }) => {
  // 1. Get latest simulated user input
  const userInput = turns[turns.length - 1].content;
  // 2. Call chatbot
  const response = await chatbot.agentWithMemory.invoke(
    { input: userInput },
  ...
  // 3. Return chatbot turn
  return new Turn({ role: "assistant", content: response.output });
};
...
const relevancy = new metrics.TurnRelevancyMetric();
```
...
const faithfulness = new metrics.TurnFaithfulnessMetric();
```
...
// Test cases and metrics from previous sections
await evaluate(testCases, [relevancy, faithfulness]);
```
