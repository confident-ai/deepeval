...
for (const conv of conversations) {
  const turns = conv.messages.map(
    (msg) => new Turn({ role: msg.role, content: msg.content }),
  );
...
const startSession = async (chatbot: MedicalChatbot) => {
  ...
  while (true) {
    ...
    turns.push(new Turn({ role: "assistant", content: output }));

    console.log("Baymax:", output);
  }
  rl.close();
  testCases.push(new ConversationalTestCase({ turns }));
};
...
const modelCallback = async ({
  ...
  const output =
    typeof lastMessage?.content === "string" ? lastMessage.content : "";
  // 3. Return chatbot turn
  return new Turn({ role: "assistant", content: output });
};
