...
class RAGAgent {
  ...
  vectorStoreClass: typeof FaissStore;
  k: number;
  ...
  constructor({
    ...
    this.vectorStoreClass = vectorStoreClass;
    this.k = k;
  }
  ...
  async loadVectorStore() {
    const documents = [];
    for (const documentPath of this.documentPaths) {
      const rawText = readFileSync(documentPath, "utf-8");

      const splitter = new RecursiveCharacterTextSplitter({
        chunkSize: this.chunkSize,
        chunkOverlap: this.chunkOverlap,
      });
      documents.push(...(await splitter.createDocuments([rawText])));
    }

    return this.vectorStoreClass.fromDocuments(documents, this.embeddingModel);
  }
}
```
...
```typescript
const documentPaths = ["theranos_legacy.txt"];
const agent = new RAGAgent({ documentPaths });
await agent.init();
console.log(agent.vectorStore);
```
...
```typescript
const docPath = ["theranos_legacy.txt"];

const retriever = new RAGAgent({ documentPaths: docPath });
await retriever.init();
const retrievedDocs = await retriever.retrieve("How many blood tests can you perform and how much blood do you need?");
...
class RAGAgent {
  ...
  async generate(
    ...
    const model = llmModel ?? new ChatOpenAI({ temperature: 0 });
    const prompt = (
      promptTemplate ??
      "Answer the query using the context below.\n\nContext:\n{context}\n\nQuery:\n{query}" +
        "Only use information from the context. If nothing relevant is found, respond with: 'No relevant information available.'"
...
```typescript
const docPath = ["theranos_legacy.txt"];
const query = "How many blood tests can you perform and how much blood do you need?";

const retriever = new RAGAgent({ documentPaths: docPath });
await retriever.init();
const retrievedDocs = await retriever.retrieve(query);
const generatedAnswer = await retriever.generate(query, retrievedDocs);
...
```typescript
const documentPaths = ["theranos_legacy.txt"];
const query = "What is the NanoDrop 3000, and what certifications does Theranos hold?";

const retriever = new RAGAgent({ documentPaths });
await retriever.init();
const [answer, retrievedDocs] = await retriever.answer(query);
```
...
class RAGAgent {
  ...
  async answer(query: string) {
    const retrievedDocs = await this.retrieve(query);
    const generatedAnswer = await this.generate(query, retrievedDocs);

    try {
      return JSON.parse(generatedAnswer);
    } catch {
      return { error: "Invalid JSON returned from model", rawOutput: generatedAnswer };
    }
  }
}
```
