...
const testCase = new LLMTestCase({
  input: "...", // Your query
  actualOutput: "...", // The answer from RAG
  retrievalContext: ["..."], // Your retrieved context
});
```
...
// Example: Fetch queries and responses from your database
const queries = await fetchQueriesFromDb(); // Your database query here

const testCases = [];
for (const query of queries) {
  const testCase = new LLMTestCase({
    input: query.input,
    actualOutput: query.response,
    retrievalContext: query.context,
  });
...
await dataset.push({ alias: "RAG QA Agent Dataset" });
```
...
```typescript
import { LLMTestCase, EvaluationDataset } from "deepeval";
import { RAGAgent } from "./rag_qa_agent"; // Import your RAG Agent here

const dataset = new EvaluationDataset();
await dataset.pull({ alias: "RAG QA Agent Dataset" });
const agent = new RAGAgent({ documentPaths: ["theranos_legacy.txt"] });
await agent.init();

const testCases = [];
for (const golden of dataset.goldens) {
  if (!("input" in golden)) continue;
  const retrievedDocs = await agent.retrieve(golden.input);
  const response = await agent.generate(golden.input, retrievedDocs);
  const testCase = new LLMTestCase({
    input: golden.input,
    actualOutput: String(response),
...
const relevancy = new metrics.ContextualRelevancyMetric();
const recall = new metrics.ContextualRecallMetric();
const precision = new metrics.ContextualPrecisionMetric();
```
...
const answerCorrectness = new metrics.GEval({
  name: "Answer Correctness",
  criteria: "Evaluate if the actual output's 'answer' property is correct and complete from the input and retrieved context. If the answer is not correct or complete, reduce score.",
  evaluationParams: [SingleTurnParams.INPUT, SingleTurnParams.ACTUAL_OUTPUT, SingleTurnParams.RETRIEVAL_CONTEXT],
});

const citationAccuracy = new metrics.GEval({
  name: "Citation Accuracy",
  criteria: "Check if the citations in the actual output are correct and relevant based on input and retrieved context. If they're not correct, reduce score.",
  evaluationParams: [SingleTurnParams.INPUT, SingleTurnParams.ACTUAL_OUTPUT, SingleTurnParams.RETRIEVAL_CONTEXT],
});
...
const retrieverMetrics = [relevancy, recall, precision];

await evaluate(testCases, retrieverMetrics);
```
...
const generatorMetrics = [answerCorrectness, citationAccuracy];

await evaluate(testCases, generatorMetrics);
```
