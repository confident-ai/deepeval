...
import { RAGAgent } from "./qa_agent"; // import your RAG QA Agent here

// Evaluate for each golden
const documentPath = ["theranos_legacy.txt"];
const retriever = new RAGAgent({ documentPaths: documentPath });
await retriever.init();

const retrieverTestCases = [];
const generatorTestCases = [];
for (const golden of dataset.goldens) {
  if (!("input" in golden)) continue;
  const retrievedDocs = await retriever.retrieve(golden.input);
  const generatedAnswer = await retriever.generate(golden.input, retrievedDocs);
  const testCase = new LLMTestCase({
    input: golden.input,
    actualOutput: String(generatedAnswer),
    expectedOutput: golden.expectedOutput,
    retrievalContext: retrievedDocs,
  });
  generatorTestCases.push(testCase);
  retrieverTestCases.push(testCase);
}

console.log(retrieverTestCases.length);
console.log(generatorTestCases.length);
```
