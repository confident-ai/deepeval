...
April focused on tightening core APIs, improving observability, and expanding model and simulator capabilities. Several legacy hooks and aliases were removed or deprecated in favor of clearer defaults and more reliable tracing, including richer correlation fields, optional internal span instrumentation, better OTel export mapping, and fixes to multi-turn trace inputs. Evaluation workflows gained `metadata`/`tags` across params, a new `generate` CLI for producing synthetic goldens, and better test-run artifact control, alongside broader simulator coverage with a controller API and safer defaults. Model support and compatibility improved with additions like `gpt-5.4` defaults, `gpt-5.4-mini`,`
...
March focused on making observability and evaluation workflows more reliable and configurable across providers. We shipped first-class AgentCore and OpenInference integrations with richer OpenTelemetry span extraction, OTLP exporting (including a local test-mode exporter), consistent `metric_collection` updates, and clearer dependency and `CONFIDENT_API_KEY` error handling. Provider and platform support improved with better Bedrock behavior and docs, safer Azure keyless auth, and environment-driven evaluation provider selection, while numerous tracing and concurrency fixes tightened correctness for LangChain/LangGraph, CrewAI, generators, pydantic-ai, and async evaluations. We also added `LL
...
February focused on making prompts, tools, and tracing more consistent end-to-end, especially after the API migration. Prompt identity and caching now use commit hashes, pulled and pushed prompts preserve tool metadata, and structured tool fields are converted into a JSON Schema `input_schema` to streamline function/tool calling. Tracing and offline evaluation were hardened with richer prompt metadata in spans, corrected endpoints and parameters, and more reliable observer behavior (including generator return capture), while model- and provider-specific fixes improved compatibility across Azure reasoning models and Amazon Bedrock async evaluations. Operational polish rounded out the month,包括
...
January focused on expanding provider compatibility and hardening observability, with new OpenRouter support via an OpenAI-compatible API, improved Azure OpenAI auth options, and clearer control over Gemini clients and defaults. Tracing and OpenTelemetry integrations were stabilized across LangChain, LangGraph, and PydanticAI, with better endpoint configurability, improved context propagation, and reduced external overhead by removing the New Relic exporter. Evaluation workflows improved with `upload()` for `GEval` metrics, richer verdict context, stricter scoring and validation for edge cases, and better dataset/prompt API key handling and trace/test-case association. Documentation and DX k
