...
July focused on reliability and deterministic agent evaluation, with multiple fixes that eliminate runtime errors and improve prompt rendering and chunk-size validation edge cases. Import ergonomics were restored for dotted access under `deepeval.evaluate` while keeping the callable API intact, and MCP metrics were tightened to report tool usage consistently across call types with more permissive server type validation. The month also introduced zero-LLM metrics like `AgentLoopDetectionMetric` for loop detection and `ToolPermissionMetric` for enforceable tool allow/deny policies suitable for CI gating, alongside added TypeScript support for MCP tool-call reporting. Release engineering and DX

### New Feature

#### python-v4.0.8
- Add `AgentLoopDetectionMetric` to deterministically detect looping and cyclical patterns in agent traces without LLM calls. It combines tool-call repetition, reasoning stagnation, and call-graph cycle checks into a weighted 0.0–1.0 score for zero-cost, low-latency monitoring. ([#2645](https://github.com/confident-ai/deepeval/pull/2645)) {/* pr:2645 */} ([Jeel3011](https://github.com/Jeel3011))
- Add `ToolPermissionMetric` to deterministically verify an agent only calls authorized tools using allowlist/denylist policies. Deny rules take precedence, and the score reflects the fraction of authorized calls (returns `1.0` when no tools were called). Runs without any LLM or API key, making it suitable for CI gating. ([#2826](https://github.com/confident-ai/deepeval/pull/2826)) {/* pr:2826 */} ([Raju R](https://github.com/gh-raju))
- Add TypeScript support for MCP metrics when tool calls are provided via `toolsCalled`, including a `ToolCallType` enum to distinguish MCP vs function calls. ([#2866](https://github.com/confident-ai/deepeval/pull/2866)) {/* pr:2866 */} ([Vamshi Adimalla](https://github.com/A-Vamshi))


### Improvement

#### python-v4.0.8
- Improve draft release notes generation by using an AI-assisted script to clean up and categorize PR entries, with automatic fallback to deterministic titles when AI is unavailable. ([#2861](https://github.com/confident-ai/deepeval/pull/2861)) {/* pr:2861 */} ([Vamshi Adimalla](https://github.com/A-Vamshi))
- Fix spelling typos across the documentation and a few source-code comments/docstrings. No functional or API changes. ([#2865](https://github.com/confident-ai/deepeval/pull/2865)) {/* pr:2865 */} ([max](https://github.com/maxtaran2010))


### Bug Fix

#### python-v4.0.8
- Fix RAGAS telemetry by updating `capture_metric_type()` calls to include the required `in_component` argument, matching the new signature. This prevents runtime errors caused by the previous parameter mismatch. ([#2568](https://github.com/confident-ai/deepeval/pull/2568)) {/* pr:2568 */} ([Sachin Jha](https://github.com/sachinML))
- Fix `TaskCompletionMetric` prompt rendering by correctly passing `tools_called_formatted` into the `extract_goal_and_outcome` template. This prevents `MetricTemplateInterpolationError` under strict Jinja rendering and ensures called tools are included in the prompt, including clean handling of empty tool lists. ([#2808](https://github.com/confident-ai/deepeval/pull/2808)) {/* pr:2808 */} ([MinYi Xie](https://github.com/ppcvote))
- Fix `ContextGenerator.validate_chunk_size` to avoid `UnboundLocalError` when a source document produces 0 chunks. It now surfaces the intended informative `ValueError` and only shows the suggestions header when there are actual suggestions to display. ([#2849](https://github.com/confident-ai/deepeval/pull/2849)) {/* pr:2849 */} ([Muhammad Hassaan](https://github.com/hassaanch23))
- Fix dotted imports like `deepeval.evaluate.configs` failing after `deepeval.evaluate` was rebound to a function. Dotted access to config classes now works again while keeping `deepeval.evaluate` callable, with regression coverage for the config module. ([#2851](https://github.com/confident-ai/deepeval/pull/2851)) {/* pr:2851 */} ([ErenAta16](https://github.com/ErenAta16))
- Fix MCP metrics to report tool usage from `tools_called` and generate interaction text consistently across tool, resource, and prompt calls. Relax MCP server type validation so lists of dicts work even when `mcp.types` isn’t installed. ([#2864](https://github.com/confident-ai/deepeval/pull/2864)) {/* pr:2864 */} ([Vamshi Adimalla](https://github.com/A-Vamshi))



## April

April focused on tightening APIs, expanding model coverage, and making evaluation workflows more observable and reproducible. Legacy configuration and hooks were removed in favor of clearer defaults like `CONFIDENT_API_KEY`, active-trace assertions, and simplified metric/debug plumbing, while new `metadata`/`tags` support and improved golden handling made single- and multi-turn evals easier to round-trip. Tracing and simulation saw major upgrades with richer correlation fields, optional internal span capture, a more robust conversation simulator controller, and fixes across OpenAI and PydanticAI instrumentation. The month also added a `generate` CLI for synthetic goldens, prompt branch tools
...
March focused on significantly improving observability and trace exports for agentic and LLM workflows, with new AgentCore and OpenInference integrations that enrich OpenTelemetry spans, extract inputs/outputs, and support OTLP exporting with configurable metadata, tags, and `metric_collection`, plus stronger docs and CI coverage. Provider selection and model initialization were made more flexible and reliable across OpenAI, Anthropic, Gemini, Azure, and Amazon Bedrock, while the codebase was reformatted under updated Black rules for more consistent tooling. A broad set of fixes tightened correctness and stability in concurrent evaluations, span and token usage extraction across frameworks,
...
February focused on making prompts and tools more robust end-to-end, with improved tool payload support on push/pull, consistent JSON Schema `input_schema` generation, and more reliable prompt identity and caching based on commit hashes. Tracing and telemetry saw major polish, including richer prompt metadata on spans across integrations, safer OpenTelemetry attribute handling, and multiple fixes to evaluation endpoints, offline workflows, and the `@observe` decorator. Platform and dependency quality improved with a configurable cache directory, better Windows cleanup, Click 8.3 compatibility, and a slimmer default install by removing the OTLP gRPC exporter dependency. Regional and provider‑
...
January focused on expanding model/provider connectivity and making evaluation and tracing workflows more robust. The release added OpenRouter support via an OpenAI-compatible API, improved Gemini and Vertex AI client selection and defaults, and enabled Azure OpenAI authentication with Azure AD tokens, alongside better pricing, retries, and headers. Observability and integrations saw major stabilization with improved OpenTelemetry configuration, deterministic LangChain/LangGraph and CrewAI behavior, refined PydanticAI instrumentation, and reduced telemetry overhead by removing the New Relic exporter. On the evaluation side, GEval metrics can now be uploaded to Confident AI, verdicts include`
