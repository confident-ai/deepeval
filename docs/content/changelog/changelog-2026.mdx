...
September's release introduces a custom WebRTC connector, enabling real-time voice communication and expanding integration options for voice agents and services. Users can now store test run results in a local SQLite database, offering a more robust alternative to JSON files. The CLI and environment configuration have been updated to support the new storage backend, and telemetry now tracks which local storage option is in use.

### New Feature

#### v4.2.3
- Add a custom WebRTC connector to enable voice communication using WebRTC protocols. This allows users to connect to voice agents or services over WebRTC, supporting real-time audio streaming and signaling via HTTP or WebSocket endpoints. The connector handles SDP negotiation, candidate exchange, and provides methods for sending audio and text data over data channels. This addition expands the available transport options for voice integrations. ([#3276](https://github.com/confident-ai/deepeval/pull/3276)) {/* pr:3276 */} ([Tanay](https://github.com/tanayvaswani))
...
March was a month of significant improvements in observability, reliability, and flexibility for the tool. Major highlights include deepened OpenTelemetry and OpenInference integrations, enabling richer tracing and metric collection for agents, tools, and LLMs, with robust export and validation options. The release also focused on enhancing provider support and configuration, making it easier to switch between cloud and local backends, and improving the accuracy and consistency of tracing, evaluation, and metric reporting. Numerous bug fixes addressed edge cases in async evaluation, memory management, and provider-specific behaviors, while documentation and code formatting updates ensure a a
