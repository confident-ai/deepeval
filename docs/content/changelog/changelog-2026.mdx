...
April focused on simplifying configuration while deepening evaluation, tracing, and simulator capabilities. Legacy aliases and hooks were removed in favor of clearer APIs like `CONFIDENT_API_KEY`, active-trace assertions, and unified `metadata`/`tags` support across `SingleTurnParams` and `MultiTurnParams`, alongside better controls for saving structured test-run results. The toolchain expanded with a new `generate` CLI for producing synthetic goldens, a Cursor/skills-compatible `deepeval` skill, prompt branch management, and a more robust conversation simulator with controller decisions and a public `SimulationTemplate`. Model and observability support improved with added Anthropic and Open
...
March focused on much deeper observability for agent and LLM workflows, with new AgentCore and OpenInference integrations that instrument OpenTelemetry spans, extract inputs/outputs for agent, tool, and LLM traces, and export via OTLP with configurable metadata, tags, and `metric_collection`, plus a local test-mode exporter and clearer dependency/API-key errors. Provider selection and model initialization became more reliable and flexible, including environment-driven evaluation backends, better Amazon Bedrock detection, and improved Azure OpenAI keyless auth handling. The release also strengthened correctness and stability across tracing, evaluation, and caching, fixing async metric cross‑谈
...
February focused on making prompts and tools more robust across caching, pushing, and tracing, with prompt identity now centered on commit hashes to keep versions, tools, and schema data consistent through API changes. Tool calling got stronger end-to-end support, including conversion of structured tool fields into a JSON Schema `input_schema` and more consistent schema generation even for empty inputs. Observability and evaluation reliability improved with richer prompt metadata in spans, multiple fixes across offline evaluation endpoints and decorators, and cleaner OpenTelemetry defaults by trimming exporter dependencies and avoiding `None` attributes. The month also delivered broad harden
...
January focused on broader model and provider compatibility alongside sturdier tracing and evaluation workflows. We added OpenRouter support with an OpenAI-compatible API, structured outputs, retries, custom headers, and pricing controls, expanded authentication options for Azure OpenAI and Gemini/Vertex AI, and refreshed Gemini defaults to current model IDs. Observability and integrations were hardened with improved OpenTelemetry configuration, removal of the New Relic exporter, and a wave of fixes across LangChain/LangGraph, Pydantic AI, CrewAI, and Bedrock to keep traces deterministic and correctly parented. On the evaluation side, `GEval` uploads to Confident AI, trace serialization now能
