...
In September, the documentation for generating goldens from documents was improved by including three previously missing parameters in the Custom Golden Generation section, ensuring users have access to complete and accurate configuration details. Additionally, the new `ConversationalJevEval` metric was introduced, offering enhanced evaluation capabilities for multi-turn conversations with support for custom parameters and detailed reasoning across conversation turns.

### New Feature

#### v4.2.5
- Add ConversationalJevEval metric for evaluating multi-turn conversations using JevEval questions, enabling more comprehensive assessment of conversational test cases. This metric supports custom evaluation parameters and provides detailed reasoning for outcomes across conversation turns. ([#3337](https://github.com/confident-ai/deepeval/pull/3337)) {/* pr:3337 */} ([Jeffrey Ip](https://github.com/penguine-ip))


### Improvement

#### v4.2.5
- Update the Generate Goldens From Documents guide to include three previously missing parameters in the Custom Golden Generation section, ensuring all configuration options are documented and parameter counts are accurate. ([#3311](https://github.com/confident-ai/deepeval/pull/3311)) {/* pr:3311 */} ([Abdelrahman Mohamed Ali](https://github.com/AbdoAlshoki2))
...
February's release focused on enhancing tool and prompt management, with new support for customizable cache directories and improved handling of prompt versions and schemas. The update introduced more robust integration with external APIs, including support for the AU data region and better compatibility with Azure and Amazon Bedrock models. Numerous bug fixes addressed issues in tracing, documentation, and platform compatibility, while dependency management was streamlined to reduce installation overhead. Overall, these changes improve reliability, flexibility, and developer experience across the toolchain.
