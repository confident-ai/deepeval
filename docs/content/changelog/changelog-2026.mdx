...
September's release introduces enhanced flexibility to persona functionality with new metadata and noise URL fields, allowing for more customizable configurations. Additionally, support for Claude Opus 5.5 has been added to the Anthropic model registry, ensuring accurate parameter handling and cost calculations while maintaining stability for existing models.

### New Feature

#### v4.2.6
- Add support for metadata and noise URL fields to persona functionality, enabling more flexible configuration and enhanced customization options. ([#3333](https://github.com/confident-ai/deepeval/pull/3333)) {/* pr:3333 */} ([Tanay](https://github.com/tanayvaswani))
- Add support for Claude Opus 5.5 in the Anthropic model registry, ensuring correct handling of temperature and thinking parameters and accurate cost calculation. Existing models and AmazonBedrockModel remain unchanged. ([#3349](https://github.com/confident-ai/deepeval/pull/3349)) {/* pr:3349 */} ([Lucas Kim](https://github.com/kimnamu))



## August

August brought significant improvements to the developer tool, focusing on enhanced evaluation workflows, expanded TypeScript support, and greater flexibility in authentication and data management. Key metrics were updated to align with standard scoring conventions, and new features such as voice conversation simulation and component-level evaluations were introduced. The onboarding experience was streamlined with a setup wizard and dynamic server-driven questionnaires, while numerous bug fixes and documentation updates improved reliability and clarity. These changes collectively make the tool more robust, user-friendly, and adaptable for both Python and TypeScript projects.
...
April's release focused on streamlining configuration and improving model support, with legacy options removed in favor of clearer, more consistent APIs. Major enhancements include new CLI tools for generating synthetic goldens, expanded tracing and metadata capabilities, and support for the latest OpenAI and Anthropic models. The documentation site was overhauled for better usability, and numerous fixes were made to tracing, evaluation workflows, and schema handling to ensure more robust and accurate results. These updates collectively make the tool more powerful, easier to use, and better suited for modern Python development workflows.
...
January saw major improvements in model integration, authentication, and documentation across the tool. New support for OpenRouter and Azure AD authentication expands model compatibility, while enhancements to tracing, metric collection, and error handling make integrations with LangChain, LangGraph, and Pydantic AI more robust and reliable. The release also introduces an automated changelog generator and updates documentation for clarity and easier navigation. Numerous bug fixes address edge cases in evaluation, tracing, and response parsing, resulting in a smoother developer experience.
