...
September's release focused on enhancing reliability, scalability, and AI agent compatibility across the tool. Golden test case management was improved for more robust API interactions, while changelog generation now leverages GPT-4.1 and better supports monorepo workflows. Voice mode saw significant upgrades, including support for multiple speech providers, improved diagnostics, and the ability to record full conversation audio with detailed analysis. The platform is now more discoverable and usable by AI agents, with new endpoints, metadata, and in-browser tools, and the conversation simulator supports true parallelism for more realistic testing scenarios.
...
- Add support for multiple speech providers in voice mode, including ElevenLabs, Deepgram, Cartesia, and AssemblyAI, with new environment variables to configure text-to-speech and speech-to-text providers. Update .env.example with configuration options and improve documentation pointers for voice mode features. Refactor imports for speech models to new module paths. Add a root Typer CLI app for easier command registration. Improve logging and diagnostics for audio events in voice simulations. ([#3139](https://github.com/confident-ai/deepeval/pull/3139)) {/* pr:3139 */} ([Jeffrey Ip](https://github.com/penguine-ip))
...
- Add support for recording full conversation audio in voice simulations, including stereo WAV output with caller and agent channels. Improve turn detection and latency logging for voice simulations, and update verbose logs to print breakdowns as indented JSON with rounded floats. Voice simulations now better handle duplex connectors, interruptions, and greeting capture, providing more accurate and detailed artifacts for analysis. ([#3163](https://github.com/confident-ai/deepeval/pull/3163)) {/* pr:3163 */} ([Tanay](https://github.com/tanayvaswani))


### Improvement

#### v4.2.1
- Update the changelog generation script to use the gpt-4.1 model and improve compatibility with monorepo tag schemes. The script now correctly handles tags for different SDKs and ensures changelog entries are rendered with consistent version formatting. Existing PRs are tracked more reliably to prevent duplicate entries across releases. Command-line arguments have been enhanced to support multiple tags and improved usability. ([#3105](https://github.com/confident-ai/deepeval/pull/3105)) {/* pr:3105 */} ([Vamshi Adimalla](https://github.com/A-Vamshi))
- Update the documentation banner to inform users about changes in metrics' scoring logic, clarifying that higher scores now indicate better performance. The banner links to the changelog for more details on this update. ([#3107](https://github.com/confident-ai/deepeval/pull/3107)) {/* pr:3107 */} ([Vamshi Adimalla](https://github.com/A-Vamshi))
