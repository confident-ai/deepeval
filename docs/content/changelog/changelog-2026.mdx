...
April focused on simplifying core APIs while expanding evaluation, simulation, and tracing capabilities. Several legacy hooks and configs were removed or deprecated in favor of clearer defaults, including `CONFIDENT_API_KEY`, active-trace assertions, consolidated debug behavior, and structured JSON test-run outputs. The tool gained stronger eval and data-generation workflows with richer `metadata`/`tags` across params, a new `generate` CLI for synthetic goldens, and improved conversation simulation via a controller API and `SimulationTemplate`. Model and observability support also advanced with new OpenAI and Anthropic models, more reliable logprob-aware metrics, deeper optional internal and
...
March focused on making observability a first-class experience, with new AgentCore and OpenInference integrations that instrument OpenTelemetry spans, extract agent/tool/LLM inputs and outputs, and export via OTLP with configurable metadata, tags, and `metric_collection`, plus clearer dependency and API key errors. Provider flexibility improved with environment-based evaluation selection, better model initialization (including Amazon Bedrock), and fixes across Azure keyless auth and token usage extraction for LangChain/LangGraph. Reliability and correctness saw a broad sweep of fixes, including async metric isolation, more robust tracing for generators, CrewAI, and pydantic-ai, safer caching
...
February focused on making prompts and tools first-class citizens while tightening up tracing and caching reliability. Prompt identity and caching were revamped around commit hashes, with richer span metadata and better OpenTelemetry behavior, plus multiple fixes across integrations and offline evaluations. Tool calling support expanded end-to-end, including consistent JSON Schema `input_schema` generation and preservation of tool/schema data when pulling from cache. The month also delivered pragmatic stability improvements, from environment-configurable cache directories and improved Windows cleanup to broader dependency compatibility and fixes for Azure, Bedrock, and generator-based `@obse
...
January focused on expanding provider coverage and making integrations more predictable and configurable. We added OpenRouter support via an OpenAI-compatible API, improved Azure OpenAI auth with `azure_ad_token` options, and clarified Gemini/Vertex selection with updated defaults that avoid retired model IDs and better ADC fallback. Tracing and telemetry saw major stabilization across LangChain, LangGraph, and PydanticAI, with improved OpenTelemetry configuration via `CONFIDENT_OTEL_URL`, better trace metadata handling (including `test_case_id`), and removal of the New Relic exporter to reduce overhead. On the evaluation and docs side, `GEval` gained `upload()` support to Confident AI, key/
