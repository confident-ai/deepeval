...
September's release introduces enhanced analytics for documentation-driven signups by tracking user navigation paths in a compact and privacy-conscious manner. Async tracing is now more secure, as class names replace instance data in span inputs, reducing the risk of sensitive information exposure. Additionally, experimental integration with TypeSafe AI's System One model enables more deterministic and auditable evaluation decisions for metrics and classifiers, offering users an alternative to LLM-based verdicts.

### Experimental Feature

#### v4.2.4
- Add experimental support for routing metric and classifier decision steps to TypeSafe AI's System One model (Jev) instead of the LLM. In experimental mode, key evaluation metrics and all built-in classifiers use System One for verdicts, scores, and label assignments, with no LLM fallback. Users must configure a TypeSafe API key to enable this feature. This allows evaluation decisions to leverage TypeSafe's model for more deterministic and auditable outcomes. ([#3332](https://github.com/confident-ai/deepeval/pull/3332)) {/* pr:3332 */} ([Jeffrey Ip](https://github.com/penguine-ip))


### Improvement

#### v4.2.4
- Add tracking of docs pages visited to app signup links by including a compact `site_path` parameter, allowing attribution of user signups to specific documentation journeys. The trail is capped at 300 characters and records up to 40 steps, ensuring recent navigation history is preserved. This enhances the ability to analyze which docs pages lead to user signups without affecting other outbound links. ([#3289](https://github.com/confident-ai/deepeval/pull/3289)) {/* pr:3289 */} ([Tony](https://github.com/tony-confidentai))


### Bug Fix

#### v4.2.4
- Fix async tracing to replace the `self` argument with the class name in observed span inputs, preventing sensitive instance data from being captured during tracing of async methods. ([#3300](https://github.com/confident-ai/deepeval/pull/3300)) {/* pr:3300 */} ([Yekta Kocaoğullar](https://github.com/ykocaogullar))



## August

August brought significant improvements to the developer tool, focusing on enhanced evaluation workflows, expanded TypeScript support, and greater flexibility in authentication and data management. Key metrics were updated to align with standard scoring conventions, and new features such as voice conversation simulations and a streamlined setup wizard improved usability and onboarding. Numerous bug fixes addressed issues in scoring, serialization, and documentation, while updates to prompts and model references ensured compatibility with a wider range of providers. The release also introduced more robust trace handling, flaky test tracking, and advanced DAG metric construction, making the AI
...
June's release brings major enhancements to evaluation flexibility and transparency, with expanded support for new models like Gemini 3 and Claude Opus 4-8, including multimodal and structured outputs. The TypeScript SDK and CLI gain new features for governance checks and comprehensive metric support, while dataset versioning and improved context synthesis enable better reproducibility and multi-file project support. Token and cost tracking are now surfaced throughout the tool, and numerous documentation updates and bug fixes improve clarity, reliability, and integration with custom models and external providers. Project organization and test workflows have also been refined, making the tool
...
February's release focused on enhancing tool and prompt support, with significant improvements to schema handling and prompt versioning. The update introduced more flexible cache directory management and expanded regional support for Confident AI requests. Numerous bug fixes addressed issues in tracing, documentation, and platform compatibility, while dependency management was streamlined to reduce installation friction. Overall, these changes improve reliability, developer experience, and cross-platform robustness.
