...
April focused on stronger tracing and observability, with richer span metadata like `turn_id` and `test_case_id`, improved OTel export propagation, and more reliable integrations across OpenAI, PydanticAI, and MCP detection. Prompt workflows also got more scalable with full branch support and branch-scoped cache/commit lookups, helping teams keep versions organized. Model and cost/metrics reliability improved through added model support and entries, corrected pricing and token accounting, and safer handling when log-probability metrics aren’t available. A set of quality fixes rounded things out, including better typing, schema handling for structured outputs, more robust golden/tool-call CSV
...
March focused on making evaluations and tracing more reliable while expanding observability integrations. Streaming and concurrent workflows now capture outputs and metrics correctly, with fixes across generator tracing, async trace evaluation, tool span ordering, and richer context propagation. Evaluation accuracy and stability improved through a safer HumanEval sandbox, more resilient scoring and caching behavior, and clearer configuration defaults and validation. The month also added deeper OpenTelemetry support via AgentCore and OpenInference with configurable export and metadata, improved provider selection across backends, and rounded things out with Windows cleanup fixes, memory-leak 防
...
February focused on making prompts and tools first-class citizens, with stronger end-to-end support for pushing, pulling, caching, and updating prompts that carry `tools` payloads and consistent JSON Schema `input_schema` generation for function calling. Prompt identity and trace logging were hardened by relying on commit hashes and reliably recording alias, label, version, and metadata across integrations, while telemetry noise was reduced and the default install slimmed down by removing the OTLP gRPC exporter dependency. Reliability improvements landed across async AWS Bedrock evaluations, Azure reasoning-model requests, offline trace evaluation routing, and the `@observe` decorator, along
...
January focused on making tracing, integrations, and provider support more reliable and configurable, with improved OpenTelemetry setup via `CONFIDENT_OTEL_URL`, better region-aware routing from API key prefixes, and cleaner telemetry defaults. Model and provider support was refreshed and expanded, including updated Gemini defaults, explicit `use_vertexai` selection, OpenRouter support, and Azure OpenAI authentication via Azure AD tokens. Evaluation and dataset workflows improved with clearer docs, richer verdict context through attached `expected_output`, `test_case_id` trace association, multi-API-key support on dataset and prompt objects, and `upload()` for `GEval` and `ConversationalGEav
