...
April focused on simplifying configuration and improving observability across evaluation and simulation workflows. Legacy knobs and hooks were removed in favor of clearer defaults, including stricter Confident authentication, streamlined metric logging, and richer, more reliable tracing with optional internal spans plus better `turn_id`/`test_case_id` propagation. Evaluation got more flexible with `metadata`/`tags` support in `SingleTurnParams` and `MultiTurnParams`, new controls for structured test-run artifacts, and a new `generate` CLI for producing synthetic goldens. The release also expanded model support and correctness for newer OpenAI and Anthropic variants, strengthened conversation
...
March focused on deeper observability for agentic workloads, adding AgentCore and OpenInference integrations that capture richer agent/tool/LLM inputs and outputs, classify spans, and export via OTLP with configurable metadata, tags, and `metric_collection`, plus a local test-mode path and clearer dependency errors. Provider ergonomics improved with environment-variable selection, better `initialize_model()` behavior for Amazon Bedrock, and fixes for Azure keyless auth and token-usage extraction across LangChain/LangGraph versions. Reliability and correctness saw broad attention, including concurrency-safe async trace evaluation, more robust tool and generator tracing, CLI result aggregation
...
February focused on making prompts, tools, and tracing more consistent and portable across the stack. Prompt identity and caching were hardened around commit hashes with better support for prompt commits endpoints, richer span metadata, and more reliable cache behavior, alongside new tool support for pulling, caching, pushing, and updating prompts with standardized JSON Schema `input_schema` handling. Reliability improvements landed across integrations and evaluations, including fixes for Azure reasoning models, Bedrock async credential handling, offline evaluation requests, generator observation, and multiple synthesizer and CLI crash cases. Packaging and platform compatibility also got a n
...
January focused on expanding model/provider compatibility and hardening tracing and evaluation workflows. We added OpenRouter support via an OpenAI-compatible API with dynamic model names, structured outputs, retries, custom headers, and flexible pricing, while also improving Gemini/Vertex AI and Azure OpenAI authentication and defaults to avoid retired models. Observability and integrations saw major stability work, including better OpenTelemetry configuration, more reliable LangChain/LangGraph and PydanticAI tracing behavior, and reduced telemetry overhead by removing the New Relic exporter. The evaluation and dataset toolchain improved with `GEval` uploads to Confident AI, clearer verdict
