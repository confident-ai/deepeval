...
:::info
Offline evaluation is supported for CrewAI `Agent`. And only metrics with parameters `input`, `output`, `expected_output` and `tools_called` are supported for evaluation.
:::
