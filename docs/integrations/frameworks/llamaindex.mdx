...
if response_object is not None:
    generated_output = response_object.response
...
Unit testing LlamaIndex is as simple as defining an `EvaluationDataset` and generating `generated_output`s and `retrieval_context`s at evaluation time. Building upon the previous example:
...
:::note
In this example, we initialized an `EvaluationDataset` with a `Golden` instead of an `LLMTestCase`. This is because we're generating `generated_output`s and `retrieval_context`s at evaluation time, meaning we cannot initialize with test cases since an `LLMTestCase` requires an `generated_output` to create. Remember, a `Golden` do not require an `generated_output`, so whilst test cases are always ready for evaluation, a golden isn't.
:::
