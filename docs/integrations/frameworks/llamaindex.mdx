...
Setup tracing for LlamaIndex and create a `FunctionAgent`. Use `trace` context manager to set up the `AgentSpanContext` (or `LlmSpanContext` if you want to evaluate the LLM span).
