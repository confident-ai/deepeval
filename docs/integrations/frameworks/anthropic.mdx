...
To evaluate your Claude application during development, opt for local evals. This allows you to run evaluations on the data points directly on your machine.
...
:::info
With `deepeval`’s `Anthropic` client, input and output are auto-extracted for every generation, so you can run evaluations like **Answer Relevancy** without extra setup. For metrics needing extra info (e.g. **Faithfulness**), just pass `retrieval_context` or `context` in the `trace()` context.
:::

### Evaluating Claude within Components

For component-level evaluation, use `deepeval`'s `Anthropic` client and add the `@observe` decorator to your component functions. Pass your chosen metrics via the `trace()` context.
...
To evaluate your Claude application's traces in production, enable online evals. This runs automatic evaluations on all incoming traces using Confident AI's server.
