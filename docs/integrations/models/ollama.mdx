...
There is **ONE** mandatory and **THREE** optional parameters when creating an `OllamaModel`:

- `model`: A string specifying the name of the Ollama model to use. Defaults to `OLLAMA_MODEL_NAME` if not passed; raises an error at runtime if unset.
