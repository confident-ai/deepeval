...
```bash
deepeval set-moonshot --prompt-api-key \
...
- [Optional] `cost_per_input_token`: A float specifying the cost for each input token for the provided model. Defaults to `MOONSHOT_COST_PER_INPUT_TOKEN` if not passed; raises an error at runtime if DeepEval has no pricing metadata for the model and the parameter is unset.
- [Optional] `cost_per_output_token`: A float specifying the cost for each output token for the provided model. Defaults to `MOONSHOT_COST_PER_OUTPUT_TOKEN` if not passed; raises an error at runtime if DeepEval has no pricing metadata for the model and the parameter is unset.
