...
# OpenAI model
model = LiteLLMModel(
    model_name="openai/gpt-3.5-turbo",  # Provider must be specified
...
# OpenAI
model = LiteLLMModel(model_name="openai/gpt-3.5-turbo")
metric = AnswerRelevancyMetric(model=model)

# Anthropic
model = LiteLLMModel(model_name="anthropic/claude-3-opus")
metric = AnswerRelevancyMetric(model=model)

# Google
model = LiteLLMModel(model_name="google/gemini-pro")
metric = AnswerRelevancyMetric(model=model)

# LM Studio
model = LiteLLMModel(
    model_name="lm-studio/Meta-Llama-3.1-8B-Instruct-GGUF",
...
```python
model = LiteLLMModel(
    model_name="custom/your-model-name",  # Provider must be specified
