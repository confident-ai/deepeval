...
To evaluate your Qdrant retriever, you'll first need to prepare an `LLMTestCase`, which includes an `input`, `generated_output`, `expected_output`, and `retrieval_context`. This requires defining an `input` and `expected_output` before generating a response and extracting the retrieval contexts.
...
To generate the response or `generated_output` from your RAG pipeline, you'll first need to retrieve relevant contexts from your `Qdrant` collection. To achieve this, we'll define a `search` function that embeds the `input` using the same embedding model (`all-MiniLM-L6-v2`) as above, then search for the top 3 most similar vectors and extract the corresponding texts.
...
generated_output = generate(prompt) # hypothetical function, replace with your own LLM
print(generated_output)
```

We'll then pass the input and expected output that was initially defined into an `LLMTestCase`, along with the generated output and retrieval context that we generated and searched for.
...
Before proceeding with evaluations, let's examine the `generated_output` that was generated:
