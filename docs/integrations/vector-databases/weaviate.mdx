...
- **Constructing a Test Case**: to do so, define an `input` query that represents a typical search scenario and prepare the expected output. Then generate the `generated_output` for the given input and extract the retrieved context during generation.
...
The first step in generating the `generated_output` from your RAG pipeline is retrieving the relevant `retrieval_context` from your Qdrant collection based on the input query. Below is a function that encodes the query, searches for the top 3 most relevant vectors in Qdrant, and extracts the corresponding text from the retrieved results.
...
generated_output = generate(prompt)  # Replace with your LLM function
print(generated_output)
```

With both the `generated_output` and `retrieval_context` generated, we now have all the necessary parameters to construct our test case:
...
Before proceeding with the evaluation, let's examine the generated `generated_output`.
...
for vectorizer in ["all-MiniLM-L6-v2", "bge-small-en", "nomic-embed-text-v1"]:
    ...
    test_case = LLMTestCase(
        input=query,
        generated_output=llm.generate(query, retrieval_context),
        retrieval_context=retrieval_context,
