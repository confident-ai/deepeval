...
:::information
By default, `input` and `generated_output` are required for all metrics. However, `retrieval_context`, `context`, and `expected_output` are optional, and different metrics may or may not require additional parameters. To check the specific requirements, [visit the metrics section](/docs/metrics-introduction).  
:::
...
Printing the `generated_output` generated by our RAG pipeline yields the following example:
...
With all the elements ready, we'll create an `LLMTestCase` by providing the input and expected output, along with the generated output and retrieved context.
