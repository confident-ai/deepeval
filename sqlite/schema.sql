...
--
-- This file is the single source of truth for the DDL that both SDKs embed:
--   Python:     deepeval/sqlite_store/store.py        (_SCHEMA)
--   TypeScript: typescript/src/sqlite-store/store.ts  (SCHEMA)
-- Parity tests in each SDK fail if the embedded copy drifts from this file, and
-- the docs page (docs/content/docs/evaluation-local-backend-storage.mdx) is
-- checked against it too. Edit here first, then mirror into both SDKs and bump
-- SCHEMA_VERSION.
--
