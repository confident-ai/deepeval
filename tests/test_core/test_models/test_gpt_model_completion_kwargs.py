"""Tests for GPTModel completion_kwargs parameter"""
...
class TestGPTModelCompletionKwargs:
    """Test suite for GPTModel completion_kwargs functionality"""

    def test_init_without_completion_kwargs(self):
        """Test that GPTModel initializes correctly without completion_kwargs"""
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            model = GPTModel(model="gpt-4o")
            assert model.completion_kwargs == {}
            assert model.model_name == "gpt-4o"

    def test_init_with_completion_kwargs(self):
        """Test that GPTModel initializes correctly with completion_kwargs"""
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            completion_kwargs = {
    ...
    def test_init_with_both_client_and_completion_kwargs(self):
        """Test that client kwargs and completion_kwargs are kept separate"""
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
    ...
    def test_generate_with_completion_kwargs(self, mock_openai_class):
        """Test that completion_kwargs are passed to generate method"""
        # Setup mock
        mock_client = Mock()
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            model = GPTModel(
                model="gpt-5",
                completion_kwargs={"reasoning_effort": "high", "seed": 123},
            )

            # Call generate
            output, cost = model.generate("test prompt")

            # Verify the completion was called with completion_kwargs
    ...
    @patch("deepeval.models.llms.openai_model.OpenAI")
    def test_generate_without_completion_kwargs(self, mock_openai_class):
        """Test that generate works without completion_kwargs"""
        # Setup mock
        mock_client = Mock()
        mock_openai_class.return_value = mock_client
        mock_completion = Mock()
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            ...
            # Call generate without completion_kwargs
            output, cost = model.generate("test prompt")
    ...
    @patch("deepeval.models.llms.openai_model.OpenAI")
    def test_generate_with_schema_and_completion_kwargs(
        self, mock_openai_class
    ):
        """Test that completion_kwargs are passed when using structured output"""
        # Setup mock
        mock_client = Mock()
        mock_openai_class.return_value = mock_client
        mock_beta = Mock()
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            model = GPTModel(
                ...
                completion_kwargs={"reasoning_effort": "low", "top_p": 0.9},
            )

            # Call generate with schema
            output, cost = model.generate("test prompt", SampleSchema)

            # Verify the parse method was called with completion_kwargs
    ...
    async def test_async_generate_with_completion_kwargs(
        self, mock_async_openai_class
    ):
        """Test that completion_kwargs are passed to async generate method"""
        # Setup mock
    ...
    @patch("deepeval.models.llms.openai_model.AsyncOpenAI")
    async def test_async_generate_with_schema_and_completion_kwargs(
        self, mock_async_openai_class
    ):
        """Test that completion_kwargs are passed to async generate with schema"""
        # Setup mock
        mock_client = MagicMock()
        mock_async_openai_class.return_value = mock_client
        mock_beta = MagicMock()
    ...
    @patch("deepeval.models.llms.openai_model.OpenAI")
    def test_generate_raw_response_with_completion_kwargs(
        self, mock_openai_class
    ):
        """Test that completion_kwargs are passed to generate_raw_response"""
        # Setup mock
        mock_client = Mock()
        mock_openai_class.return_value = mock_client
        mock_completion = Mock()
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            ...
            # Verify the completion was called with both method params and completion_kwargs
            mock_client.chat.completions.create.assert_called_once_with(
                model="gpt-4o",
                messages=[{"role": "user", "content": "test prompt"}],
                temperature=0,
    ...
    @patch("deepeval.models.llms.openai_model.OpenAI")
    def test_generate_samples_with_completion_kwargs(self, mock_openai_class):
        """Test that completion_kwargs are passed to generate_samples"""
        # Setup mock
        mock_client = Mock()
        mock_openai_class.return_value = mock_client
        mock_response = Mock()
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            model = GPTModel(
                model="gpt-4o", completion_kwargs={"reasoning_effort": "low"}
            )
    ...
    def test_backwards_compatibility(self):
        """Test that existing code without completion_kwargs still works"""
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
    ...
    def test_gpt5_auto_temperature_adjustment(self):
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            ...
            for model_name in gpt5_models:
                ...
                assert model.completion_kwargs == {"reasoning_effort": "high"}

    def test_empty_completion_kwargs(self):
        """Test that empty completion_kwargs dict works correctly"""
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            model = GPTModel(model="gpt-4o", completion_kwargs={})
            assert model.completion_kwargs == {}

    def test_none_completion_kwargs(self):
        """Test that None completion_kwargs is handled correctly"""
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            model = GPTModel(model="gpt-4o", completion_kwargs=None)
            assert model.completion_kwargs == {}
