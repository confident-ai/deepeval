...
def test_build_goldens_from_dataset_multi_turn():
    raw_dataset = {
        "goldens": [
            {
                "scenario": "User wants to buy a ticket",
                "expected_outcome": "Ticket purchased",
            }
        ]
    }
    goldens = _build_goldens_from_dataset(raw_dataset)
...
def test_build_goldens_from_dataset_mixed_error():
    ...
    with pytest.raises(
        RuntimeError, match="Mixed golden types are not allowed"
    ):
...
def test_build_goldens_from_dataset_alias_valid(mock_eval_dataset_class):
    ...
    mock_instance.goldens = [
        Golden(input="test alias input", expected_output="test alias output")
    ]
...
def test_build_metrics_valid():
    raw_metrics = [{"metric": "AnswerRelevancyMetric", "threshold": 0.6}]
...
def test_build_metrics_invalid_class():
    raw_metrics = [{"metric": "NonExistentMetric"}]
...
def test_build_metrics_invalid_kwargs():
    raw_metrics = [{"metric": "AnswerRelevancyMetric", "invalid_arg": 123}]
    with pytest.raises(
        TypeError, match="Failed to construct AnswerRelevancyMetric"
    ):
        _build_metrics(raw_metrics)
...
def test_load_model_callback_missing_file():
    with pytest.raises(
        FileNotFoundError, match="Model callback file not found"
    ):
...
def test_load_model_callback_missing_function(tmp_path):
    ...
    with pytest.raises(
        AttributeError, match="Function 'my_callback' not found"
    ):
...
def test_post_github_pr_comment_new(
    mock_post, mock_get, mock_env_get, tmp_path
):
...
def test_post_github_pr_comment_update(
    mock_patch, mock_get, mock_env_get, tmp_path
):
    # Setup mock environment
    mock_env_get.side_effect = lambda k: {
        "GITHUB_TOKEN": "fake_token",
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = [
        {"id": 456, "body": "🚀 DeepEval Evaluation Results previously"}
    ]
    mock_get.return_value = mock_response
