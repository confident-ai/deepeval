...
class TestMCPFunctionality:
    @patch("deepeval.test_case.mcp.validate_mcp_servers")
    def test_mcp_servers_validation(self, mock_validate):
        mock_validate.return_value = None

        mcp_server = MCPServer(server_name="test_server")

        # Valid case
        test_case = LLMTestCase(input="test", mcp_servers=[mcp_server])
        assert test_case.mcp_servers == [mcp_server]

        # Invalid case - not a list
        with pytest.raises(
            TypeError,
            match="'mcp_server' must be None or a list of 'MCPServer'",
        ):
            LLMTestCase(input="test", mcp_servers="not a list")

        # Invalid case - list with wrong type
        with pytest.raises(
            TypeError,
            match="'mcp_server' must be None or a list of 'MCPServer'",
        ):
            LLMTestCase(
                input="test", mcp_servers=[mcp_server, "not an mcp server"]
            )

    def test_mcp_servers_none_case(self):
        test_case = LLMTestCase(input="test", mcp_servers=None)
        assert test_case.mcp_servers is None

    def test_mcp_tools_called_invalid_types(self):
        # Invalid case - not a list
        with pytest.raises(
            TypeError,
            match="The 'tools_called' must be a list of 'MCPToolCall'",
        ):
            LLMTestCase(input="test", mcp_tools_called="not a list")
        ...
        with pytest.raises(
            ...
            LLMTestCase(input="test", mcp_tools_called=["not an mcp tool call"])

    def test_mcp_resources_called_invalid_types(self):
        # Invalid case - not a list
        with pytest.raises(
            TypeError,
            match="The 'resources_called' must be a list of 'MCPResourceCall'",
        ):
            LLMTestCase(input="test", mcp_resources_called="not a list")

    def test_mcp_prompts_called_invalid_types(self):
        # Invalid case - not a list
        with pytest.raises(
            TypeError,
            match="The 'prompts_called' must be a list of 'MCPPromptCall'",
        ):
            LLMTestCase(input="test", mcp_prompts_called="not a list")

    def test_mcp_fields_none_cases(self):
        test_case = LLMTestCase(
            input="test",
            mcp_servers=None,
            mcp_tools_called=None,
            mcp_resources_called=None,
            mcp_prompts_called=None,
        )
    ...
    def test_mcp_empty_lists(self):
        # This will skip the type validation since empty lists pass the initial checks
        test_case = LLMTestCase(
            input="test",
            mcp_servers=[],
            mcp_tools_called=[],
        ...
        assert test_case.mcp_servers == []
        assert test_case.mcp_tools_called == []
        assert test_case.mcp_resources_called == []
        assert test_case.mcp_prompts_called == []
