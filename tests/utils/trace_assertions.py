...
def assert_trace_json(json_path: str, mode: str = "assert"):
    """
    Decorator that either:
      - mode="assert" (default): compares trace structure against an expected JSON file
      - mode="generate": writes the captured trace to json_path
    ...
    """
    if mode == "generate":
        return generate_trace_json(json_path)

    if mode != "assert":
        raise ValueError("mode must be 'assert' or 'generate'")
