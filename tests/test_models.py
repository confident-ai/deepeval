...
@pytest.mark.parametrize("model_class", [GPTModel, AzureOpenAIModel, OllamaModel, LocalModel])
def test_generate_without_schema_returns_string(model_class):
    ...
    assert isinstance(output, str), f"{model_class.__name__} should return a string when no schema is provided"

@pytest.mark.parametrize("model_class", [GPTModel, AzureOpenAIModel, OllamaModel, LocalModel])
def test_generate_with_schema_returns_city_object(model_class):
    model: DeepEvalBaseLLM = model_class()
    output, _ = model.generate(input_text, City)
    assert isinstance(output, City), f"{model_class.__name__} should return a City object when schema is provided"
