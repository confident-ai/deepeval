"""Offline tests for ContextualRecallMetric's degenerate-input handling.

Contextual recall measures how well ``expected_output`` can be attributed to
``retrieval_context``. When either is degenerate (no context to recall from, or
no expected output to verify) the answer is trivially 0. Previously the metric
still round-tripped through the eval LLM for these cases — wasting latency/cost
and producing non-deterministic results. We now short-circuit to a fixed,
explainable outcome.
...
def _make_metric() -> ContextualRecallMetric:
...
class TestDegenerateShortCircuit:
    ...
    def test_sync_and_async_agree(self):
        ...
        async_metric = ContextualRecallMetric(
            model=_stub_model(),
            async_mode=True,
            include_reason=True,
        )
    ...
    def test_non_degenerate_is_not_empty(
        self,
    ):
        metric = _make_metric()
...
class TestLLMNotContacted:
    ...
    def test_empty_expected_never_calls_a_generate(self):
        metric = ContextualRecallMetric(model=_stub_model(), async_mode=True)
