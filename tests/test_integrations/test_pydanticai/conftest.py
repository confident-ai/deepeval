...
def deepeval_isolated_no_disk(tmp_path, monkeypatch):
    ...
    for mod in (tr, ds):
        ...
        monkeypatch.setattr(mod, "LATEST_TEST_RUN_FILE_PATH", str(tmp_latest), raising=False)
    ...
    monkeypatch.setattr(tr.TestRunManager, "save_test_run",             lambda self, *a, **k: None, raising=False)
    monkeypatch.setattr(tr.TestRunManager, "save_final_test_run_link",  lambda self, *a, **k: None, raising=False)
    monkeypatch.setattr(tr.TestRunManager, "save_test_run_locally",     lambda self: None,          raising=False)
