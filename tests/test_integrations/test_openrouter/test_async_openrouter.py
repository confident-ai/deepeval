...
# Clients are built inside each test, not at module scope: each test gets its own
# event loop, and an async HTTP client outlives only the loop it was created on.
def openrouter_client() -> OpenRouter:
    return OpenRouter()


def openai_client() -> AsyncOpenAI:
    """The same gateway via the OpenAI SDK: provider "OpenRouter", integration "OpenAI"."""
    return AsyncOpenAI(
